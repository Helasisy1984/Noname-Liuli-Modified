		...
		game:{
					...
					switch(game.me.type){
						case 'human':
							...
							if(!_status.brawl||!_status.brawl.chooseCharacterFixed){
								if(!ui.cheat&&get.config('change_choice')&&!game.rz_isPaiWeiing) ui.create.cheat();
								if(!ui.cheat2&&get.config('free_choose')&&!game.rz_isPaiWeiing) ui.create.cheat2();
							}
							break;
						case 'mech':
			...
			chooseCharacterSiguo:function(){
				...
				next.setContent(function(){
					...
					if(!ui.cheat&&get.config('change_choice')&&!game.rz_isPaiWeiing){
						ui.create.cheat();
					}
					if(!ui.cheat2&&get.config('free_choose')&&!game.rz_isPaiWeiing){
						ui.create.cheat2();
					}
					'step 2'
					if(ui.cheat){
			...
			chooseCharacterTwo:function(){
				...
				next.setContent(function(){
					...
					var choose=[];
					event.list=list;
					_status.characterlist=((lib.config["extension_段位制裁_eloPlayers"]&&game.rz_isPaiWeiing)?get.paiweiPlayers(20):list4);
					...
					var next=game.me.chooseButton(true,dialog,basenum).set('onfree',true)
					/*if(basenum==1) next.set('selectButton',function(){
					    if(ui.selected.buttons.length) {
					        return [0,1];
					    }
					    return [1,2];
					...
					if(event.current==game.me||(event.four_assign&&event.current.side==game.me.side)){
						...
						for(var i=0;i<dialog.buttons.length;i++){
							if(names.contains(dialog.buttons[i].link)){
								...
								dialog.buttons[i].style['pointer-events']='none !important';
								dialog.buttons[i].style.opacity=0.3;
							}else {
							    dialog.buttons[i].style['pointer-events']='';
				...
				dieAfter:function(source){
					if(_status.connectMode){
						...
						if(game.rz_isPaiWeiing) {
							...
							var me=game.me._trueMe||game.me;
							for(var i=0;i<game.players.length;i++){
							    if(game.players[i].side==me.side){
							        game.winRzL+=0.4;
							    }else {
							        game.losRzL+=0.4;
							    }
							}
							if(game.winRzL<0) game.winRzL=0;
							if(game.losRzL<0) game.losRzL=0;
							var twoPlus=0;
							if(get.config('replace_character_two')) twoPlus=10;
							if(game.roundNumber<9+twoPlus) {
							    game.losRzl+=0.6;
							    game.winRzL+=0.6;
							}else if(game.roundNumber<16+twoPlus) {
							    game.losRzl+=0.4;
							    game.winRzL+=0.4;
							}else if(game.roundNumber<24+twoPlus) {
							    game.losRzl+=0.2;
							    game.winRzL+=0.2;
							}
						}
