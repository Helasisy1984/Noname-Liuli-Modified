	...
	return {
		...
		start:function(){
			...
			if(!lib.config.new_tutorial){
				...
				var step1=function(){
					if(lib.config.guide_check) {
					    game.createCss(`.to_the_top_guide{
					        z-index: 10000 !important;
					        opacity: 1 !important;
					    }`);
				        ui.arena.style.display='none';
					    ui.arena.style.visibility='hidden';
					    ui.background.style.zIndex=9999;
					    ui.window.classList.add('noclick_important');
    					ui.control.classList.add('noclick_click_important');
    					ui.create.dialog('欢迎来到无名杀，是否进入新手向导？').style.visibility='visible';
    					ui.dialog.add('<div class="text center">跳过后，你可以在选项-其它中恢复新手向导');
    					ui.auto.hide();
    					ui.arena.classList.remove('only_dialog');
    					ui.arena.classList.add('to_the_top_guide');
    				    game.saveConfig('new_tutorial',true);
    				    game.saveConfig('guide_check',false);
    				    setTimeout(function(){
					    ...
					    ui.create.control('继续',step2).style.visibility='visible';
					}else {
    					ui.create.dialog('<span style="font-size: 30px">三国杀·琉璃版</span>');
    					ui.dialog.add('<div class="left">　　本游戏由赫拉西斯匠心打造，以村长水乎的无名杀为基础，整合多个大型扩展最终而成。我们衷心希望为三国杀爱好者呈现一款完全免费、体验流畅、还原手机版风格的优秀作品。游戏完全开源，请勿用于盈利或其他非法用途。<br>　　最后，祝您能够畅享游戏的乐趣！');
    					ui.dialog.style.transform='translateY(-40%)';
    					ui.auto.hide();
    				    game.saveConfig('new_tutorial',true);
    					ui.create.control("开始体验"/*'跳过向导'*/,function(){
    						clear();
    						clear2();
    						game.resume();
				...
				var step2=function(){
					if(!lib.config.phonelayout){
						...
						'，将布局改成移动可以使按钮变大').style.visibility='visible';
						...
						ui.create.control('继续',step3).style.visibility='visible';
					}
					else{
						step3();
					}
				...
				var step3=function(){
					if(lib.config.touchscreen){
						clear();
						ui.create.dialog('游戏在初始化时已经预设好配置'/*'触屏模式中，下划可以显示菜单，上划可以切换托管，双指单击可以暂停'*/).style.visibility='visible';
						ui.dialog.add('<div class="text center">在菜单中的选项或者扩展设置中可以根据喜好自行修改'/*'<div class="text center">你可以在选项-通用-中更改手势设置'*/);
						ui.create.control('继续',step4).style.visibility='visible';
					}
					else{
						step4();
					}
				...
				var step4=lib.genAsync(function*(){
					clear();
				    ui.background.style.zIndex=8;
					ui.window.classList.add('noclick_important');
					ui.click.configMenu();
					var easyDialog=function(str,func) {
					    clear();
					    var dialog=ui.create.dialog(str);
					    dialog.classList.add('to_the_top_guide');
					    //dialog.classList.add('noclick_click_important');
					    dialog.style.visibility='visible';
					    var control=ui.create.control('继续', func);
					    control.style.top='calc(100% - 105px)';
					    control.classList.add('noclick_click_important');
					    control.style.visibility='visible';
					    return control;
					};
					yield new Promise(resolve => easyDialog('在菜单中，可以进行各项设置',resolve));
					yield new Promise(resolve => easyDialog('在开始页面可以对游玩的模式进行规则的调整',resolve));
					ui.click.menuTab('选项');
					yield new Promise(resolve => easyDialog('在选项页面可以修改游戏的速度和外观等内容',resolve));
					//yield new Promise(resolve => easyDialog('如果你感到游戏较卡，可以开启流畅模式',resolve));
					yield new Promise(resolve => easyDialog('在技能一栏中，可以设置自动发动或双将禁配的技能',resolve));
					ui.click.menuTab('武将');
					yield new Promise(resolve => easyDialog('在武将页面双击或长按武将可以查看武将信息',resolve));
					yield new Promise(resolve => easyDialog('游戏默认只展示部分武将池，可以在“手杀将池”扩展中关闭隐藏',resolve));
					//yield new Promise(resolve => easyDialog('在武将或卡牌一栏中，单击武将/卡牌可以将其禁用',resolve));
					ui.click.menuTab('卡牌');
					yield new Promise(resolve => easyDialog('在卡牌页面长按卡牌可以查看描述，单击可以设置禁用',resolve));
					ui.click.menuTab('扩展');
					yield new Promise(resolve => easyDialog('游戏自带一系列支持美化和功能的扩展，可以根据喜好对其功能进行调整',resolve));
					yield new Promise(resolve => easyDialog('如果想要发挥创造力，可以在列表底部点击“制作扩展”进行创作',resolve));
					ui.click.menuTab('其它');
					yield new Promise(resolve => easyDialog('对战的时候，玩家可以在控制一栏进行作弊',resolve));
					yield new Promise(resolve => easyDialog('在命令一栏中，开发者可以输入游戏指令进行调试',resolve));
					//yield new Promise(resolve => easyDialog('在战局中可以输入游戏命令，或者管理录像',resolve));
					//yield new Promise(resolve => easyDialog('在帮助中，可以检查更新和下载素材',resolve));
					ui.click.configMenu();
				...
				var step5=function(){
				    ui.background.style.opacity=0;
				    ui.background.style.setProperty('transition', 'opacity 1s linear', 'important');
				    ui.background.style.zIndex=9999;
				    setTimeout(function(){
				        ui.background.style.opacity=1;
				    },10);
					clear();
					ui.create.dialog('对于本游戏的问题，可以在如真似幻的登陆页面右上方查看联系方式进行反馈').style.visibility='visible';
					ui.create.control('继续',step6).style.visibility='visible';
				};
				var step6=function(){
					clear();
					ui.create.dialog('如果还有其它问题，欢迎来到百度无名杀吧进行交流').style.visibility='visible';
					ui.create.control('完成',function(){
						clear();
						clear2();
						game.resume();
					    game.saveConfig('new_tutorial',true);
					    ui.arena.classList.remove('to_the_top_guide');
						setTimeout(function(){
						    game.reload();
						},100);
					}).style.visibility='visible';
		...
		game:{
				...
				next.setContent(function(){
 				...
 				game.bZhu.isZhu=true;
					...
					if(game.me==game.rZhu||game.me==game.bZhu){
						...
						game.me.chooseButton(true,['请选择您的武将牌',[list,'character'],""]).set('selectButton',function(){
					        ui.update();
					        return 1;
					    });
					}
					...
					if(!event.isZhu){
						...
						game.me.chooseButton(true,['请选择您的武将牌',[event.map[group].randomRemove(5),'character'],""]).set('selectButton',function(){
					        ui.update();
					        return 1;
					    });
					}
			...
			chooseCharacter:function(){
				...
				next.setContent(function(){
					...
					if(get.is.double(name)){
						...
						list.remove('shen');
						list.remove('devil');
						game.me.chooseControl(list).set('prompt','请选择'+(lib.character[name][1]=='shen'?'神':'魔')+'武将的势力');
					}
					"step 2"
			...
			chooseCharacterOL:function(){
				...
				next.setContent(function(){
					...
					if((game.zhu.group=='shen'||game.zhu.group=='devil')&&!game.zhu.isUnseen(0)){
						var list=['wei','shu','wu','qun','jin','key'];
						for(var i=0;i<list.length;i++){
							if(!lib.group.contains(list[i])) list.splice(i--,1);
							else list[i]=['','','group_'+list[i]];
						}
						game.zhu.chooseButton(['请选择'+(game.zhu.group=='shen'?'神':'魔')+'武将的势力',[list,'vcard']],true).set('ai',function(){
							return Math.random();
						});
					}
					...
					for(var i in result){
						...
						if(get.is.double(result[i][0])||
						 lib.character[result[i][0]]&&(lib.character[result[i][0]][1]=='shen'||lib.character[result[i][0]][1]=='devil')&&!lib.character[result[i][0]][4].contains('hiddenSkill')) shen.push(lib.playerOL[i]);
					}
							...
							game.broadcastAll(function(){
								...
								if(game.zhu&&game.zhu.isAlive()&&get.population('nei')==1&&get.config('nei_fullscreenpop')) {
								    if(lib.config.extension_十周年UI_enable) {
								        //alert('主公vs内奸');
								        decadeUI.animation.playSpine({ name: 'neijian', scale: 0.7 });
								        game.playAudio('..','extension','十周年UI','audio','neijian.mp3');
								    }else {
								        game.me.$fullscreenpop('<span style="font-family:xinwei"><span data-nature="fire">陛下</span><span data-nature="soil">何故</span><span data-nature="thunder">谋反</span></span>',null,null,false);
								    }
								}
							});
						}
