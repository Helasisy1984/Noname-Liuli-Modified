	...
	return {
		...
		start:function(){
			...
			if(_status.mode=='normal'){
				if(lib.config["extension_段位制裁_eloPlayers"]&&game.rz_isPaiWeiing) {
				    game.broadcastAll(function(singleTranslate,characterSingle){
    					_status.mode='normal';
    					_status.characterlist=get.paiweiPlayers(12);
		...
		game:{
			...
			checkResult:function(){
				var player=(game.me._trueMe||game.me);
				if(game.rz_isPaiWeiing&&_status.characterChoice) {
				    game.winRzL=Math.max(_status.characterChoice[player.identity].slice(0).length-3,0)/2;
				    game.losRzL=Math.max(_status.characterChoice[player.enemy.identity].slice(0).length-3,0)/2;
				}
			...
			chooseCharacter:function(){
				...
				next.setContent(function(){
					...
					event.cho=false;
					"step 1"
					event.zhuhas=_status.characterlist.randomRemove(3);
					event.fanhas=_status.characterlist.randomRemove(3);
					_status.characterChoice={
						zhu:event.zhuhas.slice(),
						fan:event.fanhas.slice(),
					...
					if(event.cho=='fan') event.cholist['fan']=event.cholist['fan'].concat(result.links);
					if(event.cholist['fan'].length<1) {
					    event.goto(2);
					    return;
					}
					game.broadcastAll(function(link,choosing,first,id){
						...
						if(dialog){
							if(event.choosing==game.me){
								choosing='你';
							}
							else{
								choosing='对手';
							}
							dialog.content.firstChild.innerHTML=choosing+'选择了'+get.translation(event.cholist[event.cho]);
							for(var i=0;i<dialog.buttons.length;i++){
							    if(game.me==game.zhu) {
							        if(event.cholist['zhu'].contains(dialog.buttons[i].link)) {
							            dialog.buttons[i].classList.add('glow');
							        }else if(event.cholist['fan'].contains(dialog.buttons[i].link)) {
							            dialog.buttons[i].style.opacity=0.3;
							        }
							    }else {
							        if(event.cholist['fan'].contains(dialog.buttons[i].link)) {
							            dialog.buttons[i].classList.add('glow');
							        }else if(event.cholist['zhu'].contains(dialog.buttons[i].link)) {
							            dialog.buttons[i].style.opacity=0.3;
							        }
							    }
					...
					_status.characterChoice.all.removeArray(result.links);
					if(event.cho=='zhu') event.cholist['zhu']=event.cholist['zhu'].concat(result.links);
					if(event.cholist['zhu'].length<2) {
					    event.goto(3);
					    return;
					}
					game.broadcastAll(function(link,choosing,first,id){
					...
					event.cho='fan';
					event.choosing=game.fan;
					var next=game.fan.chooseButton(true);
					next.filterButton=function(button){
						return _status.event.canChoose.contains(button.link);
					};
					next.dialog=event.videoIdx;
					...
					_status.characterChoice.zhu.addArray(result.links);
					_status.characterChoice.all.removeArray(result.links);
					event.cholist['zhu']=event.cholist['zhu'].concat(result.links);
					if(event.cholist['zhu'].length<3) {
					    event.goto(5);
					    return;
					}
					game.broadcastAll(function(link,choosing,first,id){
					...
					"step 7"
					game.forbidResumeJinDu=true;
					game.initJinDuTiao('pause',true);
					_status.characterChoice['zhu']=event.cholist['zhu'].concat(event.zhuhas);
					_status.characterChoice['fan']=event.cholist['fan'].concat(event.fanhas);
					game.broadcastAll('closeDialog',event.videoIdx);
					event.playcholist=[];
					//if(_status.mode=='changban') {
					    event.videoIdxd=lib.status.videoId++;
					    game.broadcastAll(function(id,list){
				    		var dialog=ui.create.dialog('请选择出场武将',[_status.characterChoice[game.me.identity],'character']);
			    			dialog.videoId=id;
			    		},event.videoIdxd,game.me);
					//}
					"step 8"
					if(_status.mode=='changban') {
					game.broadcastAll(function(link,choosing,first,id){
						...
						//dialog.videoId=id;
						if(dialog){
							if(!event.playcholist.length){
								choosing='主';
							}
							else{
								choosing='副';
							}
							dialog.content.firstChild.innerHTML='请选择出场'+choosing+'将';
							for(var i=0;i<dialog.buttons.length;i++){
								if(event.playcholist.contains(dialog.buttons[i].link)) {
							        dialog.buttons[i].classList.add('glow');
							    }
							}
						}
					},result.links,game.me,false,event.videoIdxd);
					var next=game.me.chooseButton(true);
					next.filterButton=function(button){
						return !_status.event.cantChoose.contains(button.link);
					};
					next.dialog=event.videoIdxd;
					next.cantChoose=event.playcholist;
					next.ai=function(){return Math.random()};
					}else {
					//改成这样选不会闪
					game.broadcastAll(function(link,choosing,first,id){
						...
						//dialog.videoId=id;
						if(dialog){
							dialog.content.firstChild.innerHTML='请选择出场武将';
							for(var i=0;i<dialog.buttons.length;i++){
								if(event.playcholist.contains(dialog.buttons[i].link)) {
							        dialog.buttons[i].classList.add('glow');
							    }
					...
					next.ai=function(){return Math.random()};
					    //这个会闪不知道什么原因
					    //game.me.chooseButton(true,['请选择出场武将',[_status.characterChoice[game.me.identity],'character']],1);
					}
					"step 9"
					event.playcholist=event.playcholist.concat(result.links);
					if(_status.mode=='changban'&&event.playcholist.length<2) event.goto(8);
					"step 10"
					game.forbidResumeJinDu=false;
					//if(_status.mode=='changban') 
					game.broadcastAll('closeDialog',event.videoIdxd);
					"step 11"
					
					//if(event.cholist['zhu'].length>3) alert_old(event.cholist['zhu'][3][0]);
					//if(event.cholist['fan'].length>3) alert_old(event.cholist['fan'][3][0]);
					game.me.init(event.playcholist[0],_status.mode=='changban'?event.playcholist[1]:null);
					_status.characterChoice[game.me.identity].removeArray(event.playcholist);
					//_status.characterChoice=game.me==game.zhu?event.cholist['fan']:event.cholist['zhu'];
