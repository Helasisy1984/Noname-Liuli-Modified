			...
			chooseCharacter:function(){
				...
				next.setContent(function(){
					...
					for(i in lib.character){
						...
						//Helasisy修：国战武将和其他武将同步屏蔽
						if(i.indexOf('gz_')==0&&lib.filter.characterDisabled(i.slice(3))) continue;
						...
						next.filterButton=function(button){
							...
							var filterChoice=function(name1,name2){
								...
								if(doublex){
									var double=get.is.double(name2,true);
									if(double) {
									    //if(!doublex.some(group=>double.contains(group))) unselect(button);
									    return doublex.some(group=>double.contains(group));
									}
									//if(!doublex.contains(group2)) unselect(button);
									return doublex.contains(group2);
								}
								else{
									if(group1=='ye') {
									    //if(!(group2!='ye')) unselect(button);
									    return group2!='ye';
									}
									var double=get.is.double(name2,true);
									if(double) {
									    //if(!double.contains(group1)) unselect(button);
									    return double.contains(group1);
									}
									//if(!(group1==group2)) unselect(button);
									return group1==group2;
								}
							};
							if(!ui.selected.buttons.length){
								if(!ui.dialog.buttons.some(but=>{
							...
							if(!filterChoice(ui.selected.buttons[0].link,button.link)) unselect(button);
