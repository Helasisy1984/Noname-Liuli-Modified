...
game.import('mode',function(lib,game,ui,get,ai,_status){
	return {
		...
		start:function(){
		    ...
		    dialog.style["background-image"]=('url("'+lib.assetURL+'image/dialog_brawl.png")');
			...
			var createNode=function(name){
				...
				//哇！金色传说
				var caption=game.modeGoldTitle(info.name);
				var modeinfo='';
				if(info.mode){
			...
			var clickStart=function(){
				...
				if(active){
					...
					//名字是这！
					game.specialModeTitle=info.name;
					if(info.init){
						info.init();
					}
				}
			...
			var start=ui.create.div('.menubutton.round.highlight',game.addGoldFont('斗','#shadow;'),dialog.content,clickStart);
			start.style["background-image"]=('url("'+lib.assetURL+'image/start.png")');
			start.style['background-size']='100% 100%';
		...
		brawl:{
			huanhuazhizhan:{
				...
				showcase:function(init){
                    ...
                    // 椭圆参数 - 调整比例获得更好的环绕效果
                    var lx = this.offsetWidth/2 - 150;  // 水平半径
                    var ly = Math.min(lx * 0.6, (this.offsetHeight/2 - 80)); // 垂直半径(更扁的椭圆)
                    
                    var setPos=function(node){
                        var i = node.index;
                        var deg = Math.PI/4 * i;  // 45度间隔
                        
                        // 椭圆参数方程
                        var dx = Math.round(lx * Math.cos(deg));
                        var dy = Math.round(ly * Math.sin(deg));
                        
                        // 关键修改：根据垂直位置计算z-index
                        // sin(deg)值越大(越靠下)的节点z-index越高
                        var zIndex = 100 + Math.round(50 * Math.sin(deg));
                        node.style.zIndex = zIndex;
                        
                        // 添加缩放效果：近大远小
                        var scale = 0.7 + 0.3 * (Math.sin(deg) + 1)/2;
                        
                        node.style.transform = `translate(${dx}px, ${dy}px) scale(${scale})`;
                        node.style.opacity = 0.9 + 0.1 * (Math.sin(deg) + 1)/2; // 透明度微调
                    }
                    
                    var characterz = ['guyong','litong','mazhong','fuwan','chengpu','liaohua','xinxianying','liuyu'];
                    
                    // 按从远到近顺序创建节点 (确保基础堆叠顺序正确)
                    for(var i = 7; i >= 0; i--){  // 反向创建：远的先创建
                        ...
                        node.style.width = '122px';
                        node.style.position = 'absolute';  // 确保定位生效
                        node.style.left = 'calc(50% - 60px)';
                        node.style.top = 'calc(50% - 60px)';
                        node.style.transition = 'transform 0.5s ease, opacity 0.5s ease'; // 平滑动画
                    ...
                    this.showcaseinterval = setInterval(function(){
                        ...
                        for(var i = 0; i < nodes.length; i++){
                            nodes[i].index = (nodes[i].index + 1) % 8;
                            setPos(nodes[i]);  // 更新位置和z-index
                        }
                    }, 1000);
				},
				init:function(){},
					...
					var pack={
						...
						game:{
							...
							randomMission:function(){
								...
								ui.huanhuazhizhan.style.zIndex=5;
							},
							...
							chooseCharacter:function(){
								...
								next.setContent(function(){
									...
									game.me.chooseButton(['请选择角色形象',[_status.characterlist.randomRemove(5),'character'],""],true).set('selectButton',function(){
					                    ui.update();
					                    return 1;
					                }).onfree=true;
									'step 1'
			...
			qunxionggeju:{
				...
				showcase:function(init){
					...
					var func=function(){
						...
						//card.node.name.innerHTML=card.node.name.innerHTML+'·'+card.node.name2.innerHTML;
						card.node.name.innerHTML=(get.characterSurname(name[0]).join(''))+'·'+(get.characterSurname(name[1]).join(''));
						card.node.name2.remove();
						node.nodes.push(card);
						card.style.position='absolute';
						card.style.zIndex=2;
						card.style.transition='all 2s';
						card.style.height='169px';
						card.style.width='122px';
				...
				next.setContent(function(){
					...
					if(_status.auto){
						...
						next.filterButton=function(button){
							...
							var unselect=function(button,select) {
							     if(ui.selected.buttons&&ui.selected.buttons.length&&ui.selected.buttons.contains(button)) {
							         var select=true;
							     }
							     if(lib.config.extension_无名补丁_xindchoose_old) {
							         var shade='';
							     }else {
							         var shade='drop-shadow(0px 0px 3px rgba(0, 0, 0, 1))';
							     }
							     button.style.transition='all 0.5s';
							     var cho=get.config('guozhan_unselect');
							     if(select) {
							         //button.style.opacity=1;
							         //button.style.filter='grayscale(0) brightness(1)'+shade;
							         if(cho=='bright') {
							             button.style.filter='brightness(1)'+shade;
							             button.style.opacity=1;
							         }else if(cho=='opacity') {
							             button.style.filter=''+shade;
							             button.style.opacity=1;
							         }else {
							             button.style.filter='grayscale(0) brightness(1)'+shade;
							             button.style.opacity=1;
							         ...
							         if(cho=='bright') {
							             button.style.filter='brightness(0.6)'+shade;
							             button.style.opacity=1;
							         }else if(cho=='opacity') {
							             button.style.filter=''+shade;
							             button.style.opacity=0.3;
							         }else {
							             button.style.filter='grayscale(1) brightness(0.7)'+shade;
							             button.style.opacity=1;
							         }
							     }
							};
							unselect(button,true);
							if(ui.selected.buttons&&ui.selected.buttons.length>=2) {
							    unselect(button);
							}
							if(lib.character[button.link][4].contains('hiddenSkill')) {
							    unselect(button);
							    return false;
							}
							return true;
			...
			daozhiyueying:{
				...
				showcase:function(init){
					...
					if(init){
						...
						player1.style.height='169px';
						player1.style.width='122px';
						player2.style.height='169px';
						player2.style.width='122px';
					...
					var func=function(){
						game.linexy([
							player1.getLeft()+player1.offsetWidth*2.7,
							player1.getTop()+player1.offsetHeight*1.3-30+20,
							player2.getLeft()+player2.offsetWidth*2.7,
							player2.getTop()+player2.offsetHeight*1.3-30+20,
						],node);
						...
						setTimeout(function(){
							...
							game.linexy([
							    player2.getLeft()+player1.offsetWidth*2.7,
							    player2.getTop()+player1.offsetHeight*1.3+30+20,
							    player1.getLeft()+player2.offsetWidth*2.7,
							    player1.getTop()+player2.offsetHeight*1.3+30+20,
							],node);
							var card=createCard(true);
			...
			weiwoduzun:{
				...
				showcase:function(init){
					...
					var player;
					if(init){
						player=ui.create.player(null,true).init('shen_lvbu');
						...
						game.createCss(`.avatar_boss_lvbu {
    				        background-image: url("${lib.assetURL}image/mode/boss/character/boss_lvbu2.jpg") !important;
    				    }`);
						player.node.avatar.classList.add('avatar_boss_lvbu');
						player.node.avatar.show();
						player.style.height='169px';
						player.style.width='122.5px';
			...
			tongjiangmoshi:{
				...
				showcase:function(init){
                    ...
                    var setPos = function(node) {
                        ...
                        // 关键改进：动态z-index确保正确堆叠
                        var zIndex = 50 + Math.round(100 * (Math.sin(deg) + 1)/2);
                        node.style.zIndex = zIndex;
                        
                        // 透视效果：近大远小
                        var scale = 0.7 + 0.3 * (Math.sin(deg) + 1)/2;
                        var opacity = 0.8 + 0.2 * (Math.sin(deg) + 1)/2;
                    ...
                    for(var i = 0; i < 8; i++){
                        ...
                        node.style.left = 'calc(50% - 60px)';
                        node.style.top = 'calc(50% - 60px)';
                        node.style.transition = 'transform 0.8s cubic-bezier(0.2, 0.8, 0.3, 1), opacity 0.8s ease';
                        node.style.willChange = 'transform, opacity'; // 优化性能
   			...
   			game.chooseCharacterTwo=function(){
   				...
   				next.setContent(function(){
   					...
   					next.set('filterButton',function(button){
   						...
   						if(event.filterChoice(button.link,ui.selected.buttons[0].link)) {
   						    button.style.opacity=1;
   						}else {
   						    button.style.opacity=0.3;
   						}
				...
				showcase:function(init){
					...
					var func=function(){
						...
						setTimeout(function(){
 						...
 						game.linexy([
							    player1.getLeft()+player1.offsetWidth*2.7,
							    player1.getTop()+player1.offsetHeight*1.5+20,
							    player2.getLeft()+player2.offsetWidth*2.7,
							    player2.getTop()+player2.offsetHeight*1.5+20,
							],node);
							setTimeout(function(){
   				...
   				game.fellow.node.identity.classList.remove('guessing');
   					...
   					game.fan.nextSeat=game.zhu;
							...
							if(_status.qianlidanji.completeNumber!=5){
 							...
 							event.list=list2;
 							var next = game.zhu.chooseControl()
 							next.set('choiceList',list)
 							next.set('prompt','请选择一项奖励'+'<br>（当前已通过'+_status.qianlidanji.completeNumber+'关）');
							}
							'step 2'
							if(_status.qianlidanji.completeNumber!=5){
								if(result.index==3){
      	...
      	var next=game.createEvent('chooseCharacter');
   				...
   				next.setContent(function(){
   					...
   					game.me.chooseButton(dialog,true).set('onfree',true).set('selectButton',function(){
					    ui.update();
					    return 1;
					});
   					...
   					game.zhu.chooseButton(['请选择对手的登场武将',[_status.characterlist.randomGets(3),'character'],""],true).set('selectButton',function(){
					    ui.update();
					    return 1;
					});
   					"step 3"
			...
			liangjunduilei:{
				...
				showcase:function(init){
					...
					if(init){
						...
						player3.style.height='169px';
						player3.style.width='122px';
						...
						player4.style.height='169px';
						player4.style.width='122px';
