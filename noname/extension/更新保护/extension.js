...
game.import("extension", function (lib, game, ui, get, ai, _status) {
	...
	if(lib.config.extension_更新保护_autoAddExts) game.getFileList('extension',function(folders,files){
    if(!lib||!lib.config) return;
    var exts=lib.config.extensions;
    allFoldersExt=[];
    var plays=['coin','boss','wuxing','cardpile'];
    var addExtStr='';
    for(var i=0;i<folders.length;i++) {
        var es=folders[i];
        if(exts.contains(es)||plays.contains(es)||es=='SJ Settings') {
            continue;
        }
        allFoldersExt.add(es);
        //exts.add(es);
    }
    var addNext=function(next){
        game.getFileList('extension/'+next,function(folders2,files2){
            //alert(next+':'+files2);
            if(files2.contains('extension.js')) {
                addExtStr+='、'+next;
                exts.add(next);
                if(lib.config.extensions) lib.config.extensions.add(next);
				game.saveConfig('extension_'+next+'_enable',true);
            }
            if(allFoldersExt.length) {
                addNext(allFoldersExt.shift());
            }else if(addExtStr!='') {
                game.saveConfig('extensions',exts);
                var message='已识别扩展：'+addExtStr.slice(1)+'。请重新加载游戏！';
                var tips=function() {
    			...
    			tips();
                var forbid=ui.create.div(ui.window);
				game.createCss(`.forbid_window{
				    width: 100%;
				    height: 100%;
				    left: 0;
				    top: 0;
				...
				forbid.classList.add('forbid_window');
            }
        });
    }
    if(allFoldersExt.length) addNext(allFoldersExt.shift());
    //game.saveConfig('extensions',exts);
    });
	/*if (!game.getExtensionConfig('更新保护', 'incompatibleExtension') && (game.getExtensionConfig('概念武将', 'enable') || game.getExtensionConfig('假装无敌', 'enable'))) {
		alert('「更新保护」扩展提示您：\r\n安装【概念武将】和【假装无敌】扩展后，本扩展出现任何bug后果自负');
		game.saveExtensionConfig('更新保护', 'incompatibleExtension', true);
	}*/
	 ...
	 */
	function myFetch(url, options = { timeout: typeof game.getExtensionConfig('更新保护', 'timeout') == 'number' ? game.getExtensionConfig('更新保护', 'timeout') : 3000 }) {
	...
	const assetConfigFun = function (configName) {
		return function (bool) {
			game.saveExtensionConfig('更新保护', configName, bool);
		}
	};
	...
	return {
		...
		content: function (config, pack) {
			...
			const interval = setInterval(() => {
				...
				// @ts-ignore
				const insertWeb = ul.insertBefore(document.createElement('div'), ul.firstElementChild);
				//insertLi.innerHTML = "<span class='bluetext'>更新功能已由「更新保护」扩展覆盖</span></br>";
				insertWeb.innerHTML = '<span style="opacity:0.7;font-size:15px">Webview版本 - '+getChromeVersion()+'</span></p>';
				const insertVS = ul.insertBefore(document.createElement('li'), ul.firstElementChild);
				//insertLi.innerHTML = "<span class='bluetext'>更新功能已由「更新保护」扩展覆盖</span></br>";
				insertVS.innerHTML = '整合版本：Ver '+window.liulikillVersion+'<p style="margin-top:8px;white-space:nowrap">';
				const insertNo = ul.insertBefore(document.createElement('div'), ul.firstElementChild);
				//insertLi.innerHTML = "<span class='bluetext'>更新功能已由「更新保护」扩展覆盖</span></br>";
				insertNo.innerHTML = '<span style="opacity:0.7;font-size:15px">请勿替换或更新游戏本体！</span></p>';
				const insertLi = ul.insertBefore(document.createElement('li'), ul.firstElementChild);
				//insertLi.innerHTML = "<span class='bluetext'>更新功能已由「更新保护」扩展覆盖</span></br>";
				insertLi.innerHTML = '三国杀·琉璃版';
				// a标签跳转到扩展界面
				const jumpToExt = document.createElement('a');
				jumpToExt.href = "javascript:void(0)";
				jumpToExt.onclick = () => ui.click.extensionTab('更新保护');
				...
				clearInterval(interval);
				/*console.log("「更新保护」扩展已修改更新界面");*/
			}, 500);

			// 重启前下载失败的文件名(去除重复的)
			if (!Array.isArray(lib.config.extension_更新保护_brokenFile)) {
				game.saveExtensionConfig('更新保护', 'brokenFile', []);
			} else {
				game.saveExtensionConfig('更新保护', 'brokenFile', Array.from(new Set([...lib.config.extension_更新保护_brokenFile])));
			}

			// 如果有没下载完就重启的文件
			let brokenFileArr = lib.config.extension_更新保护_brokenFile;
			window.saveBrokenFile = () => {
				game.saveExtensionConfig('更新保护', 'brokenFile', [...new Set(brokenFileArr)]);
			};
			...
			if (brokenFileArr && brokenFileArr.length) {
				...
				setTimeout(() => {
					if (confirm(`检测到有未下载成功的文件(${brokenFileArr})，是否进行重新下载?`)) {
						...
						// 下载之前保留下载列表
						brokenFileArr.forEach(v => lib.config.extension_更新保护_brokenFile.add(v));
						game.saveConfigValue('extension_更新保护_brokenFile');
			...
			function checkUpdate() {
				game.shijianGetUpdateFiles().then(({ update }) => {
					...
					// 获取到更新，进行提示
					let str = '有新版本' + update.version + '可用，是否前往「更新保护」扩展界面下载？';
			...
			setInterval(() => {
				if (game.getExtensionConfig('更新保护', 'auto_check_update')) checkUpdate();
			}, 1000 * 60 * 10);
			// 刚开启时候的网络请求不会成功
			if (game.getExtensionConfig('更新保护', 'auto_check_update')) setTimeout(checkUpdate, 10000);
		},
		precontent: function () {
		    delete lib.extensionMenu.extension_更新保护.delete;
            delete lib.extensionMenu.extension_更新保护.enable;
            var tratype=lib.config.ext_translation_type||'new';
		    var tralist=window.extTraList[tratype];
            //禁止乱删基础扩展
            setTimeout(function(){
            if(tralist) for(var ext in tralist) {
                if(!lib.extensionMenu['extension_'+ext]) continue;
                if(lib.extensionMenu['extension_'+ext].delete) {
                    delete lib.extensionMenu['extension_'+ext].delete;
                    //alert(ext);
                }
                if(lib.extensionMenu['extension_'+ext].edit) {
                    delete lib.extensionMenu['extension_'+ext].edit;
                    //alert(ext);
                }
                if(lib.config.extension_更新保护_cancelForbid&&lib.extensionMenu['extension_'+ext].enable) {
                    //delete lib.extensionMenu['extension_'+ext].enable;
                    //lib.config['extension_'+ext+'_enable']=true;
                    lib.extensionMenu['extension_'+ext].enable.lockEnable=true;
                    //game.jj=lib.extensionMenu['extension_'+ext].enable;
                    //alert(ext);
                }
            }
            },2000);
            if(lib.config.extension_更新保护_cancelForbid) {
                for(var ext in tralist) {
                    game.saveConfig('extension_'+ext+'_enable',true);
                }
                //十秒禁掉开关
                /*setTimeout(function(){
                    for(var exts in tralist) {
                        if(!lib.extensionMenu['extension_'+exts]) continue;
                        if(lib.extensionMenu['extension_'+exts].enable) {
                            delete lib.extensionMenu['extension_'+exts].enable;
                        }
                    }
                },10000);*/
            }
			...
			// 初始化，更新地址修改为URC
			if (!game.getExtensionConfig('更新保护', 'update_link')) {
				game.saveConfig('update_link', 'URC');
				game.saveExtensionConfig('更新保护', 'update_link', 'URC');
				lib.updateURL = lib.updateURLS['URC'];
			} else {
				game.saveConfig('update_link', game.getExtensionConfig('更新保护', 'update_link'));
			}
				...
				lib.configMenu.general.config.update_link = {
					...
					onclick: function (item) {
						if (lib.updateURLS[item]) {
							...
							game.saveExtensionConfig('更新保护', 'update_link', item);
			...
			game.shijianDownload = (url, onsuccess, onerror, onprogress) => {
				...
				lib.config.extension_更新保护_brokenFile.add(downloadUrl);
				game.saveConfigValue('extension_更新保护_brokenFile');
				...
				function success(FileEntry, skipDownload) {
					if (FileEntry && !skipDownload && ['config', 'xuanwu'].includes(lib.config.update_link)) {
						...
						lib.config.extension_更新保护_brokenFile.remove(downloadUrl);
						game.saveConfigValue('extension_更新保护_brokenFile');
						...
						.then(arrayBuffer => {
							...
							if (!fs.existsSync(dirPath)) {
								...
								if (stat.isDirectory()) {
									...
									lib.config.extension_更新保护_brokenFile.remove(path + '/' + name);
									game.saveConfigValue('extension_更新保护_brokenFile');
			...
			game.shijianMultiDownload = (list, onsuccess, onerror, onfinish, onprogress) => {
				...
				let reload = (current) => {
					...
					game.shijianDownload(current, skipDownload => {
						...
						if (!game.getExtensionConfig('更新保护', 'logProgress')) return;
						if (typeof onprogress == 'function') {
			...
			(window.Reflect || Object).defineProperty(game, 'checkForUpdate', {
				...
				get() {
					return function () {
						if (!sessionStorage.getItem('更新保护_checkForUpdate')) {
							//alert('无名杀自带的自动检查更新已禁用，请使用更新保护扩展内的自动检查更新功能');
							sessionStorage.setItem('更新保护_checkForUpdate', '1');
						}
			...
			lib.init.css(lib.assetURL + 'extension/更新保护', 'extension');
		},
		config: {
			show_version: {
				clear: true,
				nopointer: true,
				name: '扩展版本：三国杀·琉璃版专属版本',
			},
			show_qqqun: {
				clear: true,
				nopointer: true,
				name: '懒人包群：623260187〈琉璃交流群〉',
			},
			update_link_explain: {
				clear: true,
				nopointer: true,
			...
			update_link: {
				...
				onclick: function (item) {
					if (item != game.getExtensionConfig('更新保护', 'update_link')) {
			...
			/*timeout: {
				...
				onblur: function (e) {
					...
					game.saveExtensionConfig('更新保护', 'timeout', time);
			...
			/*logProgress: {
				...
				onclick: function (bool) {
					game.saveExtensionConfig('更新保护', 'logProgress', bool);
				}
			},*/
			/*auto_check_update: {
				init: (() => {
				...
				name: '每10分钟自动检测更新',
				onclick: function (bool) {
					game.saveExtensionConfig('更新保护', 'auto_check_update', bool);
					game.saveConfig('auto_check_update', false);
				}
			},*/
			checkForUpdate: {
					...
					/*if (button.disabled) return;
						...
						game.shijianGetUpdateFiles().then(({ update, source_list: updates }) => {
							...
							let goupdate = (files, update) => {
								...
								if (!game.getExtensionConfig('更新保护', 'updateAll') && Array.isArray(files)) {
								...
								// 下载之前保留下载列表
								updates.forEach(v => lib.config.extension_更新保护_brokenFile.add(v));
								game.saveConfigValue('extension_更新保护_brokenFile');
			...
			/*updateAll: {
				...
				name: '强制更新所有主文件',
				/*onclick: (bool) => {
					game.saveExtensionConfig('更新保护', 'updateAll', bool);
				}*/
				...
				onclick: async function () {
					...
					/*if (button.disabled) {
						...
						game.shijianGetUpdateAssets().then(({ assets: updates, skins }) => {
							...
							//如果没选择【检查图片素材】（全部）
							if (!game.getExtensionConfig('更新保护', 'assetImageFull')) {
							...
							for (let i = 0; i < updates.length; i++) {
								switch (updates[i].slice(0, 5)) {
									case 'image':
										...
										if (!game.getExtensionConfig('更新保护', 'assetImageFull')) {
											//如果没选择检查图片更新（部分）
											if (!game.getExtensionConfig('更新保护', 'assetImage')) {
												//跳过
												updates.splice(i--, 1);
											} else {
												//更新部分素材
										...
										break;
									case 'audio':
										if (!game.getExtensionConfig('更新保护', 'assetAudio')) {
											updates.splice(i--, 1);
										}
										break;

									case 'font/':
										if (!game.getExtensionConfig('更新保护', 'assetFont')) {
											updates.splice(i--, 1);
										}
							...
							// 如果更新皮肤
							if (game.getExtensionConfig('更新保护', 'assetSkin') && typeof skins == 'object') {
								for (let i in skins) {
			...
			cancelForbid: {
				init: true,
				intro: '锁定基础扩展并保持开启，此为懒人包保护开关，进阶玩法或开发者可选择关闭。',
				name: '锁定基础扩展',
			},
			autoAddExts: {
				init: true,
				intro: '开启后将文件解压到扩展目录，即可自动识别成扩展，无需万能导入法（文件夹名称需设置正确）。',
				name: '自动识别扩展',
			},
			/*assetFont: {
				init: true,
