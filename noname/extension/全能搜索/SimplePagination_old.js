...
class SimplePagination {
    ...
    hiddenEllipse(selector) {
        ...
        if(!this.selectorEle(selector)) return;
    ...
    setTotalPageCount(totalPageCount) {
        ...
        // 当需要省略符号占位时，确定 active的位置
        this.state.totalPageCount > this.state.maxShowBtnCount + 2 && (this.state.activePosition = Math.ceil(this.state.maxShowBtnCount / 2))
        this.renderPageDOM();
    }
}