	...
	return {
		...
		content: function(config, pack) {
            ...
            class Searcher {
                ...
                constructor(target) {
                    ...
                    if (game.getExtensionConfig('全能搜索', 'backgroundImage')&&game.getExtensionConfig('全能搜索', 'backgroundImage') !== 'none') {
                        if (game.getExtensionConfig('全能搜索', 'backgroundImage') == '随机') {
                            ui.background.setBackgroundImage("extension/全能搜索/img/" + ['相爱相杀', '才颜双绝'].randomGet() + ".png");
                        } else {
                            ui.background.setBackgroundImage("extension/全能搜索/img/" + game.getExtensionConfig('全能搜索', 'backgroundImage'));
                        }
                    }else if(lib.config.image_background){
                     ...
                     * @type {string[]}
                     *  */
                    //this.searchList = game.getExtensionConfig('全能搜索', 'searchList') || [];关闭历史搜索
					this.searchList = [];
					/**
                    ...
                    //this.clear = ui.create.div('.clear', this.menu, '清除内容', this.clearDataList.bind(this));
                    ...
                    for (const arr of [['findCharacter', '武将'], ['findCard', '卡牌'], ['findSkill', '技能'], ['findCode', '代码']]) {
                        const key = arr[0];
                        const value = arr[1];
                    ...
                    this.close.addEventListener('click', () => {
                        ...
                        //手杀配件恢复：不必要，resume2里面已经恢复了
                        /*if(window.shoushaBlanks&&window.shoushaBlanks.length) {
                    ...
                    for (const data of name) {
                        ...
                        this.fragmentDataList[this.fragmentList.length - 1].push((lastFragment) => {
                            ...
                            d.insertAdjacentHTML('afterbegin', `
                                ...
                                <span class="bluetext" ondblclick="game.全能搜索_copy(this.nextSibling)">武将信息&nbsp</span>
                                <span>${get.characterIntro(charName)}</span>
                                </br>

                                <span class="bluetext" ondblclick="game.全能搜索_copy(this.nextElementSibling.nextElementSibling.this.nextElementSibling.nextElementSibling)">所在武将包&nbsp</span>
                                ...
                                <span class="bluetext" ondblclick="game.全能搜索_copy(this.nextSibling)">武将称号&nbsp</span>
                    ...
                    for (const data of name) {
                        ...
                        */
                        ...
                        this.fragmentDataList[this.fragmentList.length - 1].push((lastFragment) => {
                            ...
                            if(cardData) d.insertAdjacentHTML('afterbegin', `
                                </br>
                                <span class="bluetext" ondblclick="game.全能搜索_copy(this.nextElementSibling.nextElementSibling)">卡牌名称</span>
                                ...
                                <span class="bluetext" ondblclick="game.全能搜索_copy(this.nextElementSibling.nextElementSibling)">卡牌类别</span>
                        ...
                        this.fragmentDataList[this.fragmentList.length - 1].push((lastFragment) => {
                            ...
                            d.insertAdjacentHTML('afterbegin', `
                                <li>
                                    ...
                                    <span class="bluetext" ondblclick="game.全能搜索_copy(this.nextSibling)">技能描述</span>：${typeof info.replace=='function' ? (info.replace(/<\/li>/g, '').split('<li>').map((v, i) => i == 0 ? v : ('•' + v)).join('<br/>')) : '无'}</br>
                ...
                tujianBegin(dialog, result, canAddToDataList) {
                    ...
                    this.fragmentList = [];
                    this.fragmentDataList = [];
                    if (result === "" || result === null || result === undefined) return qhly_prompt('alert',"请输入名称");
                    game.saveExtensionConfig('全能搜索', 'searchList', this.searchList);
					...
					if ([resultCharacter, resultCard, resultSKill, resultCode].every(v => v == false)) {
						...
						qhly_prompt('alert',`没有符合条件的武将，卡牌，技能或代码!(搜索内容："${result}")`);
						this.clearDialog(dialog);
						return;
					}
                    ...
                    if (this.fragmentList.length) {
                        if(typeof this.updatePage!='function') {
                            const Tslp = this.slp || new SimplePagination(1);
                            this.slp = Tslp;
                            Tslp.init({
                                container: '.pagination',
                                onPageChange: state => {
                            ...
                            this.updatePage = page => {
                                ...
                                this.dialog.content.appendChild(fragment.cloneNode(true));
                                // @ts-ignore
                                if (Tslp.state.totalPageCount != this.fragmentList.length) {
                                    Tslp.setTotalPageCount(this.fragmentList.length);
                                }
                            }
                        };
                        this.updatePage(1);
            ...
            const getSystem = setInterval(() => {
                if (ui.system1 || ui.system2) {
                    ...
                    ui.Searcher = ui.create.system('全能搜索', function () {
                        ...
                        window.shoushaBlanksNotHide=true;
				        setTimeout(function(){
				            window.shoushaBlanksNotShow=false;
				            window.shoushaBlanksNotHide=false;
			        	},200);
                        //叉掉进度条
		...
		config: {
            version: {
                nopointer: true,
                clear: true,
                name: "扩展版本: v3.2<br>更新日期: 2023-04-7",
            },
            css: {
                clear: true,
                'name': '更换代码高亮样式',
            ...
            background: {
                ...
                onclick(item) {
                    var item2=item;
                    if(['随机','none'].indexOf(item)==-1) item2=item2+'.png';
                    game.saveExtensionConfig('全能搜索', 'backgroundImage', item2);
                }
            },
			"loadUpdateContent": {
				clear: true,
