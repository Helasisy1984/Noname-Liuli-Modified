game.import("extension", function(lib, game, ui, get, ai, _status) {
    ...
    */
    ...
    function createProgress(str) {
        ...
        div.id = 'importZip';
        div.className = 'drag-read-importZip';
    ...
    // 创建扩展名称输入模态框
    function createExtensionNameModal(defaultName, callback) {
        const modalBackground = document.createElement('div');
        modalBackground.className = 'drag-read-overlay';

        const modal = document.createElement('div');
        modal.className = 'drag-read-extension-modal';
        
        const title = document.createElement('div');
        title.className = 'drag-read-modal-title';
        title.textContent = '请输入扩展名称';
        
        const input = document.createElement('input');
        input.type = 'text';
        input.className = 'drag-read-modal-input';
        input.value = defaultName;
        
        const buttonContainer = document.createElement('div');
        buttonContainer.className = 'drag-read-modal-buttons';
        
        const confirmBtn = document.createElement('button');
        confirmBtn.className = 'drag-read-modal-btn confirm';
        confirmBtn.textContent = '确认';
        
        const cancelBtn = document.createElement('button');
        cancelBtn.className = 'drag-read-modal-btn cancel';
    ...
    return {
        ...
        precontent: function() {
            ...
            body.addEventListener('drop', async function (e) {
                    ...
                    if (index == 1) { //扩展
                        fileReader.onload = function (fileLoadedEvent) {
                            ...
                            zip3.loadAsync(data).then(zip3 => {
                                ...
                                // 使用压缩包名称（不带后缀）作为默认扩展名
                                let defaultName = name.replace(/\.(zip|7z)$/i, '');
                                
                                // 弹出输入框让用户输入扩展名
                                createExtensionNameModal(defaultName, (extname) => {
                                    if (!extname) {
                                        div.remove();
                                        return;
                                    }
                ...
                (name.endsWith('.zip') ? nextDo : nextDo_7z)(1);
            });
                            ...
                            if (isNaN(nowVersion) || nowVersion < newVersion) {
                                ...
                                fetch('https://raw.fastgit.org/nonameShijian/extinsion-DragRead/main/updateContent')
                                    .then(response => response.text()).then(txt => {
                                        ...
                                        updateContent += "\n</pre>";
                                        let layer = ui.create.div(ui.window, '.drag-read-updateContent');
                                        let close = ui.create.div(layer, '.drag-read-updateContentClose', () => {
                                            layer.remove();
                                        ...
                                        button.type = 'button';
                                        button.value = '点击下载';
                                        button.className = 'drag-read-updateContentButton menubutton large';
                                    }).catch(err => {
        ...
        help: {},
        package: {
            intro: `Windows电脑版专属扩展，把扩展包（zip或7z文件）拖入到游戏内即可导入`,
            author: "诗笺",
            diskURL: "",
            forumURL: "",
            version: "1.81",
