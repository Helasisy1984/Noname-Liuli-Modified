game.import('extension', function (lib, game, ui, get, ai, _status) { return {
	...
	content: function (config, pack) {
		...
		lib.skill._aiyh_fixEQz = {//武将权重
			enable: 'phaseUse',
			filter: function (event, player) {
				return player == game.me && lib.config.extension_AI优化_fixEQz;
			},
		...
		lib.skill._aiyh_fixECf = {//技能威胁度
			enable: 'phaseUse',
			filter: function (event, player) {
				return player == game.me && lib.config.extension_AI优化_fixECf;
			},
			filterTarget: true,
		...
		lib.translate._aiyh_fixEQz = '<font color=#FFFF00>修改权重</font>';
		lib.translate._aiyh_fixECf = '<font color=#FF3300>修改威胁度</font>';
		lib.translate._aiyh_fixWj = '<font color=#00FFFF>伪禁</font>';*/
		
		lib.translate._aiyh_neiKey = '昭告';
		lib.translate._aiyh_fixEQz = '权重';
		lib.translate._aiyh_fixECf = '威胁';
		lib.translate._aiyh_fixWj = '伪禁';
		...
		/*AI优化*/
		if(lib.config.extension_AI优化_btAi == 'shyg'){//本体AI优化
				...
				ai: {
					effect: {
						player: function (card, player, target, current) {
							...
							if (player.isUnderControl(true) && get.tag(card, 'damage') && game.hasNature(card) && get.attitude(target, player) > 0) {
								if (target.isLinked() && hzdr > 0) {
									return 'zeroplayertarget';
								}
							}
							if (get.tag(card, 'damage') && game.hasNature(card,'fire')) {
								if (target.isLinked()) {
							...
							if (get.tag(card, 'damage') && game.hasNature(card,'thunder')) {
								if (target.isLinked()) {
							...
							if (get.name(card) == 'sha') {//杀对策
								if (!player.hasSkill('xinpojun') && !player.hasSkill('repojun') && !player.hasSkill('decadepojun') && !player.hasSkillTag('unequip_ai', false, {
									...
									if ((target.hasSkill('tengjia3') || target.hasSkill('rw_tengjia4')) && !game.hasNature(card)) {
									...
									if (target.hasSkill('tengjia2') && game.hasNature(card) && get.attitude(player, target) < 0) return 1;
								...
								if (target.hasSkillTag('useShan')) {
									if (get.attitude(player, target) > 0) {
										...
										if (game.hasNature(card) && target.hasSkill('tengjia2')) return -1;
										if (!game.hasNature(card) && target.hasSkill('tengjia2')) return 'zeroplayertarget';
						...
						target: function (card, player, target, current) {
							/*免伤对策*/
							if (get.tag(card, 'damage') && !game.hasNature(card)) {
									...
									if (target.hasSkillTag('maixie')) {
										...
										if (!target.hasSkillTag('maixie_defend')) {
											if (get.attitude(player, target) > 0) {
												...
												if (get.name(card) == 'sha' && game.hasNature(card) && target.hasSkill('tengjia2')) return [0, -2.5];
											}
			...
			lib.skill._aiyh_reserved_shan = {
				...
				filter: function (event, player) {
					...
					if (
						...
						(player.getEquip('bagua') || game.hasNature(event.card,'fire') && player.hasSkill('tengjia2'))&&!player.hasSkillTag('unequip2')&&!event.player.hasSkillTag('unequip',false,{
			...
			if(lib.card.tao) lib.card.tao.ai={
				...
				result: {
					...
					target_use: function (player, target, card) {
						...
						if (player.hasSkillTag('nokeep', true, {card:card,target:target}, true)) return 2;
			...
			if(lib.card.nanman) lib.card.nanman.ai={
				...
				result: {
					player: function (player, target) {
						...
						let eff = get.effect(target, {name:'nanman'}, player, target);
			...
			if(lib.card.wanjian) lib.card.wanjian.ai={
				...
				result: {
					player: function (player, target) {
						...
						let eff = get.effect(target, {name:'wanjian'}, player, target);
			...
			if(lib.card.taoyuan) lib.card.taoyuan.ai={
				basic: {
					order: function (item, player) {
			...
			if(lib.card.huogong) lib.card.huogong.ai={
				basic: {
					order: 9.2,
					value: [3, 1],
					useful: 0.6
				...
				wuxie: function (target, card, player, viewer, status) {
					if (status * get.attitude(viewer, target) < 0 || status*get.attitude(viewer,player)>0&&!player.isMad() || Math.random() * 4 > player.countCards('h')) return 0;
				},
				result: {
					player: function (player, target) {
			...
			if(lib.card.qizhengxiangsheng){
				lib.card.qizhengxiangsheng.content=function () {
					'step 0'
		...
		if(lib.config.extension_AI优化_zbAi){//装备AI优化
			...
			if(lib.card.guanshi) lib.card.guanshi.ai={
				equipValue: function (card, player) {
					...
					for (var j = 0; j < game.players.length; j++) {
						if (game.players[j] == player || get.distance(player, game.players[j]) > 3 || get.attitude(player, game.players[j]) >= 0 || !player.canUse('sha', game.players[j]) || !game.players[j].mayHaveShan(player,'use')) continue;
				...
				lib.skill.zhangba_skill.ai={
					...
					yingbian: function (card, player, targets, viewer) {
						...
						if (get.cardtag(card, 'yingbian_damage')) {
							if (targets.filter(function (target) {
								return get.attitude(player, target) < 0 && (hit || !target.mayHaveShan(player,'use') || player.hasSkillTag(
									'directHit_ai',
									true,
									{
										target: target,
					...
					result: {
						target: function (player, target, card, isLink) {
							...
							if (!isLink && target.mayHaveShan(player,'use') && !player.hasSkillTag(
								'directHit_ai',
								true,
								{
									target: target,
			...
			if(lib.skill.pyzhuren_heart) lib.skill.pyzhuren_heart.check=true;
		}
	},
	precontent: function () {
		...
		/*if(lib.config.extension_AI优化_changelog!==lib.extensionPack.AI优化.version) lib.game.showChangeLog=function(){//更新内容
			let str=[
				'<center><font color=#00FFFF>更新日期</font>：<font color=#FFFF00>23</font>年<font color=#00FFB0>10</font>月<font color=fire>20</font>日</center>',
				'◆删除［坐骑修改］功能',
				'◆删除〖骁果〗错误优化',
				'◆删除本体已存在的万年公主〖枕戈〗ai',
				'◆修复开启［威胁度补充］后ai不发动〖乱击〗〖仁德〗等有继承旧技能的印牌技的严重bug',
				'◆此版本开始不再自动迁移数据'
			];
		...
		lib.arenaReady.push(function () {//杂项
			...
			if (lib.config.extension_AI优化_xyyh) {/*小游戏*/
				if(lib.skill.chongxu&&game.aiyh_skillOptEnabled('chongxu','跳过〖冲虚〗小游戏')){
					...
					if(!lib.skill.chongxu.ai) lib.skill.chongxu.ai={};
					lib.skill.chongxu.ai.isLuckyStar=true;
				}
				if(lib.skill.xinfu_pingcai&&game.aiyh_skillOptEnabled('xinfu_pingcai','跳过〖评才〗小游戏')){
					...
					lib.skill.xinfu_pingcai.ai.isLuckyStar=true;
				}
				...
				if(lib.skill.zhengjing&&game.aiyh_skillOptEnabled('zhengjing','跳过〖整经〗小游戏')){
					...
					if(!lib.skill.zhengjing.ai) lib.skill.zhengjing.ai={};
					lib.skill.zhengjing.ai.isLuckyStar=true;
				};
				if(lib.skill.qiaosi&&game.aiyh_skillOptEnabled('qiaosi','跳过〖巧思〗小游戏')){
					...
					if(!lib.skill.qiaosi.ai) lib.skill.qiaosi.ai={};
					lib.skill.qiaosi.ai.isLuckyStar=true;
				}
			}
		});
		if (lib.config.extension_AI优化_wjAi) lib.arenaReady.push(function(){//武将AI
			...
			if(lib.skill.refangzhu&&game.aiyh_skillOptEnabled('refangzhu')){
				lib.skill.refangzhu.content=function () {
					'step 0'
			...
			if(lib.skill.xinfu_jianjie1&&game.aiyh_skillOptEnabled('xinfu_jianjie')) lib.skill.xinfu_jianjie1.ai={
				order: 8,
				result: {
					target: function (player, target) {
						var att = get.attitude(player, target);
			...
			if(lib.skill.yechou&&game.aiyh_skillOptEnabled('yechou')) lib.skill.yechou.ai={
				maixie_defend:true,
				effect:{
					target:function(card,player,target){
						if(target.hp>1||!get.tag(card,'damage')) return 1;
						let targets=game.filterPlayer(current=>current.getDamagedHp()>1&&get.attitude(player,current)>0),num=0;
						if(!targets.length) return 1.7;
						return [1, 0, 1, -5];
					}
				}
			};
			if(lib.skill.twwuhun&&lib.skill.twwuhun.subSkill&&lib.skill.twwuhun.subSkill.gain&&game.aiyh_skillOptEnabled('twwuhun')) lib.skill.twwuhun.subSkill.gain.ai={
				maixie_defend: true,
				effect: {
					target: function (card, player, target) {
			...
			if(lib.skill.juesheng&&game.aiyh_skillOptEnabled('juesheng')) lib.skill.juesheng.ai={
				result: {
					player: function (player, target) {
			...
			if(lib.skill.zhongzuo&&game.aiyh_skillOptEnabled('zhongzuo')) lib.skill.zhongzuo.content=function () {
				'step 0'
			...
			if(lib.skill.dawu&&game.aiyh_skillOptEnabled('dawu')) lib.skill.dawu.content=function () {
				'step 0'
			...
			if(lib.skill.xinkuangfu&&lib.skill.xinkuangfu.ai&&game.aiyh_skillOptEnabled('xinkuangfu')) lib.skill.xinkuangfu.ai.result={
				target: function (player, target) {
					var att = get.attitude(player, target);
					var max = 0;
					var min = 1;
			...
			if(lib.skill.tianzuo&&game.aiyh_skillOptEnabled('tianzuo')) lib.skill.tianzuo.ai={
				effect: {
					target: function (card, player, target) {
			...
			if(lib.skill.shouli&&game.aiyh_skillOptEnabled('shouli')) lib.skill.shouli.ai={
				...
				order: function (item, player) {
					for (var i = 0; i < game.players.length; i++) {
						...
						if (get.translation(_status.event != '决斗') && _status.currentPhase == player && !player.hasCard('wuzhong') && !player.hasCard(function (card) {
				...
				result: {
					player: function (player, target) {
						...
						if (_status.event.type == 'phase') {
							if (player.hasValueTarget({name: 'sha'}) && game.hasPlayer(function (current) {
			...
			if(lib.skill.hengwu&&game.aiyh_skillOptEnabled('hengwu')){
				lib.skill.hengwu.mod={
					aiUseful: function (player, card, num) {
			...
			if(lib.skill.olmiuyan&&game.aiyh_skillOptEnabled('olmiuyan')) lib.skill.olmiuyan.ai={
				order: function (item, player) {
			...
			if(lib.skill.chengxiang&&game.aiyh_skillOptEnabled('chengxiang')){
			...
			if(lib.skill.oljianhe&&game.aiyh_skillOptEnabled('oljianhe')) lib.skill.oljianhe.check=function (card) {
				var player = _status.event.player;
			...
			if(lib.skill.lingce&&game.aiyh_skillOptEnabled('lingce')) lib.skill.lingce.ai = {
				...
				effect: {
					target: function (card, player, target) {
						...
						if (get.attitude(player, target) < 0) {
							if (numf > 0 || nume > 0) {
								if (!target.getStorage('dinghan').includes(card.name)) {
			...
			//nsp
			if(lib.skill.clanfuxun&&game.aiyh_skillOptEnabled('clanfuxun')){
				lib.skill.clanfuxun.check=function(card){
					let player=get.owner(card);
					if(lib.skill.clanfuxun.selectAi(_status.event,player,'bool')) return 15-get.value(card);
					return 0;
				};
				lib.skill.clanfuxun.selectAi=function(event,player,type){
					let cache=event.getTempCache('clanfuxun',type);
					if(typeof cache!=='undefined') return cache;
					let players=game.filterPlayer(current=>current!==player),targets={
						'player':{},
						'target':{}
					},temp,bool=true,evtx=event.getParent('phaseUse');
					for(let tar of players){
						let att=get.attitude(event.player,tar);
						targets.player[tar]=0;
						targets.target[tar]=0;
						temp=tar.countCards('h')-player.countCards('h');
						if(tar.countGainableCards(player,'h')>0){
							bool=false;
							if(att>0) targets.player[tar]+=0.01*tar.countCards('h');
							else targets.target[tar]-=0.6/tar.countCards('h');
						}
						if(Math.abs(temp)!==2||tar.hasHistory('lose',evt=>{
							return evt.getParent(3).name!='clanfuxun'&&evt.getParent('phaseUse')==evtx&&evt.cards2.length;
						})||tar.hasHistory('gain',evt=>{
							return evt.getParent().name!='clanfuxun'&&evt.getParent('phaseUse')==evtx&&evt.cards.length;
						})) continue;
						if(temp>0&&tar.countGainableCards(player,'h')>0){
							bool=false;
							targets.player[tar]+=0.6;
						}
						else if(temp<0&&player.countGainableCards(tar,'h')&&(att>0||player.countCards('h',card=>{
							return get.value(card)<9.3-1.8*Math.min(3.5,player.hp);
						})>1)) targets.player[tar]+=0.6;
					}
					event.putTempCache('clanfuxun','bool',bool);
					event.putTempCache('clanfuxun','targets',targets);
					return event.getTempCache('clanfuxun',type);
				};
				if(lib.skill.clanfuxun.ai) lib.skill.clanfuxun.ai={
					order:function(item,player){
						let evtx=_status.event.getParent('phaseUse');
						if(game.hasPlayer(tar=>{
							if(evtx&&get.attitude(player,tar)<0&&tar.countCards('h')-player.countCards('h')===2&&tar.countGainableCards(player,'h')) return !tar.hasHistory('lose',evt=>{
								return evt.getParent(3).name!='clanfuxun'&&evt.getParent('phaseUse')==evtx&&evt.cards2.length;
							})&&!tar.hasHistory('gain',evt=>{
								return evt.getParent().name!='clanfuxun'&&evt.getParent('phaseUse')==evtx&&evt.cards.length;
							});
					...
					result:{
						player:function(player,target){
							let num=lib.skill.clanfuxun.selectAi(_status.event,player,'targets').player[target];
							if(ui.selected.cards.length) num--;
							return num;
						},
						target:function(player,target){
							let num=lib.skill.clanfuxun.selectAi(_status.event,player,'targets').target[target];
							if(!ui.selected.cards.length) num--;
							return num;
						}
					}
			...
			if(lib.skill.changbiao) {
				...
				if(game.aiyh_skillOptEnabled('changbiao','优化〖长标〗吃桃ai','changbiao_ai')){
					...
					lib.skill.changbiao.ai.nokeep=true;
			...
			if(lib.skill.dczhizhe&&game.aiyh_skillOptEnabled('dczhizhe','优化〖智哲〗牌存牌ai','dczhizhe_mod')) lib.skill.dczhizhe.mod={
				aiOrder:function(player,card,num){
			...
			if(lib.skill.mansi_viewas&&game.aiyh_skillOptEnabled('mansi','优化〖蛮嗣〗吃桃ai','mansi_viewas')){
				...
				lib.skill.mansi_viewas.ai.nokeep=true;
			...
			if(lib.skill.dcwumei&&lib.skill.dcwumei.subSkill&&lib.skill.dcwumei.subSkill.wake&&game.aiyh_skillOptEnabled('dcwumei','优化〖寤寐〗额外回合ai','dcwumei_wake')){
				lib.skill.dcwumei.subSkill.wake.init=function(player){
					game.addGlobalSkill('dcwumei_all');
				};
				lib.skill.dcwumei.subSkill.wake.onremove=function(player){
					game.removeGlobalSkill('dcwumei_all');
				};
				lib.skill.dcwumei_all={
					trigger:{player:'dieAfter'},
					filter:function(event,player){
			...
			if(lib.skill.dcctjiuxian&&game.aiyh_skillOptEnabled('dcctjiuxian')) lib.skill.dcctjiuxian.ai={
				order:function(){
					return get.order({name:'juedou'})*0.9;
				},
				tag:{
					respond:2,
					respondSha:2,
			...
			if(lib.skill.qingtan&&game.aiyh_skillOptEnabled('qingtan')) lib.skill.qingtan.content=function(){
				...
				if(result.control!='cancel2'){
					...
					for(let i=0;i<cards.length;i++){
						if(event.cards2.includes(cards[i])) targets[i].$give(cards[i],player,false);
					}
				...
				let draws=[];
				for(let i=0;i<cards.length;i++){
					if(!event.cards2.includes(cards[i])) targets[i].discard(cards[i]).delay=false;
			...
			if(lib.skill.dczhubi&&game.aiyh_skillOptEnabled('dczhubi','ai无脑〖铸币〗','dczhubi_check')) delete lib.skill.dczhubi.check;
			if(lib.skill.channi&&game.aiyh_skillOptEnabled('channi')){
				lib.skill.channi.check=function(card){
					var player=_status.event.player,num=player.hasSkill('nifu')?15:8;
				...
				if(lib.skill.channi.subSkill&&lib.skill.channi.subSkill.backup) lib.skill.channi.subSkill.backup.check=(card)=>get.name(card)=='sha'?0:5.5-get.value(card);
			}
			...
			if(lib.skill.rexiantu){
				if(game.aiyh_skillOptEnabled('rexiantu','优化〖献图〗给牌ai')) lib.skill.rexiantu.check=function(event,player){
					...
					if(player.hp<=1&&!player.hasCard(card=>get.name(card)==='tao'||get.name(card)==='jiu','hs')) return false;
					return true;
				};
			...
			if(lib.skill.sbjiang&&lib.skill.sbjiang.subSkill&&lib.skill.sbjiang.subSkill.qiben&&game.aiyh_skillOptEnabled('sbjiang','优化〖激昂〗吃桃ai','sbjiang_qiben')){
				...
				lib.skill.sbjiang.subSkill.qiben.ai.nokeep=true;
			...
			if(lib.skill.sbkurou){
				...
				if(game.aiyh_skillOptEnabled('sbkurou','增加〖苦肉〗濒死ai','sbkurou_ai')){
					...
					lib.skill.sbkurou.ai.nokeep=true;
			...
			if(lib.config.extension_AI优化_btAi=='gjcx'){
				...
				for(let i of zhs){
					...
					lib.skill[i].ai.nokeep=true;
					let stf='lib.skill.'+i+'.ai.skillTagFilter=function(player,tag,arg){\n\tif(tag==="nokeep") return (!arg||arg.card&&get.name(arg.card)==="tao")&&player.isPhaseUsing()&&!player.getStat().skill.'+i+'&&player.hasCard((card)=>get.name(card)!="tao","h");\n};';
					try{
						eval(stf);
					}catch(e){
				...
				if(lib.skill.zhanjue&&game.aiyh_skillOptEnabled('zhanjue','优化〖战绝〗吃桃ai','zhanjue_ai')){
					...
					lib.skill.zhanjue.ai.nokeep=true;
				...
				if(lib.skill.rezhanjue&&game.aiyh_skillOptEnabled('rezhanjue','优化〖战绝〗吃桃ai','rezhanjue_ai')){
					...
					lib.skill.rezhanjue.ai.nokeep=true;
				...
				if(lib.skill.reqice&&game.aiyh_skillOptEnabled('reqice','优化〖奇策〗吃桃ai','reqice_ai')){
					...
					lib.skill.reqice.ai.nokeep=true;
		...
		if (lib.config.extension_AI优化_btAi != 'off') lib.arenaReady.push(function(){//本体AI优化
			...
			if(lib.card.tiesuo) lib.card.tiesuo.ai = {
				wuxie: function (target, card, player, viewer, status) {
					if (status*get.attitude(viewer,player)>0&&!player.isMad() || target.hasSkillTag('nodamage') || target.hasSkillTag('nofire') || target.hasSkillTag('nothunder') || (1+target.countCards('hs'))*_status.event.getRand()>1.57 || get.attitude(viewer, player) > 0) return 0;
				},
				basic: {
					order: 7,
					useful: 4,
			...
			if(lib.skill.gx_chongyingshenfu&&lib.skill.gx_chongyingshenfu.subSkill&&lib.skill.gx_chongyingshenfu.subSkill.effect) lib.skill.gx_chongyingshenfu.subSkill.effect.ai={
				effect:{
					target:function(card,player,target){
						if(typeof card=='object'&&target.storage.gx_chongyingshenfu_effect&&target.getStorage('gx_chongyingshenfu_effect').includes(card.name)&&!target.hasSkillTag('unequip2')&&(get.itemtype(player)!='player'||!player.hasSkillTag('jueqing',false,target)&&!player.hasSkillTag('unequip',false,{
							name:card.name,
							target:target,
							card:card,
						}))){
					...
					ai: {
						effect: {
							player: function (card, player, target) {
								...
								if (get.name(card) == 'sha') {
									if (!game.hasNature(card) && (target.getEquip('tengjia') || target.getEquip('rewrite_tengjia')) && !player.hasSkill('zhuque_skill')) return 'zeroplayertarget';
	...
	config: {
				...
				switch (link) {
					case 'jieshao':
						node.innerHTML =
							...
							'<font color=#FFFF00>本体武将优化相关</font>：<br>DIY文鸯 膂力<br>孙策&孙坚 英魂<br>刘焉 立牧<br>许贡 业仇<br>界曹丕 放逐<br>神司马懿 完杀<br>华佗 急救<br>司马徽 荐杰<br>邓艾 屯田<br>神荀彧 灵策 奇正相生<br>神马超 横骛 狩骊<br>贾逵 忠佐<br>吕虔 献刀<br>SP贾诩 拥嫡<br>范疆张达 决生<br>孙权 制衡<br>神赵云 龙魂<br>神诸葛 大雾<br>新潘凤 狂斧<br>曹髦 潜龙 决讨<br>神甘宁 劫营<br>界颜良文丑 双雄<br>山包界蔡文姬 悲歌<br>曹冲 仁心 称象<br>王瓘 谬焰' +
							'<br>滕芳兰〖哀尘〗<br>晋司马师〖夷灭〗<br>界祝融〖长标〗<br>罗宪〖带砺〗<br>张瑾云〖继椒〗<br>万年公主〖枕戈〗<br>孙策〖激昂〗<br>武诸葛亮〖情势〗〖智哲〗<br>夏侯玄〖清议〗<br>赵娥〖言誓〗<br>OL阚泽、十周年阚泽〖宽释〗<br>曹华〖归离〗<br>周善〖胆迎〗<br>张松、界张松〖献图〗<br>张角、旧张角〖雷击〗<br>界张角〖雷击〗〖鬼道〗<br>薛灵芸〖暗织〗<br>孙权、界孙权〖制衡〗<br>刘谌、界刘谌〖战绝〗<br>荀攸、界荀攸〖奇策〗<br>刘辩〖诗怨〗<br>TW严纲〖先锋〗<br>十周年陈泰〖救陷〗<br>神孙策〖英霸〗<br>鲁肃、界鲁肃〖好施〗<br>周宣〖寤寐〗<br>战役篇孔融〖名士〗<br>卫兹〖援资〗<br>乐进〖骁果〗<br>界王异〖贞烈〗<br>谢灵毓〖元嫡〗<br>界满宠〖峻刑〗<br>韩猛〖截粮〗<br>谋孙权〖制衡〗〖统业〗<br>TW曹操〖治暗〗<br>严虎〖雉盗〗<br>谋祝融〖烈刃〗<br>何晏〖清谈〗<br>马日磾〖秉节〗<br>谋黄盖〖苦肉〗〖诈降〗<br>花鬘〖蛮嗣〗<br>谋孙策〖激昂〗<br>杨彪〖义争〗<br>3D丁奉〖短兵〗<br>OL周群〖天候〗<br>夏侯紫萼〖血偿〗<br>界刘禅、手杀刘禅〖放权〗';
						break;
				}
			}
		},
		...
		hsyh: {
			name: '本体函数优化',
			intro: '优化展示牌时长(showCards)、拼点AI(chooseToCompare)',
			init: true
		},
		bd4: {
			clear: true,
	...
	package: {
		...
		intro: '<font color=#00FFFF>建立者</font>：<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp柚子丶奶茶丶猫以及面具<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp翩翩浊世许公子<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp157<br><font color=#00FFFF>现更者</font>：<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp157<br><font color=#00FFFF>特别鸣谢</font>：<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp寰宇星城(插件功能)<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp༺ཌༀཉི梦ღ沫ღ惜༃ༀ(工具人)<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp萌新（转型中）(本体优化)<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp😁呲牙哥！(扩展宣传)<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp读书人(扩展宣传)<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp幸运女神在微笑(扩展宣传)<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspAurora(代码参考)<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp蓝色火鸡(代码提供)<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp呓如惑(测试反馈)<br><font color=#00FFFF>当前版本号</font>：<font color=#FFFF00>1.2.6</font><br><font color=#00FFFF>支持本体版本号</font>：<font color=#FFFF00>1.10.1</font><br><font color=#00FFFF>更新日期</font>：23年<font color=#00FFB0>10</font>月<font color=#FFFF00>20</font>日<font color=fire> 0</font>时<br>',
		author: '',
		diskURL: '',
		forumURL: '',
		version: '1.2.6'
