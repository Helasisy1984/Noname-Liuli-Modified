	...
	return {
		...
		characterSort:{
			sp:{
				...
				sp_huben:['duanjiong','ol_mengda',"caohong","xiahouba","zhugeke","zumao","wenpin","litong","mazhong","heqi","quyi","luzhi","zangba","yuejin","dingfeng","wuyan","ol_zhuling","tianyu","huojun",'zhaoyans','dengzhong','ol_furong','macheng','ol_zhangyis','ol_zhujun','maxiumatie','luoxian','ol_huban','haopu','ol_qianzhao'],
				...
				sp_wanglang:['ol_wanglang','ol_puyuan','ol_zhouqun','ol_nanhualaoxian'],
				sp_zhongdan:["cuiyan","huangfusong"],
				sp_guozhan2:["sp_dongzhuo","liqueguosi","zhangren"],
				sp_tianshu:["hanba"],
				onlyOL_demonized: ["dm_simayi","dm_lvbu","dm_diaochan"],
                sp_others:["caiyang"],
			},
		},
		characterFilter:{
		...
		character:{
			dm_lvbu: ["male", "qun", 4, ["olduoqi", "olkuangmo", "olgangquan"]],
        	dm_diaochan: ["female", "qun", 3, ["olhuanhuo", "olqingshi"]],
            dm_simayi: ["male", "wei", 3, ["olguifu", "olmoubian"]], 
			...
			ol_zhangyis:['male','shu',4,['oldianjun','olkangrui']],
		...
		characterIntro:{
			...
			zhaoyans:'赵俨（171~245年），字伯然，颍川阳翟（今河南禹州市）。东汉末年颍川“四大名士”之一，三国时期魏国名臣。熟读经史，精明强干。建安二年，投靠大将军曹操之后，起家朗陵县令，历任司空府掾、司空主薄、都督护军、扶风太守等职。魏文帝曹丕继位后，历任魏国侍中、驸马都尉、河东太守、典农中郎将、度支尚书，封宜土亭侯。魏明帝曹睿时期，历任大司农、骠骑将军、大司空等职。正始六年，去世，时年七十五，谥号为穆。',
		...
		characterTitle:{
    		...
    		dongbai: '#y魔姬',
    		zhaoxiang: '#y拾梅鹊影',
    		ol_zhangchangpu: '#y省身明德',
    		ol_xinxianying: '#y才涌花娇',
    		daxiaoqiao: '#y江东之花',
    		jin_guohuai: '#y妒贤嫉能',
    		simahui: '#y水镜先生',
    		zhangbao: '#y地公将军',
    		...
    		sp_diaochan: '#y暗黑傀儡师',
    		sp_zhaoyun: '#y真龙之意',
    		sp_sunshangxiang: '#y梦醉良缘',
    		sp_caoren: '#y险不辞难',
    		sp_jiangwei: '#y幼麒',
    		sp_machao: '#y西凉的猛狮',
    		sp_caiwenji: '#y金璧之才',
    		jsp_guanyu: '#y汉寿亭侯',
    		jsp_huangyueying: '#y闺中璞玉',
    		sp_pangde: '#y抬榇之悟',
    		sp_jiaxu: '#y算无遗策',
    		yuanshu: '#y仲家帝',
    		sp_zhangliao: '#y功果显名',
		...
		skill:{
			...
			olgoude:{
				...
				content:function(){
					...
					var contents=lib.skill.olgoude.contents;
					lib.translate['olgoude_'+result.control]="苟得";
        	...
        	olhedao: {
        		...
        		filter(event, player) {
        			if (event.name === "dying" && !player.storage.olhedao_dyingAfter) {
        			    player.storage.olhedao_dyingAfter = true;
        			    return true;
        			}
        			//game.getAllGlobalHistory("every个thing", evt => evt.name === "dying" && evt.player === player).indexOf(event) === 0;
        			return event.name !== "phase" || game.phaseNumber === 0;
        		},
        		forced: true,
        		priority: 12, //本体御用优先级
        		...
        		tianshuContent: [
        			...
        			},
        			{
        				...
        				effect: {
        					...
        					content() {
        						'step 0'
        						player.chooseToDiscard(get.prompt2(event.name/*.slice(0, -"_cost".length)*/), "he", [1, Infinity]/*, "chooseonly"*/).set("ai", lib.skill.zhiheng.check).set("logSkill", event.name);
        						'step 1'
        						if(result.bool&&result.cards) {
        			...
        			},
        			{
        				...
        				effect: {
        					...
        					content() {
        					    player
        							.chooseUseTarget(get.prompt2(event.name), { name: "sha" }, false, "nodistance")
        							.set("oncard", () => {
        			...
        			},
        			{
        				...
        				effect: {
        					...
        					content() {
    							...
    							'step 2'
        						if (result.bool === false&&event.target.isIn()) event.target.damage(2, "thunder");
        					},
        				},
        			},
        			{
        				...
        				effect: {
        					...
        					content() {
    							...
    							if(result.bool&&result.cards) {
            						player.logSkill(event.name);
            						event.chooseCardResultCards = result.cards;
            						lib.skill.olhedao.tianshuClear(event.name, player);
            						player.respond(event.chooseCardResultCards, event.name, "highlight", "noOrdering");
        						}else {
        						    event.finish();
        						}
        						'step 2'
        						...
        						'step 3'
        						trigger.player.judging[0] = event.chooseCardResultCards[0];
        						trigger.orderingCards.addArray(event.chooseCardResultCards);
        						game.log(trigger.player, "的判定牌改为", event.chooseCardResultCards[0]);
        						game.delay(2);
        					},
        					ai: {
        						rejudge: true,
        			...
        			},
        			{
        				...
        				effect: {
        					...
        					content() {
        						...
        						'step 2'
        						if (result.bool&&event.target.isIn()) player.gainPlayerCard(event.target, 2, "he", true);
        					},
        				},
        			},
        			{
        				...
        				effect: {
        					...
        					content() {
    							...
    							if(result.bool&&result.targets) {
        							player.logSkill(event.name,result.targets);
            						lib.skill.olhedao.tianshuClear(event.name, player);
            						if (result.targets.length === 1) result.targets[0].draw();
            						else {
            							game.asyncDraw(result.targets);
            							game.delayx();
            						}
        				...
        				effect: {
        					...
        					content() {
        						...
        						player
        							...
        							.set("ai", target => {
        								const player = get.player();
        								return get.effect(target, { name: "wuzhong" }, player, player) * (1 + target.countCards("h"));
        							});
    							'step 1'
    							if(result.bool&&result.targets) {
        							player.logSkill(event.name,result.targets);
        						...
        						player
        							.chooseCardTarget({
        								...
        								filterTarget(card, player, target) {
        									return !player.getStorage("olhedao_effect").includes(target)&&target!=player;
        								},
        								filterCard: lib.filter.cardDiscardable,
        								selectCard: 2,
        								position: "he",
        			...
        			},
        			{
        				...
        				effect: {
        					getIndex(event, player) {
        						return 0 + game.hasPlayer(target => target.countCards("e"));
        					},
        					direct:true,
        					content() {
        						'step 0'
        						player
        							.chooseTarget(
        								...
        								(card, player, target) => {
        									if (!ui.selected.targets.length) return true;
        									return target.countCards("e") + ui.selected.targets[0].countCards("e") > 0;
        								},
        								2
        							)
        							.set("complexTarget", true)
        			...
        			},
        			{
        				...
        				effect: {
        					...
        					check(event, player) {
        						...
        						if (get.effect(event.source, { name: "wuzhong" }, player, player) * 3 + event.num * get.damageEffect(player, event.source, player, event.nature) > 0) return true;
        			...
        			effect: {
        				...
        				mod: {
        					targetInRange(card, player, target) {
        						if (player.getStorage("olhedao_effect").includes(target)) return true;
        					},
        	...
        	olqingshu: {
        		...
        		content() {
    				...
    				'step 2'
        			event.from = result.links[0];
        			const ToItems = lib.skill.olhedao.tianshuContent.filter(item => {
        				if (event.from.fromIndex !== item.toIndex) return false;
        				return !item.filter || item.filter(event.from.name);
        			});
        			player
        				.chooseButton(['###青书：请选择“天书”效果###<div class="text center">（效果等级：' + event.from.fromIndex + "）" + event.from.name + "</div>", [ToItems.randomGets(3).map(item => [item, item.name]), "textbutton"]], true)
        				.set("ai", () => 1 + Math.random());
    				'step 3'
    				if(!result.bool||!result.links||!result.links.length) event.finish();
        			'step 4'
        			event.to = result.links[0];
        			let skill;
        			while (true) {
        	...
        	olshoushu: {
        		...
        		content() {
        			...
        			if (result?.bool && result.links?.length) {
        				...
        				var skills = target.getSkills(null, false, false).filter(skill => get.info(skill)?.olhedao);
        	...
        	olduoqi: {
        		audio: 2,
        		group: ["olduoqi_gain", "olduoqi_mark", "olduoqi_myMark"],
        		trigger: {
        			global: "phasePrepare",
        		},
        		init:function(player,skill) {
        		    if(typeof lib.onphase=='object') {
        		        //厉不厉害你坤哥，Helasisy插入回合开始前机制
        		        lib.onphase.push(function() {
        		            var next=game.createEvent('phasePrepare');
        					next.player=_status.event?.player;
        					next.skill='olduoqi_prepare';
        					next.setContent(function() {
        					    event.trigger(event.name);
					            //game.log(player,'的回合还没开始');
        					});
        		        });
        		    }
        		},
        		...
        		cardslist(player, target, killGain = false) {
        			const filterCard = card => card.storage?.isOlduoqi/*card.hasGaintag("olduoqi_tag")*/ && target._startCards.includes(card);
        		...
        		content() {
        			...
        			//好家伙，原来的代码太超前了，赫拉酱按自己的理解重写了捏
        			//效果还挺不错的
        			player.addWhen({
        			    trigger: {
        			        player: "phaseBegin",
        			    },
        			    filter: function(event, skill) {
        			        return event.skill == "olduoqi";
        			    },
        			    firstDo: true,
        			    content: function() {
        		...
        		subSkill: {
        				...
        				filter(event, player) {
        					...
        					return (
        						player
        							...
        							.indexOf(event) == 0 && event.player?._startCards?.some(card => cards.includes(card))
        					);
        				},
        				content() {
        					...
        					if (cards.length) {
        						...
        						if (get.owner(card)) {
        							player.gain(card, "give", get.owner(card));
        						} else {
        							player.gain(card, "gain2");
        						}
        					}
        			...
        			mark: {
        				...
        				popup: false,
        				filter(event, player) {
        					return game.hasPlayer(target => target.countCards('h') > 0);//game.hasPlayer(target => target._startCards?.length);
        				},
        				content() {
        					//game.filterPlayer().forEach(target => target.addGaintag(target._startCards, "olduoqi_tag"));
        					game.filterPlayer().forEach(target => {
        					    target._startCards = target.getCards('h');
        					    target._startCards.forEach(card => {
        					        card.storage.isOlduoqi = target;
        					    });
        					    if(game.isMine(target)) {
        					        target.addGaintag(target._startCards, "olduoqi_tag");
        					    }
        					});
        				},
        			},
        			myMark: {
        				...
        				firstDo:true,
        				filter:function(event,player){
        					//if(!player._startCards?.length) return false;
        					//return player._startCards.some(card => !card.hasGaintag("olduoqi_tag"));
        					if(!game.isMine(player)) return false;
        					if(!player.countCards('hes')) return false;
        					event.needToTagCards = player.getCards('hes', function(card) {
        					    return card.storage?.isOlduoqi && !card.hasGaintag("olduoqi_tag");
        					});
        					return event.needToTagCards.length > 0;
        					
        				},
        				content:function(){
        					//player.addGaintag(player._startCards,'olduoqi_tag');
        					player.addGaintag(trigger.needToTagCards,'olduoqi_tag');
        				},
        			},
        			tag: {},
        			limit: {
        	...
        	olkuangmo: {
        		...
        		content() {
        			'step 0'
        			if(['level2','level3'].contains(lib.config.effect_dm_lvbu)) {
                        let type = lib.config.effect_dm_lvbu;
                        if(type == 'level3') {
                        ...
                        game.playAnimation('extension/标记补充/animation/molvbu.mp4', function() {
                            ...
                            setTimeout(function() {
                                ...
                                player.createHighLight([[255,255,255],[255,255,50],[255,50,50]],7,{level:10}).style.animation = 'olkuangmo_loop 5s infinite ease';
                            }, 3400);
                        }, function() {
                            item.clearImg();
                        });
					...
					if(!player.storage[name+"_player"]) player.storage[name+"_player"] = '';
					player.addMark(name+"_player", get.translation(target));
					if(!target.storage[name+"_player"]) target.storage[name+"_player"] = '';
					target.addMark(name+"_player", get.translation(player));
        		...
        		subSkill: {
        			point: {
        				...
        				content() {
        					'step 0'
        					player.chooseTarget(get.prompt("olkuangmo"), "重新指定一名角色，你与其成为“狂”角色", lib.filter.notMe, true)
        						.set("ai", target => {
        							const player = get.player();
        							return get.damageEffect(target, player, player);
        						});
    						'step 1'
        					if (!result.bool || !result?.targets[0]?.isIn()) {
        						event.finish();
        					}else {
            					const target = result.targets[0];
            					player.line(target, "fire");
        			...
        			effect_player: {
        				...
        				intro: {
        					content: function(storage,player){
        					    return "跟"+get.translation(player.storage.olkuangmo_effect)+"魔怔上了";
    					    },
        				},
        			},
        			effect: {
        				...
        				filter(event, player) {
        					if (
        						...
        						/*event.player.getHistory('damage', evt => {
        						    return evt.source == player && evt.player == event.player;
    						    }).indexOf(event) != 0*/
    						    player.getHistory('sourceDamage',evt => evt.player == event.player).length > 0
        					) {
        						return false;
        					}
        				...
        				content() {
    						...
    						game.loseAsync({ lose_list: lose_list }).setContent("chooseToCompareLose");
    						'step 2'
    						game.delayx();
    						'step 3'
        	...
        	olgangquan: {
        		...
        		onChooseToUse(event) {
        			...
        			event.set(
        				"olgangquan",
        				evt.cards?.some(card => card.storage?.isOlduoqi/*card.hasGaintag("olduoqi_tag")*/)
        			);
        		},
        		chooseButton: {
        			dialog(event, player) {
        		...
        		subSkill: {
        			...
        			mark: {
        				...
        				init(player, skill) {
        					...
        					const bool = evt.cards?.some(card => card.storage?.isOlduoqi/*card.hasGaintag("olduoqi_tag")*/);
        					player.markSkill(skill);
        					game.broadcastAll(
        						function (index, player) {
        							const name = "olgangquan_mark";
        							const bgColor = get.info(name).markColor[index],
        							    textShadow = `0px 0px 4px ${bgColor}`,
        								text = `<span style = "text-shadow: ${textShadow}, ${textShadow}, ${textShadow};">${index ? "火杀" : "决斗"}</span>`;
        	...
        	olqingshi: {
        		...
        		content() {
        			...
        			'step 3'
        			if(!event.noAnimation && ['level2','level3'].contains(lib.config.effect_dm_diaochan)) {
		    ...
		    olguifu: {
                ...
                trigger: {
                    global: "roundStart",
                    player: ["recoverEnd", "damageSource", "loseHpEnd"]
                },
                check: () => true,
                frequent: true,
                ...
                content: function() {
                    ...
                    "step 1"
                    player.gain(event.card, "gain2").gaintag.add("olguifu");
                    event.finish();
                },
                ...
                subSkill: {
                    viewAs: {
                        ...
                        filter(event, player) {
                            const names = player.storage.olguifu_record.card
                                .slice(0)
                                .removeArray(player.getStorage("olguifu_used"));
                            return (
                                player.hasCard(card => card.hasGaintag("olguifu"), "h") &&
                                names.some(name =>
                                    event.filterCard(
                                        get.autoViewAs({ name: name, storage: { olguifu_viewAs: true } }, "unsure"),
                                        player,
                                        event
                                    )
                                )
                        ...
                        chooseButton: {
                            ...
                            filter(button, player) {
                                ...
                                return currentEvent.getParent().filterCard(
                                    get.autoViewAs({ 
                                        name: button.link[2], 
                                        nature: button.link[3], 
                                        storage: { olguifu_viewAs: true } 
                                    }, "unsure"),
                                    player,
                                    currentEvent.getParent()
                                );
                            },
                            check(button) {
                                ...
                                return {
                                    ...
                                    precontent() {
                                        ...
                                        player.addWhen({
                                            trigger: { source: "damageSource" },
                                            filter: function(event, player) {
                                                const trigger = _status.event; // 获取当前事件对象
                                                return trigger == player.getStat().skill.olguifu;
                                            },
                                            content: function() {
                                                delete player.getStat().skill.olguifu;
                                                game.log(player, "重置了", "#g【诡伏】");
                                                player.getHistory("custom").push({ olguifu: true });
                                            },
                                        });
                                    }
                                };
                    ...
                    record: {
                        ...
                        filter(event, player) {
                            ...
                            let skill = event.getParent(1).name;
                        ...
                        content() {
                            ...
                            if (trigger.card) {
                                ...
                                const skill = trigger.getParent(1).name;
            ...
            olmoubian: {
                ...
                filter: function(event, player) {
                    try {
                        const storage = player.storage.olguifu_record || {};
                        const totalRecords = (storage.card || []).length + (storage.skill || []).length;
                        
                        return totalRecords >= 3 && !player.hasSkill("olrumo");
                    } catch (e) {
                        console.error("olmoubian filter error:", e);
                        return false;
                    }
                },
                skillAnimation: "epic",
                ...
                content: function() {
                    'step 0'
                    if(['level2','level3'].contains(lib.config.effect_dm_simayi)) {
                        let type = lib.config.effect_dm_simayi;
                        if(type == 'level3') {
                            _status.tempMusic='effect_dm_simayi';
                            game.playBackgroundMusic();
                        }
                        // 修复jie的translate括号并添加退出动画
                        game.createCss(`@keyframes textEnter {
                        ...
                        item.imgMap = {
                            ...
                            jie: {
                                width: "180px",
                                translate: "translate(calc(-50% + 369px), calc(-50% - -158px))", // 修复括号
                            },
                        };
                        ...
                        game.playAnimation('extension/标记补充/animation/mosimayi.mp4', function() {
                            Object.keys(item.imgMap).forEach((key, index) => {
                                item.imgMap[key].t = setTimeout(() => {
                                    item.createImg(key);
                                }, 3100 + index * 100);
                            ...
                            setTimeout(function() {
                                if(type == 'level3') ui.background.classList.add('bloody-effect');
                                if(type == 'level3') player.changeAvatarTo('dm_simayi','dm_simayi_shadow');
                                player.createHighLight([[255,255,255],[255,255,50],[255,50,50]],7,{level:10}).style.animation = 'olmoubian_loop 5s infinite ease';
                            }, 3400);
                        }, function() {
                            item.clearImg();
                        });
                    ...
                    try {
                        ...
                        const recordSkills = player.storage.olguifu_record?.skill || [];
                        recordSkills.forEach(function(skill) {
                            if (skill && !player.hasSkill(skill)) {
                                player.addSkill(skill);
                            }
                        });
                        if (!player.hasSkill("olzhouxi")) {
                            player.addSkill("olzhouxi");
                        }
                        player.markSkill("olmoubian");
                        
                    } catch (e) {
                        console.error("olmoubian content error:", e);
                    }
                },
                ai: {
                    combo: "olguifu",
                    result: {
                        target: function(player, target) {
                            return player.hasSkillTag("olguifu") ? 2 : 0;
                        }
                    }
                },
                intro: {
                    content: "当你的「诡伏」记录数≥3时，准备阶段开始时你可以获得技能「入魔」和记录的技能",
                },
                onremove: function(player) {
                    player.removeSkill("olrumo");
                    const recordSkills = player.storage.olguifu_record?.skill || [];
                    recordSkills.forEach(function(skill) {
                        player.removeSkill(skill);
                    });
            ...
            olzhouxi: {
                ...
                content: function () {
                    ...
                    "step 1"
                    const dialog = ui.create.dialog(`###${get.translation(event.name)}###<div class="text center">你从三个可造成伤害的技能中选择一个获得直到你的下回合开始。</div>`, [
