	...
	return {
		...
		characterSort:{
			tw:{
				...
				tw_swordsman:['xia_xushu','xia_wangyue','xia_liyans','xia_tongyuan','xia_lusu','xia_dianwei','xia_zhaoe','xia_xiahouzie'],
				tw_beidingzhongyuan: ["huan_zhugeliang", "huan_jiangwei", "huan_zhanghe", "huan_zhugeguo", "huan_simayi", "huan_zhaoyun"],
				tw_weianglongxing: ["huan_caopi", "huan_dianwei", "huan_caoang"],
		...
		character:{
			...
			huan_fantasy_zhugeliang: ["male", "shu", "3/4", ["twbeidingx", "twjielvx", "twhuanji", "twchanggui"], ['unseen']],
			...
			tw_fengxis:['male','shu',4,['twqingkou']],
			...
			xia_liyans:['male','qun',4,['twzhenhu','twlvren']],
		...
		characterIntro:{
			kaisa:'恺撒，全名盖乌斯·尤利乌斯·恺撒（Gaius Julius Caesar，公元前100年7月13日—公元前44年3月15日），史称恺撒大帝，又译盖厄斯·儒略·凯撒、加伊乌斯·朱利叶斯·凯撒等，罗马共和国（今地中海沿岸等地区）末期杰出的军事统帅、政治家，并且以其优越的才能成为了罗马帝国的奠基者。',
			...
			liyans:'李彦，号称"并州第一戟"，是童渊的师兄。早年间两人在玉真子门下一起习武，后成年出师开枝散叶。同为并州人的吕布在得知李彦的名声后，投入其门下学习武艺。',
		...
		characterTitle:{
    		...
    		tw_liufuren: '#y酷妒的海棠',
    		tw_liuzhang: '#y半生黯然',
    		tw_mateng: '#y驰骋西陲', //这个皮肤在海外和普通称号一样，但是在国内OL改名叫安狄将军
    		tw_niufudongxie: '#y虺伴蝎行',
    		tw_qiaorui: '#y穷勇技尽',
    		tw_weixu: '#y缚陈降曹',
    		tw_yanxiang: '#y明尚夙达',
    		tw_yufuluo: '#y援汉雄狼',
		...
		skill:{
			...
			twzhuiting:{
				...
				zhuSkill:true,
				audio:2,
				global:['twzhuiting_global','twzhuiting_player'],
				//修复玩家无法使用此技能、非正常条件无脑询问无懈
				subSkill:{
					player:{
					    trigger:{
					    ...
					    filter:function(event,player){
							if(player.hasSkill('twzhuiting_wuxie_black')) player.removeSkill('twzhuiting_wuxie_black');
							if(player.hasSkill('twzhuiting_wuxie_red')) player.removeSkill('twzhuiting_wuxie_red');
						...
						firstDo:true,
						content: function () {
							var cardx=trigger.getParent(4).card;
							var color=get.color(cardx);
							if(['red','black'].contains(color)) {
							    player.addTempSkill('twzhuiting_wuxie_'+color,{player:'chooseToUseAfter'});
							    player.addWhen({
							        trigger:{
                                        player:["useCard1"],
                                    },
                                    firstDo:true,
                                    cancelTrigger:{player:'chooseToUseBefore'},
                                    filter:function(event,player){
                                        return get.name(event.card)=='wuxie'&&!event.skill&&
                                        event.cards.length==1&&event.cards[0].name!='wuxie';
                                    },
                                    content:function(){
                                        player.logSkill('twzhuiting');
                                    },
							    });
							}
						},
					...
					wuxie_red:{
					    ...
					    mod:{
					        cardname:function(card,player){
					            if(get.color(card)=='red') return 'wuxie';
					        },
					    },
					},
					global:{
						...
						filter:function(event,player){
							...
							return target&&cardx&&target!=player&&target.hasZhuSkill('twzhuiting')&&player.countCards('hes',card=>get.color(card,player)==get.color(cardx));*/
							var bool=true;
							if(!['wei','qun'].contains(player.group)) bool=false;
							if(!event.filterCard({name:'wuxie'},player,event)||!lib.inpile.contains('wuxie')) bool=false;
							var target=event.getParent(4)[event.getParent(4).name=='phaseJudge'?'player':'target'];
							var cardx=event.getParent(4).card;
							if(bool&&target&&cardx&&target!=player&&target.hasZhuSkill('twzhuiting')&&player.countCards('hes',card=>get.color(card,player)==get.color(cardx))) {
							    return player!=game.me;
							}else if(event.filterCard({name:'wuxie'},player,event)&&!lib.skill.twzhuiting.subSkill.global.hasUsableCard(player,'wuxie')&&player==game.me) {
							    ...
							    player.addWhen({
							        ...
							        filter:function(event,player){
							            var cardx=event.getParent(4).card;
							            return cardx==_when.card;
							        },
							        content:function(){
							            game.broadcastAll(function(player) {
            							    if(player.forceCountChoose) delete player.forceCountChoose;
            							}, player);
						...
						viewAsFilter:function (player){
						    var event=_status.event;
							var bool=true;
							if(!game.hasPlayer(target=>target!=player&&target.hasZhuSkill('twzhuiting'))) return false;
							if(!['wei','qun'].contains(player.group)) bool=false;
							if(!lib.inpile.contains('wuxie')) bool=false;
							var cardx=event.getParent(4).card;
							if(bool&&cardx&&player.countCards('hes',card=>get.color(card,player)==get.color(cardx))) {
							    return true;
							}
							return false;
				        },
			...
			twyuejian:{
				...
				content:function(){
					...
					next.set('processAI',function(list){
						var cardsAI=list[0][1],player=_status.event.player;
						...
						if(player!=target||!target.hasWuxie()){
							for(var i=0;i<judges.length;i++){
								...
								cardsAI.sort(function(a,b){
									return (judge(b)-judge(a))*att;
								});
								if(judge(cardsAI[0])*att<0){
									stopped=true;break;
								}
								else{
									top.unshift(cardsAI.shift());
								}
							}
						}
						var bottom;
						if(!stopped){
							...
							while(cardsAI.length){
								if((get.value(cardsAI[0],player)<=5)==(att>0)) break;
								top.unshift(cardsAI.shift());
							}
						}
						bottom=cardsAI.sort(function(a,b){
							return player.getUseValue(a)-player.getUseValue(b);
						});
						return [top,bottom];
					});
        	...
        	twbeiding: {
        		audio: 2,
        		audioname2: { huan_fantasy_zhugeliang: "twbeidingx" },
        		intro: {
        			content: "已记录牌名：$",
        		},
        		mod: {
        		...
        		subSkill: {
        			record: {
        				...
        				content: function() {
        					...
        					player
        						.chooseButton([`${get.translation('twbeiding')}：你可以声明并记录至多${get.cnNumber(num)}个未以此法记录的牌名`, [vcards, "vcard"]], [1, num])
        						.set("filterButton", button => {
        							if(ui.selected.buttons&&ui.selected.buttons.contains(button)) {
        							    game.highLightCard(button,true);
        							}else {
        					...
        					'step 1'
        					if(result.bool) {
        						player.logSkill('twbeiding_record');
        						event.result = {
        							bool: result.bool,
        							cost_data: result.links,
        						};
        						const names = event.result.cost_data.map(link => [{ name: link[2], nature: link[3] }]);
        			...
        			use: {
        				...
        				content: function() {
        					...
        					event.storage = player.getStorage(event.name);
        					'step 1'
        					if(!event.storage.length) event.goto(4);
        					'step 2'
        					const name = event.storage.shift(),
        						card = get.autoViewAs({ name: name[0].name, nature: name[0].nature, isCard: true });
        					if (player.hasUseTarget(card, false)) {
        						player.chooseUseTarget(`请选择${get.translation(card)}的目标，若此牌的目标不包含${get.translation(target)}，则其摸一张牌`, card, true, false, "nodistance");
        					}else {
        						event.goto(1);
        					}
        					'step 3'
        					if (!result.targets.includes(event.target) && event.target.isIn()) event.target.draw();
        					event.goto(1);
        					'step 4'
        			...
        			huan: {
        				...
        				content: function() {
        					if (event.triggername == "useCard1") {
        						...
        						if(Array.isArray(player.storage.twbeiding)) {
        						    player.storage.twbeiding=player.storage.twbeiding.filter(i => i[0].name != trigger.card.name);
        						    player.markAuto("twbeiding");
        						}
        					}
        				},
        			},
        	...
        	twjielv: {
        		audio: 2,
        		audioname2: { huan_fantasy_zhugeliang: "twjielvx" },
        		derivation: "twjielvx",
        	...
        	twhunyou: {
        		...
        		content: function() {
        			...
        			if(player.getHp()<1) player.recover(1-player.getHp(true));
        			'step 1'
        			...
        			/*player
        				...
        				.then(() => {
        					...
        					player.changeSkin({ characterName: "huan_zhugeliang" }, "huan_fantasy_zhugeliang");
        		...
        		subSkill: {
        			buff: {
        				...
        				onremove: function(player, skill) {
        				    ...
        				    player.insertPhase();
        				    'step 1'
        					//player.changeAvatarTo('huan_zhugeliang','huan_fantasy_zhugeliang');
        					player.changePlayerTo('huan_zhugeliang','huan_fantasy_zhugeliang');
        					'step 2'
        					if(lib.config.extension_十周年UI_enable&&lib.config.effect_huan_zhugeliang) {
        				    	decadeUI.animation.playSpine({ name: 'SSHW_ruhuan_lizhuge', speed: 1, scale: 1, x: [0, 0.5], y: [0, 0.5] });
        				        decadeUI.animation.playSpine({ name: 'SSHW_ruhuan_ruhuan', speed: 1, scale: 1, x: [0, 0.5], y: [0, 0.5] });
        				    }
        					player.storage.isInHuan = true;
        					player.changeSkills(get.info("twhunyou").derivation, ["twhunyou"]);
        				},
        		...
        		chooseButton: {
        			...
        			backup: function(links, player) {
        				return {
        					...
        					content: function() {
        						'step 0'
        						event.names = lib.skill[event.name].names;
        						player.loseMaxHp();
        						'step 1'
        						game.log(player, "声明了", "#g" + get.translation(event.names));
        						player.markAuto("twbeiding", event.names);
        					},
        				};
        			},
        			prompt(links, player) {
        	...
        	twchanggui: {
        		...
        		content: function() {
        			...
        			if(lib.config.extension_十周年UI_enable&&lib.config.effect_huan_zhugeliang) {
        				decadeUI.animation.playSpine({ name: 'SSHW_chuhuan_biaozhuge', speed: 1, scale: 1, x: [0, 0.5], y: [0, 0.5] });
        			}
        			//player.changeAvatarTo('huan_zhugeliang','huan_fantasy_zhugeliang',false);
        			player.changePlayerTo('huan_fantasy_zhugeliang','huan_zhugeliang');
        			player.changeSkills(["twhunyou"], get.info("twhunyou").derivation);
        		},
        	},
        	//幻姜维
        	twqinghan: {
        		...
        		content: function() {
        			...
        			event.target = target;
        			player.chooseToCompare(event.target).set("fixedResult", { [player.playerid]: event.card });
        			'step 1'
        			...
        			'step 2'
        			if (result.bool && result.links && result.links.length) player.useCard({ name: result.links[0][2] }, event.target, false);
        			'step 3'
        			const card1 = event.comres.player,
        				card2 = event.comres.target;
        			if (get.color(card1, player) == get.color(card2, event.target)) {
        	...
        	twzhihuan: {
        		...
        		content: function() {
        			...
        			'step 1'
        			if(!event.result.bool) event.finish();
        			'step 2'
        			trigger.cancel();
        			if (event.result.cost_data == "选项一") player.gainPlayerCard(trigger.player, "e", true);
        			else {
        				for (let i = 1; i < 7; i++) {
        	...
        	rexianyuan: {
        		...
        		content: function() {
        			...
        			event.num = event.target.countMark("rexianyuan");
        			let choice;
        			if (!event.target.countCards("h")) result.control = "选项二";
        			else
        				player
        					.chooseControl()
        					.set("choiceList", ["观看" + str + "的手牌并将其中至多" + get.cnNumber(event.num) + "张牌置于牌堆顶", "令" + str + "摸" + get.cnNumber(event.num) + "张牌"])
        					.set("ai", () => (get.attitude(get.player(), _status.event.getTrigger().player) > 0 ? 1 : 0));
        			//if (typeof choice != "number") return;
        			'step 1'
        			if (result.control == "选项一") {
        				player.choosePlayerCard(event.target, "h", "visible", [1, event.num], true, '###仙援###<div class="text center">将其中至多' + get.cnNumber(event.num) + "张牌置于牌堆顶（先选择的在上）</div>");
        			} else {
        			...
        			if (result.bool && result.cards?.length) {
        				...
        				event.target.$throw(cards.length, 1000);
        				event.target.lose(cards, ui.cardPile, "insert");
        			}
        			'step 3'
        			if (_status.currentPhase !== player) event.target.clearMark("rexianyuan");
        		},
        		limit: 3,
        		intro: { content: "mark" },
        	...
        	twxianyuan: {
        		...
        		filterTarget(card, player, target) {
        			return player != target && !_status.event.twxianyuan.includes(target);
        		},
        		filterCard: true,
        		selectCard: [1, 2],
        		position: "he",
        				...
        				async content(event, trigger, player) {
        					...
        					const result = await player
        						...
        						.set("processAI", list => {
        							const num = _status.event.num;
        							let cards = list[0][1],
        								sgn = get.attitude(_status.event.player, _status.event.getTrigger().player) > 0 ? 1 : -1;
        	...
        	twlingyin: {
        		...
        		content: function() {
        			...
        			const cards = get.cards(1, true),
        				card = cards[0];
        			player.showCards(cards, get.translation(player) + "发动了【技二】");
        	...
        	twjiezhan: {
        		...
        		check: function(event, player) {
        			...
        			return get.effect(player, { name: "wuzhong" }, player, player) + get.effect(event.player, { name: "sha" }, event.player, player);
        		},
        		logTarget: "player",
        		content: function() {
        			'step 0'
        			...
        			if (target.canUse(sha, player, false, true)) target.useCard(sha, player);
        		},
        	},
        	twlongjin: {
        		audio: 2,
        		...
        		content: function() {
        			...
        			if(player.getHp()<2) player.recover(2-player.getHp(true));
        			'step 1'
        	...
        	twzongquan: {
        		...
        		content: function() {
        			...
        			const history = player.getAllHistory("useSkill", evt => evt.skill == "twzongquan");
        				//judgeEvent = game.getAllGlobalHistory("every个thing", evt => evt.name == "judge" && evt.player == player && evt.getParent().name == "twzongquan");
    				var Starget, Sjudge;
    				if(player.storage.twzongquan&&player.storage.twzongquan.target&&player.storage.twzongquan.judge) {
        			    var last=player.storage.twzongquan;
        			    Starget = last.target;
        			    Sjudge = last.judge;
        			}
        			player
        				.chooseTarget(get.prompt("twzongquan"), `选择一名角色，然后你进行判定，并令其执行相应效果` + (/*history.length > 0*/Starget ? `（上次选择的目标：${get.translation(/*history.at(-1).targets[0]*/Starget)}` : ``) + (Sjudge ? `；上次判定的结果：${get.translation(Sjudge)}）` : ``))
        				.set("ai", target => {
        					const player = get.player(),
        						att = get.attitude(player, target);
        					/*if (player.getAllHistory("useSkill", evt => evt.skill == "twzongquan").length) {
        						const history = game.getAllGlobalHistory("every个thing", evt => {
        					...
        					if(Starget == target && Sjudge) {
        					    if (Sjudge == "black" && att > 0) return 1919810;
    							if (Sjudge == "red" && att < 0) return 114514;
        					}
        					return Math.abs(att);
        				});
        			'step 1'
        			if(!result.bool || !result.targets || !result.targets.length) event.finish();
        			'step 2'
        			event.target = result.targets[0];
        			...
        			'step 3'
        			/*const historys = player.getAllHistory("useSkill", evt => evt.skill == "twzongquan"),
        				judgeEvents = game.getAllGlobalHistory("every个thing", evt => evt.name == "judge" && evt.player == player && evt.getParent().name == "twzongquan");
        			let num;
        			if (historys.length > 1 && judgeEvents.length > 1 && event.target == historys.at(-2).targets[0] && result.color != judgeEvents.at(-2).result.color) {
        				num = 3;
        			} else num = 1;*/
        			let num = 1;
        			if(player.storage.twzongquan&&player.storage.twzongquan.target&&player.storage.twzongquan.judge) {
        			    var last=player.storage.twzongquan;
        			    if(event.target==last.target&&result.color!=last.judge) num = 3;
        			}
        			player.storage.twzongquan={
        			    target:event.target,
        			    judge:result.color,
        			};
        			if (result.color == "red") {
        			    event.target.draw(num);
        			}else if (result.color == "black" && event.target.countCards("he")) {
        			    event.target.chooseToDiscard("he", num, true);
        			}
        	...
        	twguimou: {
        		...
        		content: function() {
        			...
        			var jdgPlayer = trigger.player.judging[0];
        			player
        				.chooseButton([get.translation(trigger.player) + "的" + (trigger.judgestr || "") + "判定为" + get.translation(trigger.player.judging[0]) + "，" + get.prompt("twguimou"), event.cards], true)
        				.set("filterButton", button => {
        					const player = get.player(),
        						card = button.link;
        				...
        				.set("ai", button => {
        					...
        					if (name == "twzongquan") {
        						if(!trigger.getParent().targets?.length) return 0;
        						...
        						const evt = history[history.length - 1];
        						if(!evt.getParent().targets?.length) return 0;
        						if (evt && evt.getParent().targets[0] == target) {
        							if (get.color(card) != evt.color) return 114514 * result * attitude;
        						}*/
        						var Starget, Sjudge;
                				if(player.storage?.twzongquan?.target == target) {
                    			    var last=player.storage.twzongquan;
                    			    Sjudge = last.judge;
                    			    if(get.color(card) != Sjudge) return 114514 * result * attitude;
                    			}
        					}
        					return result * attitude;
        				});
        			'step 1'
        			if (!result.bool || !result.links || !result.links.length) event.finish();
        			'step 2'
        			event.cards.removeArray(result.links);
        			player.respond(result.links, "twguimou", "highlight", "noOrdering");
        			...
        			next.set("processAI", function (list) {
        				const player = get.player(),
        					target = _status.event.getTrigger().name == "phaseZhunbei" ? player : player.next;
        			...
        			game.cardsGotoPile(top, ["top_cards", top], (event, card) => {
        	...
        	twmiewei: {
        		...
        		subSkill: {
        			damage: {
        				...
        				content() {
        					/*const num = game
        						.getGlobalHistory("every个thing", evt => {
    						...
    						let targets = [];
    						game.players.concat(game.dead).forEach(source=>{
    						    source.getHistory('useCard').filter(evt=>(evt.card?.name=='sha'&&evt.targets?.length)).forEach(evt=>{
    						        targets.addArray(evt.targets);
    						    });
    						});
    						const num = targets.unique().length;
		    ...
		    twchihui: {
				audio: 2,
				audioname: ['huan_fantasy_caoang'],
				trigger: {
					global: 'phaseBegin'
				},
				filter: function (event, player) {
					return event.player != player && player.countDisabled() < 5;
				},
				logTarget: 'player',
				content: function () {
					'step 0'
					var target = trigger.player, equips = Array.from({ length: 5 }).map((_, i) => [i + 1, get.translation(`equip${i + 1}`)]).filter(i => !player.isDisabled(`equip${i[0]}`));
					player.chooseButton(2, ["炽灰：请选择你要废除的装备栏和相应操作", '<div class="text center">即将废除的装备栏</div>', [equips, "tdnodes"], `<div class="text center">对${get.translation(target)}执行的操作</div>`, [[["discard", `弃置其牌`], ["equip", `置入装备牌`]], "tdnodes"]])
						.set("filterButton", button => {
							var link = button.link, player = _status.event.player, target = _status.event.target;
						...
						.set("target", target)
						.set("list", equips.map(i => i[0]));
					'step 1'
					if (result.bool) {
						var slot = result.links[0], target = trigger.player;
						player.disableEquip(slot);
						if (result.links[1] == 'discard') {
							if (target.countDiscardableCards(player, 'hej')) player.discardPlayerCard(target, 'hej', true);
						} else {
							var equip = get.cardPile2(card => get.subtype(card) == 'equip' + slot);
							if (equip) {
								target.equip(equip);
								game.delayx();
							}
					...
					player.loseHp();
					'step 3'
					var num = Math.min(player.getDamagedHp(), 3);
			...
			twfuxi: {
				...
				forever: true,
				trigger: {
					player: ['dying', 'disableEquipAfter']
				},
				filter: function (event, player) {
					return event.name == 'dying' || player.countDisabled() >= 5;
				},
				direct: true,
				owner: 'huan_caoang',
				content: function () {
					'step 0'
					player.chooseButton([get.prompt('twfuxi'), [[
						["phase", "当前回合结束后执行一个额外的回合"],
						["twchihui", `保留〖炽灰〗直到下次退幻`],
						["draw", `摸牌至体力上限`],
						["enable", `恢复所有装备栏`],					
					], "textbutton"]]).set("filterButton", button => {
						var link = button.link, player = _status.event.player;
						if (link == "draw" && player.countCards("h") >= player.maxHp) return false;
						if (link == "enable" && player.countDisabled() < 5) return false;
						return true;
					}).set("ai", button => {
						var link = button.link, player = _status.event.player;
						var history = player.getAllHistory('useSkill', evt => evt.skill == 'twfuxi'),
							num = history[history.length - 1]?.twfuxi_num || 0;
					...
					'step 1'
					if (result.bool) {
						player.logSkill('twfuxi');
						var history = player.getAllHistory('useSkill', evt => evt.skill == event.name);
						if (history) {
							history[history.length - 1][event.name + '_num'] = result.links.length;
						}
						if (result.links.contains('phase')) {
							game.log(player, '选择了', '#y选项一');
							player.addTempSkill('twfuxi_mark');
							player.insertPhase();
						}
						if (result.links.contains('twchihui')) {
							game.log(player, '选择了', '#y选项二');
							event.twchihui = true;
						}
						if (result.links.contains('draw')) {
							game.log(player, '选择了', '#y选项三');
							player.drawTo(Math.min(player.maxHp, 5));
						}
						if (result.links.contains('enable')) {
					...
					'step 2'
					/*if (player.name1 == 'huan_caoang') {
					    player.node.avatar.setBackground('huan_fantasy_caoang', 'character');
					    player.name = 'huan_fantasy_caoang';
					    player.name1 = 'huan_fantasy_caoang';
					    player.node.name.innerHTML = get.slimName('huan_fantasy_caoang');
					} else if (player.name2 && player.name2 == 'huan_caoang') {
					    player.node.avatar2.setBackground('huan_fantasy_caoang', 'character');
					    player.name2 = 'huan_fantasy_caoang';
					    player.node.name2.innerHTML = get.slimName('huan_fantasy_caoang');
					};
					player.update();*/
					if(lib.config.extension_十周年UI_enable&&lib.config.effect_huan_caoang) {
				    	decadeUI.animation.playSpine({ name: 'SSHW_caoang_zise', speed: 1, scale: 0.9, x: [0, 0.5], y: [0, 0.5] });
				        decadeUI.animation.playSpine({ name: 'SSHW_ruhuan_ruhuan', speed: 1, scale: 1, x: [0, 0.5], y: [0, 0.5] });
				    }
					//player.changeAvatarTo('huan_caoang','huan_fantasy_caoang');
					player.changePlayerTo('huan_caoang','huan_fantasy_caoang');
        			//player.changeSkills(["twhunyou"], get.info("twhunyou").derivation);
					var skills = ['twchihui', 'twfuxi'];
					if (event.twchihui) skills.remove('twchihui');
					player.removeSkill(skills, false, false);
					player.addSkill(['twhuangzhu', 'twliyuan', 'twjifa'], true, true, false, false);
			...
			twhuangzhu: {
				...
				content: function () {
					'step 0'
					var storage = player.getStorage('twhuangzhu_effect')
						.filter(effect => !(player.storage.twhuangzhu_equip || []).some(vcard => vcard[2] === effect));
					player.chooseButton(['###煌烛：是否选择或替换至多两个牌名？###<div class="text center">你视为拥有选择牌名的技能</div>', [storage, "vcard"], '<div class="text center">选择令以下装备技能失效</div>', [player.storage.twhuangzhu_equip || [], 'vcard']])
						.set('filterButton', button => {
							var buttons = ui.selected.buttons, event = _status.event, effect = (event.player.storage.twhuangzhu_equip || []).map(vcard => get.subtype(vcard[2])), yes = event._args[0][1][0], no = event._args[0][3][0].map(vcard => vcard[2]), card = button.link[2];
							for (var i = 0; i < buttons.length; i++) {
								var name = get.name(buttons[i]);
								if (no.contains(name)) effect.remove(get.subtype(buttons[i]));
								else {
									effect.add(get.subtype(buttons[i]));
								}
							}
							if (yes.contains(card)) {
								if (!effect.contains(get.subtype(card)) && !buttons.contains(button)) return true;
								if (effect.contains(get.subtype(card)) && buttons.contains(button)) return true;
							}
							else {
								if (effect.contains(get.subtype(card)) && !buttons.contains(button)) return true;
						...
						.set('selectButton', () => {
							var buttons = ui.selected.buttons, event = _status.event, effect = (event.player.storage.twhuangzhu_equip || []).map(vcard => get.subtype(vcard[2])), yes = event._args[0][1][0], no = event._args[0][3][0].map(vcard => vcard[2]), max = 1, top = 0, buttom = 0;
							for (var i = 0; i < buttons.length; i++) {
								var name = get.name(buttons[i]);
								if (yes.contains(name)) top++;
								else buttom++;
							}
							if (buttom == 0) max = 2;
							else if (buttom == 1) max = 3;
							else max = 4;
							return [1, max];
						})
						.set('complexButton', true)
						.set("ai", button => get.equipValue({ name: button.link[2] }, _status.event.player));
					'step 1'
					if (result.bool) {
						player.logSkill('twhuangzhu');
						player.addSkill('twhuangzhu_equip');
						var storage = player.getStorage('twhuangzhu_equip');
						var both = result.links.filter(item => storage.includes(item));//要移除的
						var newe = result.links.filter(item => !player.getStorage('twhuangzhu_equip').includes(item));//要添加的
						player.storage.twhuangzhu_equip = storage.filter(item => !both.some(r => JSON.stringify(r) === JSON.stringify(item)));
						player.markAuto('twhuangzhu_equip', newe);
						player.removeAdditionalSkill('twhuangzhu_equip');
						var addSkills = [];
						player.getStorage('twhuangzhu_equip').forEach(vcard => {
						    var name = vcard[2];
						    var skills = lib.card[name]?.skills;
						    if(!skills) return;
						    var skName = 'twhuangzhu_equip_'+name;
						    lib.skill[skName] = {
						        group: skills,
						        forced: true,
						    };
						    lib.translate[skName] = get.translation(name);
						    lib.translate[skName+'_info'] = get.translation(name+'_info');
						    addSkills.add(skName);
						});
						//player.getStorage('twhuangzhu_equip').map(vcard => lib.card[vcard[2]]?.skills || []).flat()
						player.addAdditionalSkill('twhuangzhu_equip', addSkills);
					}
				},
				group: 'twhuangzhu_record',
				subSkill: {
					record: {
						audio: 'twhuangzhu',
						trigger: {
						...
						content: function () {
							'step 0'
							var list = Array.from({ length: 5 }).map((_, i) => `equip${i}`).filter(i => player.isDisabled(i)).concat(["cancel2"]);
							player.chooseControl(list)
								.set("prompt", "煌烛：选择一个已废除装备栏的类别")
								.set("prompt2", "从牌堆或弃牌堆中随机获得一张对应副类别的装备牌，并记录其牌名")
								.set("ai", () => {
									return _status.event.controls.filter(i => i !== "cancel2").randomGet();
								});
							'step 1'
							if (result.control != 'cancel2') {
								player.logSkill('twhuangzhu_record');
								var equip = get.cardPile(card => get.subtype(card) == result.control);
								if (!equip) {
									equip = get.cardPile(card => get.type(card) == 'equip');
								}
								if (equip) {
									player.gain(equip, 'gain2');
									game.delayx();
									player.addSkill('twhuangzhu_effect');
									player.markAuto('twhuangzhu_effect', [get.name(equip)]);
								}
							}
						},
					},
					...
					equip: {
						trigger: {
							player: 'enableEquipEnd'
						},
						charlotte: true,
						forced: true,
						popup: false,
						filter: function (event, player) {
							if (!event.pos?.length) return false;
							return player.getStorage('twhuangzhu_equip').some(name => event.pos.includes(get.subtype(name)));
						},
						onremove: function (player, skill) {
							player.removeAdditionalSkill(skill);
						},
						content: function () {
							player.unmarkAuto('twhuangzhu_equip', player.getStorage('twhuangzhu_equip').filter(name => trigger.pos.includes(get.subtype(name))));
							if (!player.getStorage('twhuangzhu_equip').length) player.removeSkill('twhuangzhu_equip');
						},
						mod: {
							globalFrom: function (from, to, distance) {
			...
			twliyuan: {
				...
				enable: ['chooseToUse', 'chooseToRespond'],
				filter: function (event, player) {
					return player.countCards('hes', card => player.isDisabled(get.subtype(card)||'none'));
				},
				filterCard: function (card, player) {
					return player.isDisabled(get.subtype(card)||'none');
				},
				locked: false,
				position: 'hes',
				viewAs: {
			...
			twjifa: {
				...
				content: function () {
					'step 0'
					var history = player.getAllHistory('useSkill', evt => evt.skill == 'twfuxi');
					var num = history[history.length - 1]?.twfuxi_num;
					if (num > 0) player.loseMaxHp(num);
					'step 1'
					player.chooseControl(['twhuangzhu', 'twliyuan']).set('prompt', '选择保留的技能').set('ai', () => {
						return _status.event.controls.randomGet();
					});
					'step 2'
					/*if (player.name1 == 'huan_fantasy_caoang') {
					    player.node.avatar.setBackground('huan_caoang', 'character');
					    game.qhly_changeDynamicSkin(player.name, '经典形象');				    
					    player.name = 'huan_caoang';
					    player.name1 = 'huan_caoang';
					    player.node.name.innerHTML = get.slimName('huan_caoang');
					} else if (player.name2 && player.name2 == 'huan_fantasy_caoang') {
					    player.node.avatar2.setBackground('huan_caoang', 'character');
					    player.name2 = 'huan_caoang';
					    player.node.name2.innerHTML = get.slimName('huan_caoang');
					};
					player.update();*/
					if(lib.config.extension_十周年UI_enable&&lib.config.effect_huan_caoang) {
        				decadeUI.animation.playSpine({ name: 'SSHW_caoang_huangse', speed: 1, scale: 1, x: [0, 0.5], y: [0, 0.5] });
        			}
					//player.changeAvatarTo('huan_caoang','huan_fantasy_caoang',false);
					player.changePlayerTo('huan_fantasy_caoang','huan_caoang');
					player.removeSkill(['twhuangzhu', 'twliyuan', 'twjifa'].remove(result.control), false, false);			
					player.addSkill(['twchihui', 'twfuxi'], true, true, false, false);
					player.recover(player.maxHp - player.hp);
		...
		dynamicTranslate:{
			eu_zhitong:function(player){
			    if(player.storage.eu_zhitong) return '转换技，当你使用牌时，若目标包含，阳：自己，摸两张牌且回复1点体力；<span class="bluetext">阴：其他角色，你获得其装备区所有牌并对其造成1点伤害。</span>';
        	    return '转换技，当你使用牌时，若目标包含，<span class="bluetext">阳：自己，摸两张牌且回复1点体力；</span>阴：其他角色，你获得其装备区所有牌并对其造成1点伤害。';
			},
			twbeiding:function(player){
				if(player.storage.isInHuan) return "你使用〖北定〗记录的牌无距离限制且不计入次数；当你使用〖北定〗记录的牌结算结束后，你摸一张牌，然后从〖北定〗记录中移除此牌名。";
				return "每名角色的准备阶段，你可以声明并记录至多X个未记录的基本牌或普通锦囊牌牌名（X为你的体力值）。当前回合角色弃牌阶段结束时，你视为依次使用本回合声明的牌（无距离限制），若此牌的目标不包含其，其摸一张牌。";
			},
			twjielv:function(player){
				if(player.storage.isInHuan) return "锁定技，当你减少1点体力上限后，你回复1点体力。";
				return "锁定技。①每名角色回合结束时，若你未对其使用牌，你失去1点体力。②当你失去1点体力或受到1点伤害后，若你的体力上限小于7，你增加1点体力上限。";
			},
			...
			twfeifu: function (player) {
				if(player.storage.twfeifu) return '锁定技，转换技。阳：当你成为【杀】的唯一目标后；<span class="bluetext">阴：当你使用【杀】指定唯一目标后；目标角色须交给使用者一张牌。若此牌为装备牌，则使用者可使用此牌。</span>';
			    return '锁定技，转换技。<span class="bluetext">阳：当你成为【杀】的唯一目标后；</span>阴：当你使用【杀】指定唯一目标后；目标角色须交给使用者一张牌。若此牌为装备牌，则使用者可使用此牌。';
			},
		},
		translate:{
			tw_beimihu:'TW卑弥呼',
        	...
        	huan_caoang: '幻曹昂',
			twchihui: '炽灰',
			twchihui_info: '其他角色的回合开始时，你可以废除一个装备栏并选择一项：1.弃置其区域内的一张牌；2.将牌堆中一张对应副类别的装备牌置入其装备区。若如此做，你失去1点体力，然后摸X张牌（X为你已损失的体力值且至多为3）。',
			twfuxi: '赴曦',
			twfuxi_info: '持恒技，当你进入濒死状态时，或你的装备栏均被废除后，你可以选择并依次执行至多两项：1.于当前回合结束时获得一个额外的回合；2.此次入幻时保留〖炽灰〗；3.将手牌摸至体力上限（至多摸至五张）；4.恢复所有装备栏（你的装备栏均被废除时方可选择此项）。然后你入幻。',
			huan_fantasy_caoang: '幻曹昂',
			twhuangzhu: '煌烛',
			twhuangzhu_info: '准备阶段，你可以选择一个已被废除的装备栏，从牌堆或弃牌堆中随机获得一张对应副类别的装备牌（若无则随机获得一张装备牌），并记录此牌牌名。出牌阶段开始时，你可以选择或变更至多两个已记录且对应装备栏已被废除的装备牌牌名（每种副类别限一个），视为拥有这些装备牌的技能直到此装备栏恢复。',
			twliyuan: '离渊',
			twliyuan_info: '你可以将一张对应装备栏已被废除的装备牌当无距离和次数限制且不计入次数的【杀】使用或打出。当你以此法使用或打出牌时，你摸两张牌。',
			twjifa: '冀筏',
			twjifa_info: '锁定技，当你进入濒死状态时，你减X点体力上限（X为你上次发动〖赴曦〗选择的项数），选择此次“退幻”时保留〖煌烛〗或〖离渊〗，然后退幻。',
