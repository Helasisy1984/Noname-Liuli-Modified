	...
	return {
		...
		characterSort:{
			offline:{
				...
				offline_piracyE:['zombie_jiaxu','zombie_zombie','zhulu_jiaxu','pe_que','pe_wangyun','pe_zhonghui','pe_sunchen','pe_mengda','pe_wenqin','ns_caoanmin','jiangfei','chendong','jiangqing','kongrong','jiling','tianfeng','mateng'],
				...
				offline_shixin:['sxrm_caocao','sxrm_guanyu','sxrm_guanyu_rende','sxrm_guanyu_qingnang','sxrm_guanyu_longyin'],
				//offline_others:[""],
			},
		},
		character:{
			sxrm_guanyu_rende: ["male","devil",2,["sxrmrende"],["unseen"]],
			sxrm_guanyu_qingnang: ["male","devil",2,["sxrmqingnang"],["unseen"]],
			sxrm_guanyu_longyin: ["male","devil",2,["sxrmlongyin"],["unseen"]],
			sxrm_guanyu: ["male","devil", 5,["sxrmhanguo","sxrmweiwo"],['shu']],
    		sxrm_caocao: ["male","devil", 3,["sxrmsigu","sxrmkuxin"],['wei']],
			...
			zhulu_jiaxu:['male','shen',3,['weimu','zybishi','zyjianbing'],['qun']],
		    ...
		    zombie_jiaxu: ["male", "shen", 3, ["zombiesangluan", "zombieshibao", "zombiechuce", "zombielongmu"],['qun']],
            zombie_zombie: ["male", "qun", "2/4", ["zombieshibian", "zombieganran"], ['unseen']],
            pe_que: ["male", "qun", 4, ["peyingzhen", "peyuanjue", "peaoyong"]],
        },
		characterIntro:{
		...
		characterTitle:{
			sxrm_guanyu:'#y四海仰鼻息',
			sxrm_caocao:'#y一目窥九州',
			jsp_liubei: '#y乱世的枭雄',//S1019
			ns_caoanmin: '#y瓢取祸水',//S1023
			longyufei: '#y将魂之星',//S1044
			ps1059_guojia: '#y破晓逐辰',//S1059
			ps_lvbu: '#y虓虎之勇',//S1061
			ps1062_zhouyu: '#y盖世之才',//S1062
			ps_jiaxu: '#y控魂驱魄',//S1066
			ps_jin_simayi: '#y通达权变', //S1067储备皮肤没有定称号，暂且用晋司马懿原皮作称号
			ps_guanyu: '#y飞龙在天',//S2065
			ps2063_zhaoyun: '#y寒光凛然',//S2063
			ps2066_zhugeliang: '#y谋定天下',//S2066
			ps2067_zhaoyun: '#y战场游龙',//S2067
			ps2068_simayi: '#y谋定天下',//S2068
			ps_machao: '#y一马当先',//S2069
			ps2070_guojia: '#y谋定天下',//S2070
			ps_simayi: '#y狼顾鬼才',//S2073
			ps_zhugeliang: '#y三分天下',//S2073
			ps_caopi: '#y帝业霸途',//S2075
			ns_jiaxu: '#y乱武天下',//S2079
			ps2080_zhouyu: '#y舞剑助兴',//S2080
			ps_caozhi: '#y洛阳感怀',//S2081
			ps_shen_machao: '#y迅骛惊雷',//SX015
			sp_xiahoushi: '#y疾冲之恋',
			jsp_zhaoyun: '#y常山的游龙',
			huangjinleishi: '#y雷祭之姝',
			sp_pangtong: '#y荆楚之高俊',
			sp_daqiao: '#y韶光易逝',
			sp_ganning: '#y怀铃的乌羽',
			sp_xiahoudun: '#y啖睛的苍狼',
		...
		skill:{
		    ...
		    sxrmkuxin: {
        		...
        		content:function(){
        			...
        			target
        				...
        				.set("ai", card => {
        					...
        					if (att < 0) {
        						...
        						val = get.value(card, _status.event.targetx) - val;
        					}
        					return val;
        				});
        			'step 2'
        			if (result.bool) {
        				event.showcards.addArray(result.cards);
        			...
        			player
        				...
        				.set("ai", () => {
        					if (_status.event.player.isTurnedOver() && _status.event.cards.some(card => get.suit(card, false) === "heart")) {
        						return 1;
        					}
        					if (_status.event.cards.length <= _status.event.num && game.hasPlayer(current => current != _status.event.player && current.countCards("h", cardx => !_status.event.cards.includes(cardx)) > _status.event.cards.length && get.attitude(_status.event.player, current) < 0)) {
        						return 1;
        					}
        					if (_status.event.cards.length <= _status.event.num) {
        						return get.rand(0, 1);
        					}
        					if (_status.event.cards.some(card => get.suit(card, false) === "heart")) {
        						return 0;
        					}
        					return 1;
        				})
        				.set("cards", event.showcards)
        			...
        			game.log(player, "选择了", "#g【枯心】", "的", "#y" + result.control);
        			if (result.control == "获得一名角色的未展示牌") {
        				player
        					.chooseTarget("枯心：选择一名其他角色获得其未展示的手牌", true, lib.filter.notMe)
        					.set("ai", target => {
        						return -get.attitude(_status.event.player, target) * target.countCards("h", cardx => !_status.event.cards.includes(cardx));
        					})
        					.set("cards", event.showcards);
        			} else {
        				event.gaincards = event.showcards;
        				event.goto(7);
        			}
        			'step 6'
        			event.gaincards = result?.targets?.[0].getCards("h", cardx => !event.showcards.includes(cardx));
        			'step 7'
        			if (event.gaincards.length) {
        				player.gain(event.gaincards, "give");
        				player.showCards(event.gaincards);
        			}
        			if (!event.gaincards.some(card => get.suit(card, false) === "heart")) {
        				player.chat("孩子们，一张牌都拿不到力");
        				if (event.gaincards.length) {
        					player.discard(event.gaincards);
        				}
        				player.turnOver();
        			} else {
        				player.chat("保持富态");
        	...
        	sxrmsigu: {
        		...
        		content:function(){
        			...
        			event.changedName = name;
        			if (name) {
        				target.addAdditionalSkill(mark, [skill], true);
        				//target.setAvatar(target.name, name);
        				target.changeAvatarTo(target.name, event.changedName, true);
        			} else {
        				player.chat("孩子你是谁？");
        			}
        			'step 2'
        			...
        			target.damage();
        			'step 4'
        			if (get.info(event.name).pasts[result.number - 1]) {
        				target.removeAdditionalSkill(`desigu_${player.playerid}`);
        				//target.setAvatar(target.name, target.name);
        				target.changeAvatarTo(target.name, event.changedName, false);
        			}
        		},
        		ai: {
        			order: 1,
        			result: {
        				target(player, target) {
        					...
        					if (get.attitude(player, target) < 0 && target.hp + target.hujia <= 1) {
        						return -1;
        					}
        					if ((target.hasSkillTag("maixie") || target.hasSkillTag("maixie_hp")) && get.attitude(player, target) > 0 && target.hp + target.hujia >= 3) {
        						return 2;
        					}
        					if (get.attitude(player, target) > 0 && target.hp + target.hujia >= 4) {
        						return 1;
        					}
        					if (get.attitude(player, target) == 0 && target.hp + target.hujia >= 2) {
        						return 1;
        					}
        					return 0;
        				},
        		...
        		pasts: ["chengong", "re_xiahoudun", "re_simayi", "re_guojia", "re_xunyu", "re_caopi", "shenpei", "re_caochong", "re_xunyou", "yangxiu", "chengyu", "xizhicai", "tw_shen_guanyu"],
        		derivation: ["sxrmzhichi", "sxrmreganglie", "sxrmrefankui", "sxrmnew_reyiji", "sxrmoljieming", "sxrmfangzhu", "sxrmshibei", "sxrmrechengxiang", "sxrmzhiyu", "sxrmjilei", "sxrmbenyu", "sxrmchouce", "sxrmnew_wuhun"],
        		//原本的对应表
        		source: {
        		    "sxrmzhichi": "chengong",
        		    "sxrmreganglie": "re_xiahoudun",
        		    "sxrmrefankui": "re_simayi",
        		    "sxrmnew_reyiji": "re_guojia",
        		    "sxrmoljieming": "ol_xunyu",
        		    "sxrmfangzhu": "sb_caopi",
        		    "sxrmshibei": "shenpei",
        		    "sxrmrechengxiang": "re_caochong",
        		    "sxrmzhiyu": "re_xunyou",
        		    "sxrmjilei": "yangxiu",
        		    "sxrmbenyu": "chengyu",
        		    "sxrmchouce": "xizhicai",
        		    "sxrmnew_wuhun": "shen_guanyu",
        		},
        	},
        	//小技能
        	sxrmnew_wuhun: {
                audio: true,
                group:["sxrmnew_wuhun_mark","sxrmnew_wuhun_die","wuhun22","wuhun23"],
                trigger:{
                    player:"damageEnd",
                },
                forced:true,
                ...
                content:function() {
                    trigger.source.addMark('sxrmnew_wuhun_mark', trigger.num);
                },
                subSkill:{
                    die:{
                        audio:"sxrmnew_wuhun",
                        ...
                        filter:function(event, player) {
                            return game.hasPlayer(function(current) {
                                return current != player && current.hasMark('sxrmnew_wuhun_mark');
                            });
                        },
                        content:function() {
                            "step 0"
                            ...
                            for (var i = 0; i < game.players.length; i++) {
                                ...
                                if (current != player && current.countMark('sxrmnew_wuhun_mark') > num) {
                                    num = current.countMark('sxrmnew_wuhun_mark');
                                }
                            }
                            player.chooseTarget(true, '请选择【武魂】的目标', function(card, player, target) {
                                return target != player && target.countMark('sxrmnew_wuhun_mark') == _status.event.num;
                            })
        	...
        	sxrmchouce: {
                ...
                content:function() {
                    ...
                    'step 4'
                    if (--event.num > 0) {
                        player.chooseBool(get.prompt2('sxrmchouce'));
                    } else {
                        event.finish();
                    }
                    'step 5'
                    if (result.bool) {
                        player.logSkill('sxrmchouce');
                        event.goto(1);
                    }
                },
                ai:{
        	...
        	sxrmbenyu: {
                ...
                content:function(){
                    ...
                    var eh=player.countCards('he',function(card){
                        return lib.filter.cardDiscardable(card,player,'sxrmbenyu');
                    });
                    ...
                    if(bool1&&bool2){
                        event.chosen=true;
                        player.chooseControl('cancel2').set('prompt',get.prompt('sxrmbenyu',trigger.source)).set('choiceList',[
                    ...
                    else player.chooseBool(get.prompt('sxrmbenyu',trigger.source),'将手牌摸至'+get.cnNumber(Math.min(trigger.source.countCards('h'),5))+'张');
                    "step 2"
                    if(result.bool){
                        player.logSkill('sxrmbenyu',trigger.source);
                    ...
                    if(event.chosen){
                        ...
                        args.push(get.prompt('sxrmbenyu',trigger.source));
                        args.push('弃置'+get.cnNumber(num)+'张牌并对其造成1点伤害');
                    }
                    var next=player.chooseToDiscard.apply(player,args);
                    if(!event.chosen) next.logSkill=['sxrmbenyu',trigger.source];
                ...
                content:function() {
                    ...
                    player.chooseControl('basic', 'trick', 'equip', 'cancel2', function() {
                        ...
                        var list = ['basic', 'trick', 'equip'].filter(function(name) {
                            return (!source.storage.sxrmjilei2 || !source.storage.sxrmjilei2.contains(name));
                        });
                        ...
                        .set('prompt', get.prompt2('sxrmjilei', trigger.source))
                        .set('source', trigger.source);
                    'step 1'
                    if (result.control != 'cancel2') {
                        player.logSkill('sxrmjilei', trigger.source);
                        player.popup(get.translation(result.control) + '牌');
                        trigger.source.addTempSkill('sxrmjilei2', {
                            player: 'phaseBegin'
                        });
                        trigger.source.storage.sxrmjilei2.add(result.control);
                        trigger.source.updateMarks('sxrmjilei2');
                        if (!trigger.source.storage.sxrmjilei3) trigger.source.storage.sxrmjilei3 = [];
                        var str = '';
                        if (result.control == 'trick') str = '锦';
                        else if (result.control == 'basic') str = '基';
                        else if (result.control == 'equip') str = '装';
                        var num = '';
                        if (trigger.source.storage.sxrmjilei3.indexOf(str) == -1) trigger.source.storage.sxrmjilei3 += str;
                        if (trigger.source.storage.sxrmjilei3.indexOf('基') != -1) num += '基';
                        if (trigger.source.storage.sxrmjilei3.indexOf('锦') != -1) num += '锦';
                        if (trigger.source.storage.sxrmjilei3.indexOf('装') != -1) num += '装';
                        trigger.source.addTempSkill('sxrmjilei2_mark', {
                            player: 'phaseBegin'
                        });
                        trigger.source.storage.sxrmjilei2_mark = '';
                        trigger.source.addMark('sxrmjilei2_mark', num);
                    }
                },
                ai:{
                    "maixie_defend":true,
            ...
            sxrmjilei2: {
                ...
                onremove:function(player) {
                    player.unmarkSkill('sxrmjilei2_mark');
                    delete player.storage.sxrmjilei2_mark;
                    delete player.storage.sxrmjilei2;
                    delete player.storage.sxrmjilei3;
                    delete player.storage.gzsxrmjilei3;
                },
                mod:{
                    cardDiscardable:function(card, player) {
                        if (player.storage.sxrmjilei2.contains(get.type(card, 'trick'))) return false;
                    },
        	...
        	sxrmrechengxiang: {
                audio: true,
                group:"sxrmrechengxiang_gain",
                trigger:{
                    player:"sxrmrechengxiang_gainEnd",
                },
                direct:true,
                subfrequent:["gain"],
                filter:function(event,player){
                ...
                subSkill:{
                    gain:{
                        inherit:"chengxiang",
                        audio:"sxrmrechengxiang",
                        sub:true,
                        parentskill:"sxrmrechengxiang",
                        frequent:true,
                        trigger:{
                            player:"damageEnd",
                        },
                ...
                ai:{
                    ...
                    effect:{
                        target:function(card,player,target){
                            ...
                            if(player._sxrmshibei_tmp) return;
                            if(target.hasSkill('sxrmshibei_ai')) return;
	        ...
	        sxrmfangzhu: {
                ...
                content:function(){
                    ...
                    player.chooseTarget(get.prompt('sxrmfangzhu'),'令一名其他角色翻面'+(draw>0?'并摸'+get.cnNumber(draw)+'张牌':''),function(card,player,target){
                        return player!=target
                    }).setHiddenSkill('sxrmfangzhu').set('ai',target=>{
                    ...
                    "step 1"
                    if(result.bool){
                        player.logSkill('sxrmfangzhu',result.targets);
                        var draw=player.getDamagedHp();
            ...
            sxrmoljieming: {
                ...
                content:function(){
                    ...
                    'step 1'
                    event.count--;
                    player.chooseTarget(get.prompt2('sxrmoljieming'),function(card,player,target){
                    ...
                    if(result.bool){
                        var target=result.targets[0];
                        event.target=target;
                        player.logSkill('sxrmoljieming',target);
                    ...
                    'step 4'
                    if(event.count>0&&player.isIn()&&player.hasSkill('sxrmoljieming')) event.goto(1);
                },
                ai:{
                    expose:0.2,
                    maixie:true,
        	...
        	sxrmnew_reyiji: {
                ...
                content:function (){
                    ...
                    'step 5'
                    if(event.count>0&&player.hasSkill('sxrmnew_yiji')){
                        player.chooseBool(get.prompt2('sxrmnew_yiji'));
                    }
                    else event.finish();
                    'step 6'
                    if(result.bool){
                        player.logSkill('sxrmnew_yiji');
                        event.goto(1);
                    }
                },
                ai:{
        	...
        	sxrmreganglie: {
                ...
                content:function(){
                    ...
                    event.num--;
                    if(event.num>0&&player.hasSkill('sxrmreganglie')){
                        player.chooseBool(get.prompt2('sxrmreganglie'));
                    }
                    else{
                        event.finish();
                    }
                    "step 3"
                    if(result.bool){
                        player.logSkill('sxrmreganglie',trigger.source);
                        event.goto(1);
                    }
                },
                ai:{
        	...
        	sxrmzhichi: {
                ...
                content:function(){
                    player.addTempSkill('sxrmzhichi2',['phaseAfter','phaseBefore']);
                },
                "_priority":0,
            },
            sxrmzhichi2: {
		    ...
		    sxrmhanguo: {
                ...
                content: function () {
                    ...
                    if (result.bool) {
                        ...
                        target.addTempSkill('sxrmhujia', 'roundStart');
                ...
                ai:{
                    effect:{
                        player:function(card,player,target,current,isLink){
                            if(card.name=='sha'&&!isLink&&player.storage.sxrmhanguo?.contains(target)&&!target.hasSkillTag('filterDamage',null,{
                                player:player,
                                card:card,
                            })) return [1,0,1,-3];
                        },
            ...
            sxrmweiwo: {
                ...
                derivation: ['sxrmrende', 'sxrmqingnang', 'sxrmlongyin'],
                audio: 2,
                content: function() {
                    'step 0'
                    player.awakenSkill('sxrmweiwo');
                    event.skillList = ['rende', 'qingnang', 'longyin'];
                    event.prefix = 'sxrm_guanyu_';
                    event.allList = event.skillList.map(s=>event.prefix+s);
                    event.choList = event.allList.slice(0);
                    event.targetList = [];
                    event.choice = false;
                    'step 1'
                    if (event.choList.length < 1) event.goto(5);
                    'step 2'
                    event.choice=false;
                    player.chooseButton(ui.create.dialog('选择一名其他角色令其获得一个技能',[event.allList,'character'])).set('filterButton',function(button){
                        if(event.choList.contains(button.link)) {
                    ...
                    if(!event.isMine()){
                        result={
                            bool:game.hasPlayer(target=>get.attitude(player,target)>0&&player != target && !event.targetList.contains(target)),
                            links:[event.choList.randomGet()],
                        }
                    };
                    ...
                    if(result.bool && event.choice) {
                        ...
                        event.target.addSkillLog('sxrm'+event.choice.slice(event.prefix.length));
                        ...
                        event.choList.remove(event.choice);
            ...
            sxrmrende: {
                ...
                check: function (card) {
                    ...
                    var num = 0;
                    var ext = get.suit(card) == 'heart' ? 2 : 0;
                    ...
                    if (player.hp == player.maxHp || num > 1 || player.countCards('h') <= 1) {
                        ...
                        for (var i = 0; i < players.length; i++) {
                            if (players[i].hasSkill('haoshi') &&
                                ...
                                return 11 + ext - get.value(card);
                            }
                        }
                        if (player.countCards('h') > player.hp) return 10 + ext - get.value(card);
                        if (player.countCards('h') > 2) return 6 + ext - get.value(card);
                        return -1;
                    }
                    return 10 + ext - get.value(card);
                },
                content: function () {
            ...
            sxrmlongyin: {
                ...
                content: function () {
                    ...
                    var next = player.chooseToDiscard(get.prompt('sxrmlongyin'), '弃置一张牌' + (get.color(trigger.card) == 'red' ? '并摸一张牌' : '') + '，令' + get.translation(trigger.player) + '本次使用的【杀】不计入使用次数', 'he');
                    next.logSkill = ['sxrmlongyin', trigger.player];
                    next.set('ai', function (card) {
                        if (_status.event.go) {
        	...
        	peyingzhen: {
        		...
        		filter(event, player) {
        			...
        			if (event.skill == 'peyingzhen') return false;
        		...
        		content() {
				    ...
				    if(result.bool && result.targets) {
				        ...
				        player.logSkill('peyingzhen', result.targets);
				    }else {
				        event.finish();
				    }
				    'step 2'
        	...
        	peaoyong: {
        		...
        		content() {
        			...
        			player
        				...
        				.set("prompt", get.prompt('peaoyong'));
    				'step 1'
    				if(result.control != "cancel2") {
    				    event.control = result.control;
    				    player.logSkill('peaoyong');
    				}
    				'step 2'
        			if (event.control == "背水！") {
        			...
        			'step 4'
        			if (event.control == "选项三" || event.control == "背水！") {
        				player.chooseToUse({
        	...
        	petongkai: {
        		...
        		content() {
        			...
        			'step 2'
        			//player.chooseToGive(true, "he", trigger.target).set("visibleMove", true);
                    player.chooseCard('he','交给'+get.translation(trigger.target)+'一张牌',1,true).set('ai',function(card){
                        var val = get.value(card);
                        if(get.attitude(player,trigger.target)<1) return -val;
                        if(['wanjian','sha'].contains(trigger.card.name) && card.name=='shan') return 2 * val;
                        if(['juedou','nanman'].contains(trigger.card.name) && card.name=='sha') return 2 * val;
                        if(get.subtype(card)=='equip2' && trigger.target.isEmpty(2)) return 1.5 * val;
                        if(trigger.target.getHp() <= 1 && card.name == 'jiu') return 1.6 * val;
                        if(trigger.target.getHp() <= 2 && card.name == 'jiu') return 1.3 * val;
                        return -val;
                    });
        			'step 3'
        	...
        	zombiesangluan: {
        		...
        		content() {
    				...
    				if(result.bool) {
    				    event.targets = result.targets;
    				    player.logSkill('zombiesangluan', event.targets);
    				}else {
    				    event.finish();
    				}
    				'step 2'
        			...
        			event.source.chooseToUse(function (card, player, event) {
        					if (get.name(card) !== "sha") {
        						return false;
        					}
        					return lib.filter.filterCard.apply(this, arguments);
        				}, get.translation(event.name) + "：对" + get.translation(event.target) + "使用一张【杀】，或失去1点体力且" + get.translation(player) + "回复1点体力")
        	...
        	zombieshibao: {
        		...
        		content() {
        			'step 0'
        			if (target.getHp() > 0) {
        				event.targets = game.filterPlayer(current => [current.getPrevious(), current.getNext()].includes(target));
        				target.loseHp(target.getHp());
        				if(event.targets.length) player.line(event.targets);
        			}else {
        			    event.finish();
        			}
        			'step 1'
        			...
        			kanpo: {
        				...
        				content() {
        					...
        					if (get.type(card) !== "delay" && player.hasUseTarget(card)) {
        						player.chooseUseTarget(card, null, false);
        					}
        				},
        			},
        		},
        	...
        	zombielongmu: {
        		...
        		content() {
        			...
        			'step 1'
    				const names = get.nameList(event.target).filter(i => i !== "zombie_zombie").unique();
    				if(names.length > 1) {
    					player
							.chooseControl(names)
							.set("ai", () => {
    					...
    					event._result = { control: names[0] };
					}
					'step 2'
    				if (result.control) {
    					game.broadcastAll(player => player.revive(2), event.target);
    					let doubleDraw = false;
    					let num = (get.infoMaxHp(get.character("zombie_zombie")[2]) || get.infoHp(get.character("zombie_zombie")[2])) - (get.infoMaxHp(get.character(result.control)[2]) || get.infoHp(get.character(result.control)[2]));
    					if (num !== 0) {
    					...
    					event.doubleDraw = doubleDraw;
    					event.target.reinitCharacter(result.control, "zombie_zombie");
    				}else {
    				    event.finish();
    				}
    				'step 3'
    				if (event.doubleDraw) {
						event.target.doubleDraw();
					}
        		},
        	...
        	zombieshibian: {
        		...
        		forced: true,
        		content() {
        			let playerZ = trigger.getParent().player,
        			    skills = trigger.getParent().skills;
        			playerZ = playerZ["zombieshibian"] || playerZ;
        			let target = player;
        			if (skills.length) {
        			...
        			game.broadcastAll(
        				(player, target) => {
        					...
        					if (target.node.dieidentity?.innerHTML?.length) {
        	...
        	zombieganran: {
        		trigger: { player: "phaseJieshuBegin" },
        		filter(event, player) {
        			 event.targets = game.dead.filter(target => {
        				if (target["zombieshibian"] || get.is.playerNames(target, "zombie_zombie")) {
        					return false;
        				}
        				return player.getHistory()?.killPlayers?.contains(target);
        				//return game.getGlobalHistory("every个thing", evt => evt.name === "die" && evt.player === target && evt.source === player).length > 0;
        			}).sortBySeat();
        		...
        		content() {
        			'step 0'
        			event.allPlayers = trigger.targets.slice(0);
        			'step 1'
        			if(!event.allPlayers.length) event.finish();
        			'step 2'
        			event.target = event.allPlayers.shift();
					const names = get.nameList(event.target).filter(i => i !== "zombie_zombie").unique();
    				if(names.length > 1) {
    					player
							.chooseControl(names)
        		...
        		subSkill: {
        		    kill: {
        		        ...
        		        content() {
        		            ...
        		            if(!history.killPlayers) {
        		                history.killPlayers = [];
        		            }
        		            history.killPlayers.push(trigger.player);
        		        },
        		    },
        		},
        	},
			...
			yjzezhu:{
				...
				filter:function(event,player){
					...
					return zhu&&zhu.countGainableCards(player,zhu==player?'ej':'hej');
				},
				filterTarget:function(card,player,target){
					//var zhu=get.mode()=='identity'?get.zhu(player):game.filterPlayer(i=>i.getSeatNum()==1)[0];
					var zhu=get.zhu(player);
		...
		translate:{
            ...
            sxrmzhichi:"智迟",
            "sxrmzhichi_info":"锁定技，当你于回合外受到伤害后，所有【杀】或普通锦囊牌对你无效直到回合结束。",
            sxrmzhichi2:"智迟",
            "sxrmzhichi2_info":"智迟已发动。",
            sxrmreganglie:"刚烈",
            "sxrmreganglie_info":"当你受到1点伤害后，你可进行判定，若结果为：红色，你对伤害来源造成1点伤害；黑色，你弃置伤害来源一张牌。",
            sxrmrefankui:"反馈",
            "sxrmrefankui_info":"每当你受到1点伤害后，你可以获得伤害来源的一张牌。",
            "sxrmnew_reyiji":"遗计",
            "sxrmnew_reyiji_info":"当你受到1点伤害后，你可以摸两张牌，然后可以将至多两张手牌交给其他角色。",
            sxrmoljieming:"节命",
            "sxrmoljieming_info":"当你受到1点伤害后或死亡时，你可令一名角色摸X张牌。然后若其手牌数大于X，则其将手牌弃置至X张（X为其体力上限且至多为5）。",
            sxrmfangzhu:"放逐",
            "sxrmfangzhu_info":"当你受到伤害后，你可令一名其他角色摸X张牌（X为你已损失的体力值），然后该角色将武将牌翻面。",
            sxrmshibei:"矢北",
            "sxrmshibei_info":"锁定技，当你受到伤害后：若此伤害是你本回合第一次受到的伤害，则你回复1点体力；否则你失去1点体力。",
            sxrmrechengxiang:"称象",
            "sxrmrechengxiang_info":"当你受到伤害后，你可以亮出牌堆顶的四张牌。然后获得其中任意数量点数之和不大于13的牌。若你得到的牌点数之和为13，你复原武将牌。",
            sxrmzhiyu:"智愚",
            "sxrmzhiyu_info":"当你受到伤害后，你可以摸一张牌，然后展示所有手牌。若颜色均相同，你令伤害来源弃置一张手牌。",
            sxrmjilei:"鸡肋",
            "sxrmjilei_info":"当你受到有来源的伤害后，你可以声明一种牌的类别。若如此做，你令伤害来源不能使用、打出或弃置此类别的手牌直到其下个回合开始。",
            sxrmbenyu:"贲育",
            "sxrmbenyu_info":"当你受到伤害后，你可选择：①将手牌摸至与伤害来源手牌数相同（至多摸至5张）；②弃置大于伤害来源手牌数的牌，然后对其造成1点伤害。",
            sxrmchouce:"筹策",
            "sxrmchouce_info":"当你受到1点伤害后，你可以判定，若结果为：黑色，你弃置一名角色区域里的一张牌；红色，你选择一名角色，其摸一张牌，若其是〖先辅〗选择的角色，改为其摸两张牌。",
            "sxrmnew_wuhun":"武魂",
            "sxrmnew_wuhun_info":"锁定技，当你受到伤害后，伤害来源获得X个“梦魇”标记（X为伤害点数）。锁定技，当你死亡时，你选择一名“梦魇”标记数量最多的其他角色。该角色进行判定：若判定结果不为【桃】或【桃园结义】，则该角色死亡。",
            ...
            sxrm_guanyu_rende: '仁德',
            sxrm_guanyu_qingnang: '青囊',
            sxrm_guanyu_longyin: '龙吟',
