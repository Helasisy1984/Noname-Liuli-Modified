...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
						...
						return {
							...
							content:function(){
								...
								if(result.bool){
									var target=result.targets[0];
									player.line(target,'green');
									if(!target.storage.caozhao_infos) target.storage.caozhao_infos={};
									target.storage.caozhao_infos[cards[0].cardid]=lib.skill.caozhao_backup.cardname;
									target.addSkill('caozhao_infos');
									player.give(cards,target,'give').gaintag.add('caozhao');
								}
								else{
									if(!player.storage.caozhao_infos) player.storage.caozhao_infos={};
									player.storage.caozhao_infos[cards[0].cardid]=lib.skill.caozhao_backup.cardname;
									player.addGaintag(cards,'caozhao');
									player.addSkill('caozhao_infos');
								}
							},
							ai:{
								result:{
			...
			caozhao_infos:{
				...
				mod:{
					cardname:function(card,player){
						var map=player.storage.caozhao_infos;
		...
		characterTitle:{
    		...
    		jin_simashi: '#y晋景王',
    		jin_xiahouhui: '#y景怀皇后',
    		zhanghuyuechen: '#y不辱门庭',
    		shibao: '#y乐陵郡公',
    		jin_yanghuiyu: '#y景献皇后',
    		jin_simazhao: '#y晋文帝',
    		jin_wangyuanji: '#y文明皇后',
    		duyu: '#y文成武德',
    		weiguan: '#y兰陵郡公',
    		xuangongzhu: '#y晋宣公主',
    		zhongyan: '#y聪慧弘雅',
		...
		translate:{
			...
			qingleng:'清冷',
			qingleng_info:'一名角色的回合结束时，若其体力值与手牌数之和不小于X，则你可将一张牌当无距离限制的冰属性【杀】对其使用（X为牌堆数量的个位数）。若这是你本局游戏内首次对其发动过此技能，则你摸一张牌。',
			xuanmu:'宣穆',
			xuanmu2:'宣穆',
