...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
			...
			sangu:{
				...
				content:function(){
					...
					if(!list1.length&&!list2.length) event.finish();
						...
						player.chooseButton(dialog,true,[1,3]).set('filterButton',button=>{
    						if(ui.selected.buttons&&ui.selected.buttons.contains(button)) {
    							game.highLightCard(button,true,false);
    						}else {
		...
		translate:{
			...
			qingxian:'清弦',
			qingxian_info:'当你受到伤害/回复体力后，你可以令伤害来源/一名其他角色执行一项：失去1点体力，随机使用一张装备牌；回复1点体力，弃置一张装备牌。若其以此法使用或弃置的牌为梅花，你回复一点体力。',
			juexiang:'绝响',
