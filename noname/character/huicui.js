	...
	return {
		...
		characterSort:{
			huicui:{
				...
				sp_zhengyin:['yue_caiwenji','yue_daqiao','yue_xiaoqiao','yue_zhoufei'],
			}
		},
		skill:{
			//乐周妃
			...
			dczixi:{
				...
				direct:true,
				filter:function(event,player){
					return player.hasCard(card=>card.hasGaintag('dcqiqin'),'h');
				},
				content:function(){
					'step 0'
					var list=[];
            		for(var i=0;i<lib.inpile.length;i++){
						...
						if(get.type(name)=='delay') {
					    	//list.push(name);
					    	//list.push(game.createCard({name:name,suit:'none',number:null}));
					    	list.push(['','♬',name]);
						}
					}
					//list.push('cancel2');
					_status.commonDialog=true;
                    var dialog = ui.create.cardDialog('声明一张延时锦囊', [list, 'vcard']);
					player.chooseButton(dialog).set('ai',button=>{
						var card=game.createCard(button[2]);
						return get.info(card).usable;
					...
					if(!result.bool) event.finish();
						...
						event.judgeName=result.links[0][2];
						//result.control;
						player.chooseCardTarget({
							prompt2:'将一张“琴”置于一名角色的判定区',
							filterCard:function(card){
								return card.hasGaintag('dcqiqin');
							},
							//forced:true,
							filterTarget:function(card,player,target){
								return target.countCards('j',function(c){
									return c.judgeName==event.judgeName;
								})==0&&!target.hasJudge(event.judgeName);
							},
							ai1:function(card){
					...
					if(result.bool){
						...
						var targetCard = game.createCard({name:'dczixi_card',suit:get.suit(result.cards[0]),number:get.number(result.cards[0])});
				        player.$give(targetCard, target);
						target.addJudge({name:'dczixi_card'},result.cards[0]);
						result.cards[0].judgeName=event.judgeName;
						player.popup(get.translation(event.judgeName));
					}
				},
				//global:'dczixi_forbid',
				group:'dczixi_sb',
				subSkill:{
					sb:{
						audio:'dczixi',
						...
						prompt2:'当使用基本牌或普通锦囊牌指定唯一目标后，你可以根据其“琴”的数量：①此牌额外结算一次；②你摸两张牌；③弃置其判定区所有牌并对其造成3点伤害。',
						content:function(){
			...
			dcqiqin: {
				...
				content: function () {
					...
					if (hs.length) player.addGaintag(hs, 'dcqiqin');
					player.markAuto('dcqiqin',hs);
				},
					...
					restore:{
						...
						filter:function(event,player){
							return game.hasPlayer2(current=>{
							    return current.getStorage('dcqiqin').length
							});
						},
						forced:true,
						content:function(){
							...
							game.filterPlayer2(current=>{
							    if(current.getStorage('dcqiqin').length) {
							       cards.addArray(current.getStorage('dcqiqin'));
							    }
							});
							player.gain(cards.filterInD('d'),'gain2').gaintag.add('dcqiqin');
						},
					},
				},
				mod: {
					ignoredHandcard: function (card, player) {
						if (card.hasGaintag('dcqiqin')) {
							return true;
						}
					},
					cardDiscardable: function (card, player, name) {
						if (name == 'phaseDiscard' && card.hasGaintag('dcqiqin')) {
							return false;
						}
					},
				},
			...
			dcweiwan:{
				...
				init:function(player){
					player.storage.dcweiwan='';
				},
				filter:function(event,player){
					return game.hasPlayer(current=>current!=player)&&player.hasCard(card=>card.hasGaintag('dcqiqin'),'h');
				},
				filterCard:function(card){
					return card.hasGaintag('dcqiqin');
				},
				...
				content:function(){
					...
					event.num=0;
					player.storage.dcweiwan=target;
					'step 1'
					...
					switch(event.num){
						...
						case 2:player.addTempSkill('dcweiwan_sub1','phaseEnd');break;
						case 3:player.addTempSkill('dcweiwan_sub2','phaseEnd');break;
					}
				},
				subSkill:{
					sub1:{
						mod:{
							...
							if(target==player.storage.dcweiwan) return true;
							},
					...
					sub2:{
						mod:{
							playerEnabled:function(card,player,target){
								if(target==player.storage.dcweiwan) return false;
							},
						}
					},
				}
        	...
        	dcmoshou: {
        		...
        		content() {
        			'step 0'
        			event.num = player.maxHp - player.countMark(event.name);
        			if (event.num > 0) {
        	...
        	dcyunjiu: {
        		...
        		content: function() {
        			...
        			if (cards.length > 1) {
        				player.chooseCardButton("运柩：请选择要分配的牌", cards)
        					.set("ai", button => {
        			...
        			player.chooseTarget("选择一名其他角色获得" + get.translation(event.toGive), lib.filter.notMe)
        				.set("ai", target => {
    				...
    				if(result.bool && result.targets) {
            			player.logSkill('dcyunjiu', result.targets);
        			}else {
        			    event.finish();
        			}
        			'step 4'
        			let targetB = result.targets[0];
        			player.line(targetB, "green");
        			targetB.gain(event.toGive, "gain2").set("giver", player);
        			'step 5'
        			player.gainMaxHp();
        			'step 6'
        			player.recover();
			...
			dchuiling:{
				...
				subSkill:{
					hint:{
						...
						markHelasisy:[
						    'color: rgb(255, 100, 0);   font-weight: 900; text-shadow: 0 0 3.6px rgb(255, 255, 250), 0 0 4.3px rgb(255, 255, 100), 0 0 5px rgb(255, 255, 0);',
						    '',
						    'color: rgb(120, 120, 120); font-weight: 900; text-shadow: 0 0 3.6px rgb(255, 255, 250), 0 0 4.3px rgb(255, 255, 100), 0 0 5px rgb(255, 255, 0);',
						],
						content:function(){
							'step 0'
							var red=0,black=0;
							...
							game.broadcastAll(function(ind){
								...
								text='<span style="'+lib.skill.dchuiling_hint.markHelasisy[ind]+'">灵</span>';
			...
			dctaji:{
				...
				content:function(){
					...
					var effects=lib.skill.dctaji.effects;
					var name=evt.name;
					...
					for(var i=0;i<1+player.countMark('dcqinghuang_add');i++){
						...
						for(var effect in effects){
							if(effect==name){
								...
								next.player=player;
								next.setContent(effects[effect]);
								break;
							}
						}
					}
		...
		card:{
		    dczixi_card:{
				...
				noJudge:true,
				/*cancel:function(){
				    var targetCard = game.createCard({name:'dczixi_card',suit:get.suit(card),number:get.number(card)});
				    //var sourceCard = cards[0].copy();
				    player.showCards(targetCard);
				    //game.log(player,'的',card,'（',sourceCard,'）','对应','#y'+get.translation(cards[0].judgeName));
				},*/
				effect:function(){},
				ai:{
					basic:{
		...
		characterTitle:{
    		...
    		dc_sunru: '#y呦呦鹿鸣',
    		liuyong: '#y甘陵王',
    		wanniangongzhu: '#y还汉明珠',
    		zhanghu: '#y晋阳侯',
    		lvlingqi: '#y无双虓姬',
    		tenggongzhu: '#y芳华荏苒',
    		panghui: '#y临渭亭侯',
    		dc_zhaotongzhaoguang: '#y翊赞季兴',
    		yuantanyuanxiyuanshang: '#y兄弟阋墙',
    		yuechen: '#y广昌亭侯',
    		re_kanze: '#y慧眼的博士',
    		re_chendeng: '#y湖海之士',
    		caimaozhangyun: '#y乘雷潜狡',
    		dc_lvkuanglvxiang: '#y数合斩将',
    		dc_gaolan: '#y诽殇之柱',
    		yinfuren: '#y委身允翕',
    		chengui: '#y弄虎如婴',
    		chenjiao: '#y刚断骨鲠',
    		...
    		zhenghun: '#y民安寇灭',
    		dc_sunhanhua: '#y青丝慧剑',
    		liuchongluojun: '#y定境安民',
    		dc_huanghao: '#y便辟佞慧',
    		dc_sunziliufang: '#y谄陷负讥',
    		dc_sunchen: '#y凶竖盈溢',
    		gongsundu: '#y雄张海东',
    		mengyou: '#y蛮杰陷谋',
    		yue_caiwenji: '#y胡笳十八拍',
		...
		translate:{
        	...
        	dcmoshou_old:'墨守',
			dcmoshou_old_info:'当你成为其他角色使用的黑色牌的目标后，你可以摸X张牌（X为你本局游戏此前发动过此技能的次数÷3的余数+1）。',
			dcyunjiu_old:'运柩',
			dcyunjiu_old_info:'一名角色死亡后，你可以弃置等同于其因死亡事件的规则而弃置的牌数，将其此次弃置的牌交给一名其他角色。然后你加1点体力上限并回复1点体力。',
			gaoxiang:'高翔',
			dcchiying:'驰应',
			...
			yue_daqiao:'乐大乔',
			dczixi:'姊希',
			dczixi_info:'出牌阶段开始时和结束时，你可以声明一个延时锦囊牌名，并将一张“琴”置于一名角色的判定区，此“琴”离开其判定区前，其不能成为对应延时锦囊牌的目标。当你使用基本牌或普通锦囊牌指定唯一目标后，可以根据其判定区内牌数获得对应效果：一张：此牌额外结算一次；两张：你摸两张牌；三张：弃置其判定区所有牌并对其造成3点伤害。',
			dczixi_card:'琴',
			dczixi_card_info:'此牌离开目标角色判定区前，其不能成为对应延时锦囊牌的目标。',
			yue_xiaoqiao:'乐小乔',
			dcqiqin:'绮琴',
			dcqiqin_info:'锁定技，游戏开始时，你的初始手牌标记为“琴”且不计入手牌上限。准备阶段，你获得弃牌堆中所有“琴”。',
			yue_zhoufei:'乐周妃',
			dclingkong:'灵箜',
			dclingkongx:'灵箜',
			dclingkong_info:'锁定技，游戏开始时，你的初始手牌增加“箜篌”标记且不计入手牌上限。你于回合外获得牌后，随机将其中一张标记为“箜篌”牌。',
			dcxianshu:'贤淑',
			dcxianshu_info:'出牌阶段，你可以展示一张“箜篌”牌并交给一名其他角色。若此牌为红色，且该角色体力值小于等于你，该角色回复1点体力；若此牌为黑色，且该角色体力值大于等于你，该角色失去1点体力。然后，你摸X张牌（X为你与该角色体力值之差且至多为5）。',
			
			dcweiwan:'媦婉',
			dcweiwan_info:'出牌阶段限一次，你可以弃置一张“琴”并选择一名其他角色，随机获得其区域内与此牌花色不同的牌各一张，若你获得牌数为：一，其失去1点体力；二，你本回合对其使用牌无距离及次数限制；三，你本回合无法对其使用牌。',
			dcbeifen:'悲愤',
