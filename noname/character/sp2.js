...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		connect:true,
		character:{
			v_machao: ["male", "qun", 4, ["dczhongtao", "dcjizhan"]],
            v_caopi: ["male", "wei", 4, ["dcdianlun", "dcjiwei", "dcsugang"]],
			...
			guannings:['male','shu',3,['dcxiuwen','dclongsong']],
			...
			dc_zhaoyans:['male','wei',3,['dcfuning','dcbingji']],
	    	...
	    	mx_jiangwei:['male','shu',4,['mxzhuri','mxranji']],
            mx_simayi:['male','wei',3,['mxpingliao','mxquanmou']],
            my_simayi:['male','wei',3,['mxpingliao','mxquanmou'],['unseen']],
            mx_taishici: ["male", "wu", 4, ["olsbdulie", "olsbdouchan"], []],
            mx_pangtong: ["male", "shu", 3, ["olsbhongtu", "olsbqiwu"], ["die:true", "die:3"]],
        },
		characterSort:{
			sp2:{
				...
				sp_danqi:['dufuren','qinyilu','bianxi','dc_huban','dc_hujinding','dc_zhaoyans','wangwei','liyixiejing','guannings','dc_jsp_guanyu'],
				sp_fenghuo:['re_nanhualaoxian','tongyuan','zhangning','re_pangdegong'],
				sp_weizhen: ["v_lvbu","v_zhangliao","v_sunquan","v_dongzhuo","v_caopi","v_machao"],
				...
				sp_mouding:['mx_jiangwei','mx_simayi','mx_taishici','mx_pangtong'],
			}
		},
		skill:{
			//谋定天下+上兵伐谋系列
        		...
        		content: function() {
        			...
        			player.chooseCardTarget({
        					...
        					ai1(card) {
        						...
        						if (game.hasPlayer(current => current.isFriendOf(player))) {
        							if (
        			...
        			'step 2'
        			if (!result.cards || !result.cards.length || !result.targets || !result.targets.length) return;
        			...
        			if (!result.links || !result.links.length) {
        				for (const current of [event.target, player]) {
        				...
        				const numbers = event.cards
        						.map(card => get.number(card, player))
        						.sort((a, b) => a - b);
        				event.min = numbers[0],
        				event.max = numbers.at(-1);
        				event.card = result.links[0];
        				event.cards.remove(event.card);
        				event.cardx = get.autoViewAs(
        					{
        						name: get.name(event.card),
        						nature: get.nature(event.card),
        					},
        					[event.card]
        				);
        				const owner = get.owner(event.card);
        				const next = event.target
        					.chooseUseTarget(event.cardx, [event.card], true, false)
        					.set("throw", false)
        					.set("owner", owner)
        					.set("oncard", card => {
        				...
        				//get.is.sameNature(event.card, event.cardx, true)
        				var isSame=function(card1,card2) {
        				    if(get.suit(card1)!=get.suit(card2)) return false;
        				    if(get.name(card1)!=get.name(card2)) return false;
        				    if(get.number(card1)!=get.number(card2)) return false;
        				    return true;
        				};
        				if (event.card.name === event.cardx.name && isSame(event.card, event.cardx)) next.set("viewAs", false);
        			}
        			'step 5'
        			const restCards = event.cards.filter(card => {
        			...
        			if (
        				...
        				event.target.addSkill("olsbhongtu_limit");
        				if (!event.target.storage.olsbhongtu_limit) event.target.storage.olsbhongtu_limit = [0, 0];
        				event.target.storage.olsbhongtu_limit[0] += 2;
        			} else if (num != event.min && num != event.max) {
        				skill = "qianxi";
        			}
        			if (skill) {
        				let skillName = `olsbhongtu_${player.playerid}`;
        				event.target.addAdditionalSkill(skillName, [skill], true);
        				delete event.target.storage.olsbhongtu_phased;
        				event.target.addWhen({
        				    trigger: { player: "phaseBegin" },
        				    content: function() {
        				        player.storage.olsbhongtu_phased = true;
        				    },
        				});
        				/*event.target.when({ player: "phaseBegin" }).then(() => {
        					player.storage.olsbhongtu_phased = true;
        				});*/
        				...
        				event.target.addWhen({
        				    ...
        				    skillName: skillName,
        				    filter: function(event,player) {
        				        return _when.target.storage.olsbhongtu_phased;
        				    },
        				    content: function() {
        				        delete player.storage.olsbhongtu_phased;
        						player.removeAdditionalSkill(_when.skillName);
        				    },
        				});
        				/*event.target
        					.when({ player: "phaseEnd" })
        					.filter(() => {
        						return event.target.storage.olsbhongtu_phased;
        					})
        					.assign({
        						firstDo: true,
        						priority: Infinity,
        	...
        	olsbqiwu: {
        		...
        		filter: function(event, player) {
        			...
        			return true;//(!player.getHistory('damage').length);
        			/*return (
        				game
        					.getGlobalHistory(
        						"every个thing",
        		...
        		content: function() {
        			'step 0'
        			player.chooseToDiscard(get.prompt("olsbqiwu"), `你可以弃置一张红色牌，防止${get.translation(trigger.source)}对你造成的${trigger.num}点伤害。`, "chooseonly", { color: "red" }, "he")
        				.set("ai", card => {
        					/*if (get.event("goon")) return 6 - get.value(card);
        					return 0;*/
        					if(get.damageEffect(player, trigger.source, player) < 0)	return 6 - get.value(card);
        					return 0;
        				});
			...
			olsbdulie: {
				...
				content: function() {
					...
					player.addWhen({
					    ...
					    filter: function(evt) {return evt == _when.getParent;},
					    getParent: trigger.getParent(),
					    content: function() {
					        const num = Math.min(5, player.getAttackRange());
            ...
            mxquanmou:{
                ...
                intro:{
                    content:function(storage,player,skill){
                        if(player.storage.mxquanmou==true) return '出牌阶段每名角色限一次，你可以令攻击范围内一名其他角色交给你一张牌，此阶段你下次对其造成伤害后，可以对至多三名其他角色(该角色除外)各造成1点伤害';
                        return '出牌阶段每名角色限一次，你可以令攻击范围内一名其他角色交给你一张牌，防止你此阶段下次对其造成的伤害';
                    },
                },
                init:function(player){
                    player.storage.mxquanmou2=[];
                },
                enable:'phaseUse',
                filter:function(event,player){
                    return game.hasPlayer(function(c){
                        return !c.hasSkill('mxquanmou_r')&&player.inRange(c);
                    })
                },
                filterTarget:function(card,player,target){
                    return !target.hasSkill('mxquanmou_r')&&player.inRange(target);
                },
                changeAvatar:function(player,type) {
                    /*var list=[];
                    if(player.name&&get.character(player.name)[3].includes('mxquanmou')) list.add(player.name);
                    if(player.name1&&get.character(player.name1)[3].includes('mxquanmou')) list.add(player.name1);
                    if(player.name2&&get.character(player.name2)[3].includes('mxquanmou')) list.add(player.name2);
                    var avatar=type?'mx_simayi':'my_simayi';
                    if(list.length) list.forEach(name=>player.reinit(name,avatar));*/
                    player.changeAvatarTo('mx_simayi','my_simayi',!type);
                },
                content:function(){
                    'step 0'
                    if(target.countCards('h')) target.chooseCard(('选择一张牌交给'+get.translation(player)),true);
                    'step 1'
                    if(result.bool){
                        player.gain(result.cards[0],'giveAuto');
                    }
                    'step 2'
                    target.addTempSkill('mxquanmou_r');
                    if(player.storage.mxquanmou!=true) target.addTempSkill('mxquanmou_p');
                    else player.storage.mxquanmou2.add(target);
                    player.changeZhuanhuanji('mxquanmou');
                    //if(player.name=='mx_simayi'||player.name1=='mx_simayi'||(player.name2&&player.name2=='mx_simayi')) player.changeZhuanhuanji('mxquanmou','mx_simayi2');
                    //else player.changeZhuanhuanji('mxquanmou','mx_simayi');
                    lib.skill.mxquanmou.changeAvatar(player,!player.storage.mxquanmou);
                },
                ai:{
                    order:10,
                    result:{
                        target:function(player,target){
                            if(player.storage.mxquanmou!=true) return 1;
                            else -1.2;
                        },
                        player:function(player,target){
                            if(game.hasPlayer(function(c){
                                return !c.hasSkill('mxquanmou_r')&&player.inRange(c)&&(get.attitude(player,c)<=0||(get.attitude(player,c)>0&&c.hp<2&&c.countCards('h')>2));
                            })) return 0.6;
                            else return 0;
                        },
                    },
                },
                group:['mxquanmou_d','mxquanmou_c'],
                subSkill:{
                    r:{charlotte:true,},
                    p:{
                        trigger:{
                        ...
                        onremove:true,
                        filter:function(event,player){
                            return event.source.hasSkill('mxquanmou');
                        },
                        content:function(){
                            player.removeSkill('mxquanmou_p');
                            trigger.cancel();
                        },
                        ai:{
                            nofire:true,
                    ...
                    d:{
                        ...
                        filter:function(event,player){
                            return game.hasPlayer(function(c){
                                return player.storage.mxquanmou2&&player.storage.mxquanmou2.includes(event.player);
                            })
                        },
                        content:function(){
                            'step 0'
                            ...
                            if(result.bool){
                                ...
                                var index = player.storage.mxquanmou2.indexOf(trigger.player);
                                if (index !== -1) {
                                    player.storage.mxquanmou2.splice(index, 1);
                                }                               
                            }
                        }
                    },
                    c:{
                        ...
                        charlotte:true,
                        lastDo:true,
                        content:function(){player.storage.mxquanmou2=[];}
                    },
                },
                mod:{
			...
			mxzhuri:{
				...
				filter:function(event,player){
					if(player.hasSkill('mxzhuri_block')) return false;
					...
					}).length+player.getHistory('lose',function(evt){
					    return evt.getParent(event.name)==event;
					}).length>0;
				},
				direct:true,
				...
				content:function(){
					...
					//var trigger=map.trigger;//yield
					var result=player.chooseTarget(get.prompt('mxzhuri'),'与一名角色进行拼点，若你赢，你可以使用其中的一张拼点牌；若你没赢，你失去1点体力或令此技能于本回合失效',(card,player,target)=>{
					...
					if(result.bool){
						...
						player.logSkill('mxzhuri',target);
						result.bool=false;
					...
					'step 2'
					    if(result.bool){
							...
							if(cards.length){
								player.chooseButton(['是否使用其中的牌？',cards]).set('ai',button=>_status.event.player.getUseValue(button.link));
								event.goto(4);
							}else {
							    event.finish();
							}
							...
							var list=lib.skill.mxranji.getList(trigger);
							var result3=player.chooseControl('失去体力','技能失效').set('prompt','逐日：失去1点体力，或令此技能于本回合失效').set('ai',()=>{
								var player=_status.event.player;
								if(player.getHp>2){
					...
					'step 3'
					if(result.control) player[result.control=='失去体力'?'loseHp':'addTempSkill'](result.control=='失去体力'?1:'mxzhuri_block');
					event.finish();
					'step 4'
			...
			mxranji:{
				...
				prompt2:function(event,player){
					var str='获得技能';
					var num=lib.skill.mxranji.getNum(event,player);
				...
				content:function(){
					'step 0'
					player.awakenSkill('mxranji');
					//var player=map.player;
					//var trigger=map.trigger;
					//alert(lib.skill.mxranji.getNum(trigger,player));
					...
					if(player.countCards('h')!=player.getHandcardLimit()||player.isDamaged()){
						event.num1=player.countCards('h')-player.getHandcardLimit();
						if(!event.num1) result.control='体力值';
						else if(player.isHealthy()) result.control='手牌数';
						else {
							player.chooseControl('手牌数','体力值').set('choiceList',[
								event.num1<0?'摸'+get.cnNumber(-event.num1)+'张牌':'弃置'+get.cnNumber(event.num1)+'张牌',
							...
							}).set('list',[-event.num1,player.getDamagedHp()]);
						}
						event.goto(1);
					}else event.goto(2);
					'step 1'
					if(result.control=='手牌数'){
						if(event.num1<0) player.drawTo(player.getHandcardLimit());
						else player.chooseToDiscard(event.num1,'h',true);
					}
					else{
						player.recover(player.maxHp-player.hp);
					}
					'step 2'
					player.addTempSkill('mxranji_norecover',{source:'dieAfter'});
					//player.when('mxranjiAfter').then(()=>player.addSkill('mxranji_norecover'));
					//player.when({source:'dieAfter'}).then(()=>player.removeSkill('mxranji_norecover'));
				},
				derivation:['kunfen','zhaxiang'],
				getList:function(event){
					var lists=[];
					var phaseList=['phaseZhunbei','phaseJudge','phaseDraw','phaseUse','phaseDiscard','phaseJieshu'];
					for(var a in phaseList) {//event.getParent().phaseList
					    lists.push(a.split('|')[0]);
					}
					return lists;
					/*return .forEach(function(list){
					    return list.split('|')[0];
					});*/
				},
				getNum:function(event,player){
					//alert(lib.skill.mxranji.getList(event).slice(0,event.getParent().num));
					var phaseList=['phaseZhunbei','phaseJudge','phaseDraw','phaseUse','phaseDiscard','phaseJieshu'];
					return phaseList.filter(function(name){
					    return player.getHistory('useCard',function(evt){
					        return evt.getParent(name).name==name;
					    }).length;
					}).length;
				},
				getNum_old:function(event,player){
					//有点小bug
					return lib.skill.mxranji.getList(event).slice(0,event.getParent().num).filter(name=>player.getHistory('useCard',evt=>evt.getParent(name).name==name).length).length;
				},
				subSkill:{
					norecover:{
						charlotte:true,
			...
			dcnuchen:{
				...
				content:function(){
					...
					player.chooseToDiscard(false,'怒嗔：是否弃置至少一张'+str+'牌？','若如此做，你对其造成等量伤害；或点击“取消”，获得其所有'+str+'手牌','he',{suit:suit},[1,Infinity]).set('ai',card=>{
			...
			dcbaguan: {
        		...
        		init(player, skill) {
        			...
        			if (evt && !evt.dcbaguan) {
        			    player.storage.isDcbaguan = false;
        			    get.info("lianzhaoEffect").method(player, 30, 'dcbaguan');
        			    player.unmarkSkill("dcbaguan");
						player.storage["dcbaguan"]="";
						player.addMark("dcbaguan", "连击①");
        			    //player.addTip(skill, "霸关 可连击");
        			}else {
        			    player.unmarkSkill("dcbaguan");
        			    get.info("lianzhaoEffect").method(player, 0, 'dcbaguan');
        			}
        		},
        		onremove(player, skill) {
        			player.unmarkSkill("dcbaguan");
        			get.info("lianzhaoEffect").method(player, false, 'dcbaguan');
        		...
        		getUsed: (player, first) => {
        			...
        			if (history.targets.length!=1) return false;
        			return history;
        		...
        		filter(event, player) {
        			...
        			if (!evt||evt.dcbaguan) return false;
        			return true;
        		},
        		content() {
        			"step 0"
        			player.storage.isDcbaguan=true;
        			get.info("lianzhaoEffect").method(player, 60, 'dcbaguan');
    			    player.unmarkSkill("dcbaguan");
					player.storage["dcbaguan"]="";
					player.addMark("dcbaguan", "连击②");
        			...
        			next.set("oncard", (card, player) => {
            			...
            			if(lib.config.effect_pot_all&&typeof skinSwitch=='object'&&skinSwitch.chukuangWorkerApi?.playEffect) {
            			    skinSwitch.chukuangWorkerApi.playEffect({
                                name: "../../../十周年UI/assets/animation/weilvbu_jineng",
                                version: "4.0",
                            },{
                                scale: 0.8,
                                speed: 1,
        				...
        				_status.event.set("dcbaguan", true);
        				_status.event.baseDamage = _status.event.cards.length;
        				
            			player.unmarkSkill("dcbaguan");
    					player.storage["dcbaguan"]="";
    					player.addMark("dcbaguan", "<span style='color: rgb(255, 255, 255); text-shadow: 0 0 2.5px rgb(255, 255, 100);'>连击⚔️</span>");
        			});
        			//await next;
        			"step 1"
    			    get.info("dcbaguan").init(player, "dcbaguan");
        		},
        		intro: {
        		    content:function(storage,player){
        		        var first=0.6;
        		        var last=0.6;
        		        if(player.storage.isDcbaguan) {
        		            last=1;
        		        }else {
        		            first=1;
        		        }
        		        return "连招：<span style='opacity:"+first+";'>单目标牌</span>→<span style='opacity:"+last+";'>武器牌</span>";
        		    },
        		},
        		subSkill: {
        			mark: {
        	...
        	dcxiaowu: {
        		...
        		content() {
        			...
        			player.addWhen({
        			    trigger:{ source: "damageSource" },
        			    filter: function(event,player) {
        			        return event=player.getStat().skill.dcxiaowu;
        			    },
        			    content:function() {
        	...
        	dcporong: {
        		...
        		content() {
        			"step 0"
        			player.storage.isDcporong=true;
        			get.info("lianzhaoEffect").method(player, 60, 'dcporong');
        			player.unmarkSkill("dcporong");
					player.storage["dcporong"]="";
					player.addMark("dcporong", "连击②");
					event.hasntUsed = false;
					player.chooseBool(get.prompt2("dcporong")).set('ai',function(){
						return get.info("dcporong").check(trigger,player);
					});
        			"step 1"
        			if(result.bool) {
        			    player.logSkill("dcporong",trigger.targets.sortBySeat());
        			}else {
        			    event.hasntUsed = true;
        			    event.goto(3);
        			}
        			"step 2"
        			if(lib.config.effect_pot_all&&typeof skinSwitch=='object'&&skinSwitch.chukuangWorkerApi?.playEffect) {
        			    skinSwitch.chukuangWorkerApi.playEffect({
                            name: "../../../十周年UI/assets/animation/weizhangliao_jineng",
                            version: "4.0",
                        },{
                            scale: 0.8,
                            speed: 1,
        			...
        			player.unmarkSkill("dcporong");
					player.storage["dcporong"]="";
					player.addMark("dcporong", "<span style='color: rgb(255, 255, 255); text-shadow: 0 0 2.5px rgb(255, 255, 100);'>连击⚔️</span>");
        			const targets = trigger.targets;
        			trigger.set("dcporong", true);
        			...
        			for (const target of targets) {
        				...
        				if (targetsx.length) player.gainMultiple(targetsx);
        			}
        			trigger.effectCount+=1;
        			"step 3"
        			player.addWhen({
        			    trigger:{
        			        player:['useCardAfter','phaseBefore','phaseAfter'],
        			    },
        			    player:player,
        			    card:trigger.card,
        			    used:!event.hasntUsed,
        			    filter:function(event,player){
        			        if(event.name=='useCard'){
        			            return event.card&&event.card==_when.card;
        			        }else {
        			            return true;
        			        }
        			    },
        			    content:function(){
        			        'STEP 0'
        			        if(_when.used) lib.skill.dcjianying.getLastUsed(player).dcporong=true;
        			        'STEP 1'
        			        get.info("dcporong").init(player, "dcporong");
        			    },
        			});
        		},
        		init(player, skill) {
        			...
        			if (evt?.card && get.tag(evt.card, "damage") > 0.5 && !evt[skill]) {
        			    player.storage.isDcporong = false;
        			    get.info("lianzhaoEffect").method(player, 30, 'dcporong');
        			    player.unmarkSkill("dcporong");
    					player.storage["dcporong"]="";
    					player.addMark("dcporong", "连击①");
        			}else {
        			    player.unmarkSkill("dcporong");
        			    get.info("lianzhaoEffect").method(player, 0, 'dcporong');
        			}
        		},
        		onremove(player, skill) {
        			player.unmarkSkill("dcporong");
        			get.info("lianzhaoEffect").method(player, false, 'dcporong');
        		},
        		ai: {
        			directHit_ai: true,
        		...
        		intro: {
        		    content:function(storage,player){
        		        var first=0.6;
        		        var last=0.6;
        		        if(player.storage.isDcporong) {
        		            last=1;
        		        }else {
        		            first=1;
        		        }
        		        return "连招：<span style='opacity:"+first+";'>伤害牌</span>→<span style='opacity:"+last+";'>【杀】</span>";
        		    },
        		},
        		subSkill: {
        			mark: {
        	...
        	dcwoheng: {
        		...
        		content:function() {
        			"step 0"
        			var num = player.countMark("dcwoheng");
        			player
    				...
    				if(!result.bool) {
    				    ...
    				    event.target = result.target || result.targets[0];
    				    if(!event.target) event.finish();
    				}
    				"step 2"
    				player.logSkill("dcwoheng", event.target);
        			...
        			var goon = !player.hasSkill("dcyuhui_isInBuff");
        			var num = goon ? player.countMark("dcwoheng") : 1;
        			event.num = num;
        			if (!event.target?.isIn()) return;
        			const str1 = "摸" + get.cnNumber(num) + "张牌";
        			const str2 = "弃" + get.cnNumber(num) + "张牌";
        			event.list = [str1];
        			if (event.target.countCards("he")) event.list.push(str2);
        			player
    					.chooseControl(event.list)
    					.set("ai", function(){
    					    return get.effect(event.target, { name: "wuzhong" }, player, player) *
    							(() => {
    								if (goon && player.countMark("dcwoheng") <= 3) {
    									if (event.target.countCards("h") + num === player.countCards("h")) return 100 * num;
    								}
    								return num*0.5;
    							})() >
    							get.effect(event.target, { name: "guohe_copy2" }, event.target, player) *
    								(() => {
    									const numx = Math.min(num, event.target.countDiscardableCards(event.target, "he"));
    									if (goon && player.countMark("dcwoheng") <= 3) {
    										if (event.target.countCards("h") - numx === player.countCards("h")) return 100 * numx;
    									}
    									return numx;
    								})()
    							? str1
    					...
    					.set("prompt", get.translation("dcwoheng") + "：令" + get.translation(event.target) + "…");
        			"step 3"
        			...
        			"step 4"
        			if (player.countMark("dcwoheng") > 3 || player.countCards("h") !== event.target.countCards("h")) {
        				player.draw(2);
        			}else {
        			    event.finish();
        			}
        			...
        			order(item, player) {
        				...
        				if (
        					game.hasPlayer(target => {
        						if (get.effect(target, { name: "wuzhong" }, player, player) > 0) {
        				...
        				player(player, target) {
        					if(!target) {
        					    return game.hasPlayer(target2 => {
        					        return get.effect(target2, "dcwoheng", player, player)>0;
        					    })?5:0;
        					}
        					//const goon = !_status.event?.getParent()?.name.includes("dcyuhui_buff");
        					var goon = !player.hasSkill("dcyuhui_isInBuff");
        		...
        		init(player) {
        			/*const num = (() => {
        				...
        				for (let i = globalHistory.length - 1; i >= 0; i--) {
        					num += globalHistory[i].every个thing.filter(evt => evt.name === "dcwoheng").length;
        					if (globalHistory[i].isRound) break;
        				}
        				return num;
        			})();*/
        			const num = player.getStat('skill').dcwoheng;
        			if (num) {
        	...
        	dcyuhui: {
        		...
        		content:function() {
    				...
    				"step 2"
    				player.logSkill("dcyuhui", result.targets);
        		...
        		subSkill: {
        			...
        			buff: {
        				...
        				content:function() {
        					"step 0"
        					event.targets = get.info("dcyuhui_buff").getIndex(trigger, player);
        					"step 1"
        					if(!event.targets||!event.targets.length) {
        					    event.finish();
        					}else {
        					...
        					"step 2"
        					player.unmarkAuto("dcyuhui_buff", [event.target]);
        					if (!player.getStorage("dcyuhui_buff").length) player.removeSkill("dcyuhui_buff");
        					if (!event.target?.isIn() || !game.hasPlayer(target => target !== player)) {
        						event.goto(1);
        					}else {
            					var list = ["dcyuhui_buff", event.target];
            					player
            						.chooseCard(
            							card => {
            								return get.color(card) == "red" && get.type(card) == "basic";
            							...
            							return (
            								Math.max(
            									...game
            										...
            										.map(target => {
            											//return get.info("dcwoheng").ai.result.player(player, target);
            											return get.effect(target, "dcwoheng", player, player);
            										})
            								) - get.value(card)
            							);
            						})
            						.set("prompt2", "交给" + get.translation(event.target) + "一张红色基本牌，发动一次X为1的〖斡衡〗：令一名其他角色摸X张牌或弃置X张牌，然后若其手牌数与你不同，你摸两张牌且此技能失效至本回合结束。")
            						.set("logSkill", list);
    						}
    						"step 3"
    						if (result.bool && result.cards) {
    						    player.give(result.cards, event.target)
    						        .set('skill','dcyuhui_buff');
    						}else {
    						    event.goto(1);
    						}
    						"step 4"
    						player.addTempSkill("dcyuhui_isInBuff");
        					player.useSkill("dcwoheng");
        					"step 5"
        					player.removeSkill("dcyuhui_isInBuff");
        					event.goto(1);
        				},
        				...
        				content() {
        					...
        					if (targets.length == 1) {
            					...
            					player.chooseTarget(
        							true,
        							(card, player, target) => {
        								const targets = event.getTrigger().targets;
        					...
        					if(!result.bool) event.finish();
        					'step 2'
        					lib.translate["dcguangyong_toself"]='犷勇';
        					'step 3'
        					player.logSkill("dcguangyong", result.targets);
        					if (player.maxHp > 1) player.loseMaxHp();
        					player.gainPlayerCard(result.targets[0], "he", true);
        					'step 4'
        					lib.translate["dcguangyong_toself"]='犷勇(他人)';
        				},
        			},
        		},
        	},
        	dcjuchui: {
        		...
        		content() {
        			'step 0';
					player.storage.isDcjuchui = true;
                    get.info("lianzhaoEffect").method(player, 60, 'dcjuchui');
        			player.unmarkSkill("dcjuchui");
					player.storage["dcjuchui"]="";
					player.addMark("dcjuchui", "连击②");
					event.triggerTargets = trigger.targets;
					player.chooseTarget(get.prompt2("dcjuchui"), (card, player, target) => {
                    ...
                    if (!result.bool || !result.targets) {
            			player.unmarkSkill("dcjuchui");
                        get.info("lianzhaoEffect").method(player, 0, 'dcjuchui');
                        event.finish();
                    }else {
        			    player.logSkill("dcjuchui",result.targets);
        			}
                    'step 2';
                    if(lib.config.effect_pot_all&&typeof skinSwitch=='object'&&skinSwitch.chukuangWorkerApi?.playEffect) {
        			    skinSwitch.chukuangWorkerApi.playEffect({
                            name: "../../../十周年UI/assets/animation/weidongzhuo_jineng",
                            version: "4.0",
                        },{
                            scale: 0.8,
                            speed: 1,
                    ...
                    player.unmarkSkill("dcjuchui");
					player.storage["dcjuchui"]="";
					player.addMark("dcjuchui", "<span style='color: rgb(255, 255, 255); text-shadow: 0 0 2.5px rgb(255, 255, 100);'>连击⚔️</span>");
        			event.target = result.targets[0];
                    if (event.target.hasSex("male")) player.chat("雄性人类！");
                    game.delayx();
                    
                    'step 3';
                    if (event.target.maxHp <= player.maxHp) {
                        if (event.target.isDamaged()) {
                            player.chooseControl("失去体力", "回复体力")
                                .set("ai", () => {
                                    //const { player, targets: [eventTarget] } = _status.event.getParent();
                                    let player = event.player, eventTarget = event.triggerTargets[0];
                                    return get.effect(eventTarget, { name: "losehp" }, player, player) >= get.recoverEffect(eventTarget, player, player) ? 0 : 1;
                                })
                                .set("prompt", get.translation(event.name) + "：令" + get.translation(event.target) + "失去或回复1点体力");
                        } else {
                            result.control = "失去体力";
                        }
                        event.goto(4);
                        ...
                        if (types.length > 1) {
                            player.chooseControl(types)
                                .set("ai", () => {
                                    //const { player, targets: [eventTarget] } = _status.event.getParent();
                                    let player = event.player, eventTarget = event.triggerTargets[0];
                                    const att = Math.sign(get.attitude(player, eventTarget));
                                    return types.sort((a, b) => 
                                        att * (eventTarget.countCards("h", card => get.type2(card) === a) - eventTarget.countCards("h", card => get.type2(card) === b))
                                    )[0];
                                })
                                .set("prompt", get.translation(event.name) + "：选择获得类别的牌且" + get.translation(event.target) + "本回合不能使用此类别的牌");
                        } else {
                            result.control = types[0];
                        }
                        event.goto(5);
                    }
                    ...
                    'step 5';
                    const card = get.cardPile2(card => get.type2(card) === result.control);
                    card ? player.gain(card, "gain2") : player.chat("什么罐头我说？！");
                    event.target.addTempSkill("dcjuchui_ban");
                    event.target.markAuto("dcjuchui_ban", [result.control]);
                    player.unmarkSkill("dcjuchui");
                    get.info("lianzhaoEffect").method(player, 0, 'dcjuchui');
        		},
        		mod: {
        		...
        		onremove(player, skill) {
        			...
        			get.info("lianzhaoEffect").method(player, false, 'dcjuchui');
        		},
        		intro: {
        		    content:function(storage,player){
        		        var first=0.6;
        		        var last=0.6;
        		        if(player.storage.isDcjuchui) {
        		            last=1;
        		        }else {
        		            first=1;
        		        }
        		        return "连招：<span style='opacity:"+first+";'>装备牌</span>→<span style='opacity:"+last+";'>锦囊牌</span>";
        		    },
        		},
        		subSkill: {
        			combo: {
        				...
        				content() {
        					...
        					if(get.type(trigger.card) === "equip") {
        					    player.addSkill(event.name + "Skill");
        					}else if(player.hasSkill(event.name + "Skill")) {
        					    player.removeSkill(event.name + "Skill");
        					}
        				},
        			},
        			comboSkill: {
        				...
        				init(player, skill) {
        					...
        					player.storage.isDcjuchui = false;
        					get.info("lianzhaoEffect").method(player, 30, 'dcjuchui');
    						player.unmarkSkill("dcjuchui");
    						player.storage["dcjuchui"]="";
    						player.addMark("dcjuchui", "连击①");
        				},
            ...
            dcdianlun: {
                ...
                content: function () {
                    ...
                    player.draw(num).set("gaintag", [event.name]);
                    player.addSkill("dcdianlun_clear");
                    player.addTempSkill("dcdianlun_count", "phaseAfter"); // 添加临时技能，记录摸到的牌
                },
                ai: {
                    order: 10,
                    result: {
                ...
                subSkill: {
                    ...
                    clear: {
                        ...
                        firstDo: true,
                        content: function () {
                            player.removeSkill("dcdianlun_clear");
                            player.removeGaintag("dcdianlun");
                            player.removeSkill("dcdianlun_count"); // 移除临时技能
                        }
                    },
                    count: {
                        charlotte: true,
                        onremove: function (player) {
                            delete player.storage.dcdianlun_count;
                            player.removeGaintag("dcdianlun");
                        },
                        intro: {
                            content: (storage, player) => `已记录牌：${get.translation(storage)}`,
                        },
                        mod: {
                            targetInRange: function (card, player, target) {
                                if (card.cards) {
                                    for (var i of card.cards) {
                                        if (i.hasGaintag("dcdianlun")) return true;
                                    }
                                }
                            },
                            cardUsable: function (card, player) {
                                if (card.cards) {
                                    for (var i of card.cards) {
                                        if (i.hasGaintag("dcdianlun")) return Infinity;
                                    }
                                }
                            },
                        },
                        sub: true,
                        sourceSkill: "dcdianlun",
                    }
                }
            },
    	    dcjiwei: {
    		    ...
    		    subSkill: {
    			    global: {
    		            ...
    		            content:function(){
    			            "step 0"
    					    target.logSkill("dcjiwei", player);
    					    player.give(cards, target);
    					    target
    						    .chooseBool(`极威：是否令${get.translation(player)}发动一次至多弃置三张牌的〖典论〗`)
    						    .set("ai", () => get.attitude(_status.event.player, _status.event.getParent().player) > 0);
    			            "step 1"
    					    if (!result?.bool) {
    						    target.popup("拒绝");
    						    event.finish();
    					    ...
    					    if (result?.bool && result.cards?.length) {
    						    player.useSkill("dcdianlun", result.cards);
    					    }
    				    },
    				    ai: {
    					    order: 7,
    	    ...
    	    dcsugang: {
    		    ...
    		    content:function(){
    			    ...
    			    player
    				    .judge()
    				    .set("callback", () => {
    					    const player = _status.event.getParent(2).player,
    						    card = _status.event.judgeResult.card;
    					    if (get.position(card, true) == "o") {
    						    player.gain(card, "gain2").gaintag.add("dcsugang_viewAs");
    					    }
    					    player.addSkill("dcsugang_clear");
    				    });
    			    "step 1"
    			    if (!result?.card) {
    				    event.finish();
    			    ...
    			    if (result?.links?.length) {
    				    ...
    				    if (links.includes("debuff")) {
    					    ...
    					    player
    						    ...
    						    .set("num", num2)
    						    .set("ai", card => {
    							    return -Math.abs(get.number(card, _status.event.player) - _status.event.num);
    						    });
    				    } else event.finish();
    				    if (links.includes("buff")) {
    					    game.log(player, "选择了", "#y选项三");
    					    player.addTempSkill("rexingshang");
    				...
    				const cardx = result.cards[0],
    				    num3 = event.num,
    				    skill2 = event.name;
    				player.showCards(cardx, `${get.translation(player)}发动【肃纲】展示的牌`);
    				const range = [num3, get.number(cardx, player)].sort((a, b) => a - b);
    				player.line(game.filterPlayer(), "yellow");
    				for (const target of game.filterPlayer()) {
    					target.storage[skill2 + "_debuff"] = range;
    					target.addTempSkill(skill2 + "_debuff");
    				}
    		    },
    		    ...
    		    subSkill: {
    			    clear: {
    			        ...
    			        firstDo: true,
    			        content:function(){
    				        player.removeSkill("dcsugang_clear");
        	...
        	dczhongtao: {
        		...
        		chooseButton: {
        			dialog(event, player) {
        				return ui.create.dialog('众讨',`<div class="text center">出牌阶段限一次，你可以选择至多${Math.min(4, player.getDamagedHp() + 1)}种花色，然后随机获得弃牌堆中你选择花色的各一张牌。</div>`, [lib.suit.map(suit => "lukai_" + suit), "vcard"]);
        			},
        			check(button) {
        				return Math.random();
        			},
        	...
        	dcjizhan: {
        		...
        		content() {
        			'step 0'
        			player.storage.isDcjizhan=true;
        			get.info("lianzhaoEffect").method(player, 60, 'dcjizhan');
        			player.unmarkSkill("dcjizhan");
					player.storage["dcjizhan"]="";
					player.addMark("dcjizhan", "连击②");
					event.skill = 'dcjizhan';
        			event.triggerNum = player.getHistory("useSkill", evt => evt.skill == event.skill).length + 1;
        			player.chooseButton([
						get.prompt2(event.skill),
						[
							[
								["discard", `弃置其他角色共计至多${event.triggerNum}张牌`],
								["damage", `对一名角色造成${event.triggerNum}点伤害，然后此技能本回合失效`],
							],
							"textbutton",
						],
					]).set('filterButton',button=>{
						...
						if (button.link == "discard") {
							const list = game
								...
								.sort((a, b) => b - a);
							return list.slice(0, Math.min(_status.event.num, list.length)).reduce((eff, num) => eff + num, 0);
						}
						return Math.max(...game.filterPlayer().map(target => get.damageEffect(target, player, player) * _status.event.num));
					}).set('num',event.triggerNum);
					'step 1'
					if(result.bool && result.links?.length) {
						...
						if(event.links != "discard") {
    					    player.chooseTarget(1,trans[event.links[0]],function(card,player,target){
                                const selected = _status.event.links;
        						if (!selected.length) {
        							return false;
        						}
        						if (selected[0] == "discard") {
        							return false;
        						}
        						return true;
                            }).set('ai',function(target){
        						...
        						return get.damageEffect(target, player, player) * _status.event.num;
                            }).set('links',event.links).set('num',event.triggerNum);
                        }else {
                            event._result = {
                                bool: true,
                                targets: [],
        			...
        			if (result.bool) {
        				player.unmarkSkill("dcjizhan");
    					player.storage["dcjizhan"]="";
    					player.addMark("dcjizhan", "<span style='color: rgb(255, 255, 255); text-shadow: 0 0 2.5px rgb(255, 255, 100);'>连击⚔️</span>");
            			player.logSkill(event.skill);
        				event.targets = result.targets ?? [];
        				event.choice = event.links[0];
        				trigger.set("dcjizhan", true);
        				if(lib.config.effect_pot_all&&typeof skinSwitch=='object'&&skinSwitch.chukuangWorkerApi?.playEffect) {
            			    skinSwitch.chukuangWorkerApi.playEffect({
                                name: "../../../十周年UI/assets/animation/weimachao_jineng",
                                version: "4.0",
                            },{
                                scale: 0.8,
                                speed: 1,
        			...
        			'step 3'
        			if (event.choice != "discard") {
        			    if(event.targets.length) event.targets[0].damage(event.triggerNum);
        			...
        			'step 4'
        			if(event.triggerNum < 1) {
        			    event.goto(9);
        			}
        			'step 5'	
        			player
						.chooseTarget("###极斩###", `弃置任意名其他角色累计至多${event.triggerNum}张牌`, (card, player, target) => {
						...
						.set("lose_list", event.lose_list)
    				'step 6'
					if (result.bool && result.targets?.length) {
						event.target = result.targets[0];
						player
							.choosePlayerCard(event.target, true, "he", [1, event.triggerNum], `选择弃置${get.translation(event.target)}的牌`)
							.set("filterButton", button => {
					...
					'step 7'
					if(result.bool && result.cards?.length) {
    					event.triggerNum -= result.cards.length;
    					var index = event.lose_list.find(item => item[0] == event.target);
    					if (!index) {
    						event.lose_list.push([event.target, result.cards]);
    					} else {
    						index[1].addArray(result.cards);
    					}
    					event.target.discard(result.cards, "notBySelf").set("discarder", player);
					}
    				'step 8'
    				if (_status.connectMode) {
    					game.broadcastAll(function () {
        			...
        			event.goto(4);
        			'step 9'
			        get.info("dcjizhan").init(player, "dcjizhan", true);
        		},
        		/*init(player, skill) {
        		...
        		init(player, skill, noAdd) {
        			if(!noAdd) player.addSkill(skill + "_mark");
        			const evt = lib.skill.dcjianying.getLastUsed(player);
					if (evt?.card && get.type(evt.card) == "equip" && !evt.dcjizhan) {
        			    player.storage.isDcjizhan = false;
        			    get.info("lianzhaoEffect").method(player, 30, 'dcjizhan');
        			    player.unmarkSkill("dcjizhan");
						player.storage["dcjizhan"]="";
						player.addMark("dcjizhan", "连击①");
        			    ...
        			    player.unmarkSkill("dcjizhan");
        			    get.info("lianzhaoEffect").method(player, 0, 'dcjizhan');
        			}
        		},
        		onremove(player, skill) {
        			player.removeSkill(skill + "_mark");
        			get.info("lianzhaoEffect").method(player, false, 'dcjizhan');
        		},
        		intro: {
        		    content:function(storage,player){
        		        var first=0.6;
        		        var last=0.6;
        		        if(player.storage.isDcjizhan) {
        		            last=1;
        		        }else {
        		            first=1;
        		        }
        		        return "连招：<span style='opacity:"+first+";'>装备牌</span>→<span style='opacity:"+last+";'>黑色牌</span>";
        		    },
        		},
        		subSkill: {
        			mark: {
        	...
        	lianzhaoEffect: {
        	    method:function(player, num, skill){//0~60
        	        if(lib.config.effect_pot_all&&typeof skinSwitch=='object'&&skinSwitch.chukuangWorkerApi?.playEffect) {
        			    var isMine=player==game.me;//(player==game.me||player.isUnderControl());
        			    var positionX=isMine?-0.3:0.55;
        			    var scale=isMine?1:0.8;
        			    if(!player.wei_sword_skills) {
        			        player.wei_sword_skills = {};
        			    }
        			    if(typeof skill=='string') {
        			        player.wei_sword_skills[skill] = num;
        			    }
        			    for(var s in player.wei_sword_skills) {
        			        var max=player.wei_sword_skills[s];
        			        if(max>num) num=max;
        			    }
        			    skinSwitch.chukuangWorkerApi.playEffect({
                            name: "../../../十周年UI/assets/animation/lianzhao_jindu",
                            version: "4.0",
                        },{
                            scale: scale,
                            speed: 1,
                        ...
                        if(isMine&&num!==false) {
                            let jian;
                            if(!player.wei_sword) {
                                jian=ui.create.div(player);
                                game.createCss(`.wei_sword{
                                    /*width: 70px;
                                    height: 195px;*/
                                    width: 68px;
                                    height: 193px;
                                ...
                                jian.classList.add('wei_sword');
                                jian.jian0=ui.create.div(jian);//.classList.add('wei_sword0_type'+num);
                                jian.jian1=ui.create.div(jian);//.classList.add('wei_sword1_type'+num);
                                player.wei_sword=jian;
                            }else {
                                jian=player.wei_sword;
                            }
                            // 清除以 'wei_sword' 开头的类名
                            Array.from(jian.jian0.classList) // 将类列表转换为数组
                              .filter(className => className.startsWith('wei_sword0_')) // 找到需要删除的类
                              .forEach(className => jian.jian0.classList.remove(className)); // 逐个删除
                            Array.from(jian.jian1.classList) // 将类列表转换为数组
                              .filter(className => className.startsWith('wei_sword1_')) // 找到需要删除的类
                              .forEach(className => jian.jian1.classList.remove(className)); // 逐个删除
                            game.createCss(`.wei_sword0_type${num}{
                                width: 100%;
                                height: ${100*(60-num)/60}%;
                                left: 0;
                                top: 0;
                                background-image: url('${lib.assetURL}image/skill_wei/sword0.png');
                                background-size: 100% ${6000/(60-num)}%;
                                background-position: center top;
                                background_repeat: no-repeat;
                            }`);
                            game.createCss(`.wei_sword1_type${num}{
                                width: 100%;
                                height: ${100*num/60}%;
                                left: 0;
                                bottom: 0;
                                background-image: url('${lib.assetURL}image/skill_wei/sword1.png');
                                background-size: 100% ${6000/num}%;
                                background-position: center bottom;
                                background_repeat: no-repeat;
                            }`);
                            jian.jian0.classList.add('wei_sword0_type'+num);
                            jian.jian1.classList.add('wei_sword1_type'+num);
                        }else if(player.wei_sword&&(!get.info("lianzhaoEffect").hasWei(player)||!isMine)) {
                            player.removeChild(player.wei_sword);
                            player.wei_sword=undefined;
                        }else if(player.wei_sword&&player.wei_sword[skill]) {
                            delete player.wei_sword[skill];
                        }
        			}
        	    },
        	    hasWei:function(player){
        	        if(get.itemtype(player)!='player') return 0;
        	        let skills=player.getSkills(null,false,false).filter(function(i){
						return lib.skill[i]?.comboSkill;
					});
		...
		characterIntro:{
			...
			guannings:'关宁，《三国演义》的虚构人物，为关定之子，关平的哥哥，学文。关羽前往冀州寻找刘备时曾居于关定庄，关定命关宁、关平二子出拜。后关羽同刘备回到关定庄时，关羽向刘备介绍关宁、关平二人，关定即提出让关平拜关羽为义父。',
		...
		dynamicTranslate:{
			mxquanmou:function(player){
			    if(player.storage.mxquanmou) return '转换技，出牌阶段每名角色限一次，你可以令攻击范围内一名其他角色交给你一张牌，阳：防止你此阶段下次对其造成的伤害；<span class="bluetext">阴：此阶段你下次对其造成伤害后，可以对至多三名其他角色(该角色除外)各造成1点伤害。</span>';
			    return '转换技，出牌阶段每名角色限一次，你可以令攻击范围内一名其他角色交给你一张牌，<span class="bluetext">阳：防止你此阶段下次对其造成的伤害；</span>阴：此阶段你下次对其造成伤害后，可以对至多三名其他角色(该角色除外)各造成1点伤害。';
            },
			...
			dcdianlun:function(player){
			    var numstr = '等量';
			    if(player.hasSkill('dcdianlun_double')) numstr = '两倍';
			    return "出牌阶段限一次，你可以弃置任意张点数差相同（即等差数列）的手牌，然后摸"+numstr+"的牌，这些牌本回合不受〖肃纲〗的使用限制且无距离和次数限制。";
		    },
		},
		perfectPair:{
		...
		translate:{
			...
			mx_jiangwei_ab:'谋姜维',
			mxzhuri:'逐日',
			mxzhuri_info:'你的阶段结束时，若你本阶段失去或得到过牌，则你可以与一名角色拼点。若你赢，你可以使用其中一张拼点牌；若你没赢，你失去1点体力或令此技能于本回合无效。',
			mxranji:'燃己',
			mxranji_info:'限定技，结束阶段。若你本回合使用过牌的阶段数大于等于/小于等于体力值，你可以获得技能〖困奋〗/〖诈降〗（同时满足则都获得）。若如此做，你将手牌数调整至手牌上限或将体力值回复至体力上限，然后你不能回复体力直到你杀死角色。',
			mx_simayi:'DC谋司马懿',
            my_simayi:'DC谋司马懿',
            mx_simayi_ab:'谋司马懿',
            my_simayi_ab:'谋司马懿',
            mxpingliao:'平辽',
            mxpingliao_info:'锁定技，当你使用【杀】指定目标时，不公开指定的目标。你攻击范围内的其他角色同时选择是否打出一张红色基本牌。若此【杀】的目标未打出基本牌，其本回合无法使用或打出手牌；若有至少一名非目标打出基本牌，你摸两张牌且此阶段出【杀】次数+1。',
            mxquanmou:'权谋',
            mxquanmou_info:'转换技，出牌阶段每名角色限一次，你可以令攻击范围内一名其他角色交给你一张牌，阳：防止你此阶段下次对其造成的伤害；阴：此阶段你下次对其造成伤害后，可以对至多三名其他角色(该角色除外)各造成1点伤害。',
            mx_taishici: "OL谋太史慈",
            mx_taishici_ab: "谋太史慈",
            olsbdulie: "笃烈",
            ...
            dcporong: "破戎",
            dcporong_info: "连招技（伤害牌+【杀】）。你可获得此【杀】目标与其相邻角色各一张手牌，并令此牌对其额外结算一次。",
			v_sunquan: "威孙权",
        	v_sunquan_prefix: "威",
        	dcwoheng: "斡衡",
