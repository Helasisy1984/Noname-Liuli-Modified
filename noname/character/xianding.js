...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		character:{
            ...
            wu_huangfusong:['male','qun','1/4',['dcchaozhen','dclianjie','dcjiangxian']],
			...
			dc_zhouxuans:['male','wei',3,['dcwumei','dczhanmeng']],
		    ...
		    dc_malingli:['female','shu',3,['dclima','dcxiaoyin','dchuahuo']],
        },
		characterSort:{
			xianding:{
				sp2_huben:['wangshuang','wenyang','re_liuzan','dc_huangzu','wulan','leitong','chentai','dc_duyu','dc_wangjun'],
				sp2_shengun:["puyuan","guanlu","gexuan",'wufan','re_zhangbao','dukui','zhaozhi','zhujianping','dc_zhouxuans','zerong'],
				sp2_bizhe:['dc_luotong','dc_wangchang','chengbing','dc_yangbiao','ruanji'],
				sp2_huangjia:['caomao','liubian','dc_liuyu','quanhuijie','dingshangwan','yuanji','xielingyu','sunyu','ganfurenmifuren','dc_ganfuren','dc_mifuren','zhupeilan'],
				sp2_zhangtai:['guozhao','fanyufeng','ruanyu','yangwan','re_panshu'],
				sp2_jinse:['caojinyu','re_sunyi','re_fengfangnv','caohua','laiyinger','zhangfen','caoxian','zhugemengxue','zhugeruoxue'],
				sp2_yinyu:['zhouyi','luyi','sunlingluan'],
				sp2_doukou:['re_xinxianying','huaman','xuelingyun','dc_ruiji','duanqiaoxiao','tianshangyi','dc_malingli'],
				sp2_jichu:['zhaoang','dc_liuye','dc_wangyun','yanghong','huanfan','xizheng'],
				sp2_yuxiu:['dongguiren','dc_tengfanglan','zhangjinyun','zhoubuyi','zhugejing'],
		...
		skill:{
        	...
        	dcyanzuo: {
        		...
        		content: function() {
        			...
        			player.chooseButton(["是否视为使用其中一张牌？", cards])
			...
			dcjichun:{
                ...
                content:function(){
                    ...
                    var list=[];
                    if(game.hasPlayer(p=>p.countCards('h')<player.countCards('h'))) list.push('选项一');
                    list.push('选项二');
                    player.chooseControl(list).set('choiceList',[
                        '将'+get.translation(cards[0])+'交给一名手牌数小于你的角色，然后你摸'+lib.skill.dcweidang.getLength(cards[0])+'张牌',
                        '弃置'+get.translation(cards[0])+'，并弃置一名手牌数大于你的角色区域内至多'+lib.skill.dcweidang.getLength(cards[0])+'张牌',
                    ]).set('prompt','选择一项').set('ai',function(){
						var player=_status.event.player;
                        if(game.hasPlayer(p=>p.countCards('h')<player.countCards('h')&&get.attitude(p,player)>0)) return '选项一';
                        else return '选项二';
                    });
                    'step 1'
            ...
            dchanying:{
        		...
        		if(list.length>0){
            		...
            		if(!game.hasPlayer(function(target){
            		    return target.countCards('h')==player.countCards('h')&&target.canEquip(event.card.name);
            		})) event.finish();
        		}
        		else event.finish();
        		'step 1'
        		...
        		'step 2'
        		if(result.bool&&result.targets) result.targets[0].equip(event.card);
    			},
            },
			//诸葛若雪
			...
			dcqiongying: {
				...
				content: function (){
					...
					if (result.bool) {
						...
						var cards = player.getCards('h', o => get.suit(o) == suit);
						if (cards.length) {
			...
			dcnuanhui: {
				...
				content: function () {
					...
					event.useNum = 0;
					event.useList=[];
					event.dcnuanhuiDiscard=false;
					event.dcnuanhuiEnd=false;
					'step 2'
					var list = [];
					...
					if(result.bool){
						event.useNum++;
						event.target.chooseUseTarget(true, false, result.links[0][2]);
						if(!event.useList.includes(result.links[0][2])) event.useList.push(result.links[0][2]);
						else event.dcnuanhuiDiscard=true;
					}
					else event.dcnuanhuiEnd=true;
					'step 4'
					if (event.useNum >= event.numx||event.dcnuanhuiEnd) {
						if (event.dcnuanhuiDiscard===true) {
							event.target.lose(event.target.getCards('e'));
						}
						event.finish();
					} else if(!event.target.isDead()&&!event.dcnuanhuiEnd){
						event.goto(2);
					} else {
						event.finish();
					}
	        ...
	        dclingxi: {
	        	...
	        	content: function() {
		        	...
		        	player.chooseCard(get.prompt("dclingxi"), "将至多" + get.cnNumber(num) + "张牌称为“翼”置于武将牌上", "he", [1, num])
		        		.set("ai", card => {
	        	...
	        	subSkill: {
		        	effect: {
			        	...
			        	filter: function (event, player) {
			        		var expd=player.getExpansions("dclingxi");
			        		var suits=[];
			        		expd.forEach(card=>suits.add(get.suit(card, false)));
			        		var num = 2 * suits.length;//.reduce((list, card) => list.add(get.suit(card, false)), []).length;
	        ...
	        dczhifou: {
	        	...
	        	content: function(event, map) {
				    ...
				    if(result.control) {
			        	...
			        	switch (result.control) {
			        		case "置入“翼”":
				        		...
				        		target.chooseCard("he", choiceList[0], true);
				        		break;
				        	case "弃置卡牌":
		        	...
		        	'step 4'
		        	var target = event.target;
		        	if (result.bool) player.addToExpansion(result.cards, target, "give").gaintag.add("dclingxi");
	        	},
	        	subSkill: {
		        	0: { charlotte: true },
		        	1: { charlotte: true },
			...
			dccilv: {
				...
				content() {
					...
					'step 1'
					if (player.countCards("h") <= player.maxHp) event.finish();
					'step 2'
					let list = ["无效", "防伤", "获得"].filter(i => !player.getStorage("dccilv").includes(i));
					if (list.length == 1) {
					    result = { control: list[0] };
					}else {
						player.chooseControl(list)
			...
			dctongdao: {
				...
				content: function() {
					'step 0'
					player.chooseTarget(get.prompt2("dctongdao"))
						.set("ai", target => {
							const player = _status.event.player;
			...
			dcxiaoyin: {
				...
				content: function () {
					...
					if (event.list.length > 0)
						player.chooseButton(['硝引：选择准备当火药的牌', event.list], [1, event.list.length]);
					else
						event.finish();
					'step 3'
					if (result.bool) {
				...
				subSkill: {
					fire: {
						...
						filter: function (event,player) {
							return event.source&&event.source.isIn()&&player.getExpansions('dcxiaoyin').length > 0;
						},
						content: function () {
							'step 0'
							if (trigger.nature == 'fire') {
								trigger.source.chooseBool('弃置所有“硝引”牌并弃置一张与“硝引”牌类型相同的牌令本次伤害+1').set('ai', function () {
									return get.attitude(trigger.sourse, player) <= 0;
								});
							} else {
			...
			dczhizhe:{
				...
				content:function(){
					'step 0'
					if(lib.config.effect_wu_zhugeliang) {
    					let spine=game.playAnimationTitle('wuzhugeliang_effect.png',false,true);
    					game.playAnimation('extension/标记补充/animation/wuzhugeliang.mp4',function(){
    					    setTimeout(function() {
    					        spine.start();
    					    },5600);
    					    _status.tempStopMusic=true;
    					    game.playBackgroundMusic();
    					    game.helaAudio('../audio/background/effect_wu_zhugeliang.mp3','background',function(){
    					        _status.tempStopMusic=undefined;
    					        game.playBackgroundMusic();
    					    });
    					},function(){
    					    spine.end();
			...
			dcxiongmu:{
                    ...
                    for(var pile of piles){
                        for(var i=0;i<ui[pile].childNodes.length;i++){
                            ...
                            if(!list.contains(card)&&number==8){
                                list.push(card);
                                if(list.length>=event.cards.length) break;
                            }
                        }
                        if(list.length>=event.cards.length) break;
                    }
                ...
                subSkill:{
                    minus:{
                        ...
                        /*filter:function(event,player){
                            return game.getGlobalHistory('every个thing',evt=>{
                                return evt.name=='damage'&&evt.player==player;
                            },event).indexOf(event)==0;
                        },*/
                        //Helasisy修一下
                        filter:function(event,player){
                            return player.countCards('h')<=player.getHp();//&&!player.getHistory('damage').length;
                        },
                        round:1,
                        forced:true,
                        locked:false,
			...
			dcwuyou: {
				...
				subSkill: {
					g: {
        				...
        				content: function() {
        					'step 0'
        					event.isMe = target == player;
        					event.cards = cards;
        					if (!event.isMe) {
        						player.give(cards, target);
        					}
        					'step 1'
        					var names = lib.inpile
        						.filter(name => {
        					...
        					'step 3'
        					event.viewAs = { name: event.links[0][2], nature: event.links[0][3] };
        					if (!event.isMe) {
        						target
        							.chooseCard(/*player*/'h', 1, true)
        							.set("ai", card => {
        								const player = _status.event.player,
        					...
        					if (event.target.getCards("h").includes(card)) {
        						var card = event.cards[0];
								if (event.target != player) event.target.give(card, player).set("gaintag", ["dcwuyou"]);
								card.addGaintag('dcwuyou');
								if (typeof player.storage.dcwuyou!= 'object') player.storage.dcwuyou = new Map();
								player.storage.dcwuyou.set(card, event.viewAs.name);
        					}
        				},
        				ai: {
        					order: 10,
						...
						mod: {
							cardname: function (card, player) {
								if (get.itemtype(card) == 'card' && card.hasGaintag('dcwuyou') && typeof player.storage.dcwuyou== 'object' && player.storage.dcwuyou.has(card)) return player.storage.dcwuyou.get(card);
							},
							cardnature: function (card, player) {
								if (get.itemtype(card) == 'card' && card.hasGaintag('dcwuyou') && typeof player.storage.dcwuyou== 'object' && player.storage.dcwuyou.has(card)) return undefined;
							},
							cardUsable: function (card) {
			...
			dclianjie: {
				...
				content: function () {
					...
					"step 1"
					if (result.bool && result.targets?.length) {
						player.logSkill('dclianjie', result.targets);
						var target = result.targets[0];
			...
			dcwumei:{
				...
				content:function(){
					...
					if(result.bool){
						...
						var targets=game.filterPlayer();
						//if(!target.storage.dcwumei_wake) 
			...
			liji:{
				...
				check:function(card){
					//标记一处地点
					var val=get.value(card);
					if(!_status.event.player||!_status.event.player.getStorage('refenyin_mark')) return 8-val;
					var storages=_status.event.player.getStorage('refenyin_mark');
					if(typeof storages =='string') {
					    if(storages.indexOf(get.translation(get.suit(card)))!=-1) return 12-val;
					}else {
					    if(storages.contains(get.suit(card))) return 12-val;
					}
					return 8-val;
				},
				filterTarget:lib.filter.notMe,
		...
		characterIntro:{
			...
			dc_zhouxuans:'周宣，生卒年不详，字孔和，乐安（治今山东高苑西北）人，擅长解梦，在郡做官时曾为太守解过梦，后又给曹丕解梦，都灵验。官至中郎，死于魏明帝末年。',
		...
		translate:{
			...
			dcwuyou: '武佑',
			dcwuyou_info: '每名角色的出牌阶段限一次，其可以交给你一张手牌（若为你则跳过此步骤），然后你从五个随机的非装备牌的牌名中选择一个牌名并交给其一张手牌（此牌视为你选择的牌名且使用时无距离与次数限制）。',
			dcyixian: '义贤',
			...
			zhugeruoxue: '诸葛若雪',
			dcqiongying: '琼英',
			dcqiongying_info: '出牌阶段限一次，你可以移动场上的一张牌，然后你弃置一张与此牌花色相同的手牌（若没有则展示所有手牌）。',
			dcnuanhui: '暖惠',
			dcnuanhui_info: '结束阶段，你可以选择一名角色，该角色可以视为使用X张基本牌（X为其装备区里的牌数且至少为1）。若使用了同名牌，其弃置装备区里的所有牌。',
			zhugemengxue:'诸葛梦雪',
			dcjichun:"寄春",
            dcjichun_info:"出牌阶段限一次，你可以展示一张牌并选择一项：1、将此牌交给一名手牌数小于你的角色，然后你摸X张牌；2、弃置此牌，并弃置一名手牌数大于你的角色区域内至多X张牌（X为此牌名字数）。",
            dchanying:"寒英",
