		...
		card:{
			diaobingqianjiang:{
				...
				contentBefore:function(){
					...
					var dialog=ui.create.dialog(game.changeToGoldTitle('调兵遣将'),cards,true);
					_status.dieClose.push(dialog);
					dialog.videoId=lib.status.videoId++;
					game.addVideo('cardDialog',null,[game.changeToGoldTitle('调兵遣将'),get.cardsInfo(cards),dialog.videoId]);
				...
				content:function(){
					...
					if(target.isUnderControl(true)){
						event.dialog.setCaption(game.changeToGoldTitle('调兵遣将')+'<br>选择一张牌并用一张手牌替换之');
					}
					...
					"step 1"
					event.dialog.setCaption(game.changeToGoldTitle('调兵遣将')+'<br>正在调兵遣将..');
					if(result.bool){
						event.button=result.buttons[0];
						target.chooseCard('用一张牌替换'+get.translation(result.links),true).ai=function(card){
				...
				contentAfter:function(){
					...
					if(player.isUnderControl(true)&&!_status.auto){
						event.dialog.setCaption(game.changeToGoldTitle('调兵遣将')+'<br>将任意张牌以任意顺序置于牌堆顶（先选择的在上）');
					}
